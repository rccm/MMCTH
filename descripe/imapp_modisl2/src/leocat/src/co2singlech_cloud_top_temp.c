/*$Id: co2singlech_cloud_top_temp.c,v 1.1.1.1 2006/10/02 22:40:37 mpav Exp $*/
...
void co2singlech_cloud_top_temp (unsigned char verbose,
                                 imagerL1* imgr1, imagerL2* imgr2, 
			         sounderL1* sndr1, sounderL2* sndr2,
			         nwp_params nwp, rtm_profiles** rtm, 
			         rtm_toa rclr, rad_utils rutil)
{
  char *rout = {"co2singlech_cloud_top_temp.c"};
  unsigned char  wv_corr = 1;
  const int nchan = 6, it[] = {2}, ib[] = {0}, chan_num[] = {31, 32, 33, 34, 35, 36};
  ...
  long i, ii;
  int i_nwp, ivza, ilev, isol, iprof, iprof_win, il, l, ret_val, imin=0, j;
  size_t nsol;
