/*$Id: modis_geo_interp_1000.c,v 1.1.1.1 2006/10/02 22:40:37 mpav Exp $*/
...

{
  ...
  for (index=0; index<npts_1; index++) ptr1000[index] = MISSING_FLOAT;
  ...
  for (scan=0; scan < nscan; scan++) {
	...
	if (temp[index] == MISSING_FLOAT) bad_flg = 1;
	index++;
      }
    }
    
    if (bad_flg) {
      fprintf(stdout,"%sWARNING: found bad 5 km geolocation scan lines.\n",EXE_PROMPT);
      continue;
    }
	...
	if (scan == DEBUG && icol == 2 && irow == 2) {
	  fprintf(stderr,"i=%d  ip=%d  ix=%f  j=%d  jp=%d  jy=%f\n",i,ip, ix[icol],j,jp,jy[irow-k0]);
	  fprintf(stderr,"dx=%f  dx1=%f  dy=%f  dy1=%f\n",dx,dx1,dy,dy1);
	  fprintf(stderr,"index1=%ld  index2=%ld  index3=%ld  index4=%ld\n",index1,index2,index3,index4);
	...
	if (scan == DEBUG && icol == 2 && irow == 2) printf("intep=%f\n",ptr1000[((irow)*ncols_1000)+icol]);
      }
    }
