/******************************************************************************/
/* Read in VIIRS Level 1 MOD or IMG resoution data from HDF5 Scientific Data  */
/* Sets.  Every line and pixel is read into a pointer for a given band.       */
/*                                                                            */
/* Input:                                                                     */
...
int viirs_level1_read (char *dsetName, hid_t fileId, void **buf_ptr, int *nrank, int *nrows, int *ncols)
{
	hid_t       dsetID, dspaceID, dTypeID;          /* Handles */
	herr_t      h5_dsetStatus,h5_dspaceStatus,h5_dsetReadStatus;
	hsize_t     dimSize[2];
	int rank, row, col, idx, npts;
	int rows,cols;
	float *buf_float;
	unsigned short *buf_ushort;
	char *rout = {" viirs_level1_read"};
