...
const float cwn_aqua_shf[] = {2.647418E+03, 2.511763E+03, 2.517910E+03, 2.462446E+03,
                              2.248296E+03, 2.209550E+03, 0.0000000000, 1.479292E+03,
		              1.363638E+03, 1.169637E+03, 1.028715E+03, 9.076808E+02,
		              8.308397E+02, 7.482977E+02, 7.315760E+02, 7.190090E+02,
		              7.045020E+02};

const float tcs_aqua_shf[] = {9.993438E-01, 9.998680E-01, 9.998649E-01, 9.998729E-01,
                              9.998738E-01, 9.998774E-01, 0.0000000000, 9.995754E-01,
		              9.994906E-01, 9.995439E-01, 9.997496E-01, 9.995483E-01,
		              9.997404E-01, 9.999194E-01, 9.999071E-01, 9.999177E-01,
		              9.999211E-01};

const float tci_aqua_shf[] = {4.792821E-01, 9.260598E-02, 9.387793E-02, 8.659482E-02,
                              7.854801E-02, 7.521532E-02, 0.0000000000, 1.828557E-01,
		              2.051362E-01, 1.628724E-01, 8.003410E-02, 1.290129E-01,
		              6.810679E-02, 1.895925E-02, 2.131206E-02, 1.858586E-02,
    		              1.737030E-02};

const float cwn_terra_shf[] = {2.641767E+03, 2.505274E+03, 2.518031E+03, 2.465422E+03,
                               2.235812E+03, 2.200345E+03, 0.0000000000, 1.478026E+03,
		               1.362741E+03, 1.173198E+03, 1.027703E+03, 9.081998E+02,
		               8.315149E+02, 7.483224E+02, 7.317089E+02, 7.196677E+02,
		               7.055309E+02};

const float tcs_terra_shf[] = {9.993487E-01, 9.998699E-01, 9.998604E-01, 9.998701E-01,
                               9.998825E-01, 9.998849E-01, 0.0000000000, 9.994942E-01,
		               9.994937E-01, 9.995643E-01, 9.997499E-01, 9.995880E-01,
		               9.997388E-01, 9.999192E-01, 9.999170E-01, 9.999176E-01,
		               9.999267E-01};

const float tci_terra_shf[] = {4.744530E-01, 9.091094E-02, 9.694298E-02, 8.856134E-02,
                               7.287017E-02, 7.037161E-02, 0.0000000000, 2.177889E-01,
		               2.037728E-01, 1.559624E-01, 7.989879E-02, 1.176660E-01,
		               6.856633E-02, 1.903625E-02, 1.905849E-02, 1.858763E-02,
		               1.617507E-02};
...
float planck_btemp_terra_shf (int band, float rad)

{
  ...
  wl = 1.0e+4/cwn_terra_shf[(int)band-20];
  wl_meters = 1.0e-6 * wl;

  planckT = ((c2_planck / (wl_meters * log(c1_planck /
	     (1.0e+6 * rad * pow(wl_meters,5)) + 1.0))) -
	     tci_terra_shf[(int)band-20])/tcs_terra_shf[(int)band-20];
...
float planck_btemp_aqua_shf (int band, float rad)

{
  ...
  wl = 1.0e+4/cwn_aqua_shf[(int)band-20];
  wl_meters = 1.0e-6 * wl;

  planckT = ((c2_planck / (wl_meters * log(c1_planck /
	     (1.0e+6 * rad * pow(wl_meters,5)) + 1.0))) -
	     tci_aqua_shf[(int)band-20])/tcs_aqua_shf[(int)band-20];
...
float modis_planck_terra_shf(int band, float temp)

{
  ...
  wavelength = (1.0e+4 / cwn_terra_shf[band - 20])*1.0e-6;
  bt = temp * tcs_terra_shf[band - 20] + tci_terra_shf[band - 20];
...
float modis_planck_aqua_shf(int band, float temp)

{
  ...
  wavelength = (1.0e+4 / cwn_aqua_shf[band - 20])*1.0e-6;
  bt = temp * tcs_aqua_shf[band - 20] + tci_aqua_shf[band - 20];
...
double rad_v2w_aqua_shf(int band, double rad) {

  double lamda, I;

  lamda = cwn_aqua_shf[band - 20]/1.0e-2;
  I = rad * 1.0e-5;
...
double rad_v2w_terra_shf(int band, double rad) {

  double lamda, I;

  lamda = (double)cwn_terra_shf[band - 20]/1.0e-2;
  I = rad * 1.0e-5;
