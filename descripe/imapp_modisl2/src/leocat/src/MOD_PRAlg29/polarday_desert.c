...
!Description:
  ...
  Integer function polarday_desert.c
...
/*  Perform cloud tests. */
//  printf("\nPerforming cloud tests in polarday_desert.c\n");
...
/*  Water vapor high cloud test. */
    ...
    if(rintf(m27) != rintf(bad_data)) {
      ...
      if(m27 >= pdsh20[1]) (void) set_bit(15, pxout.testbits);
      conf = conf_test(m27, pdsh20[0], pdsh20[2], pdsh20[3],
                                  pdsh20[1]);
...
/*  11 minus 4 micron BTDIF fog and low cloud test. */
    ...
    if (pxin.visusd) {
      if (rintf(m31) != rintf(bad_data) && rintf(m22) != rintf(bad_data)) {
          ...
          if (m31_m22 >= pds11_4lo[1] && m31_m22 <= pds11_4hi[1])
            (void) set_bit(19, pxout.testbits);

          locuta[0] = pds11_4lo[0];
          locuta[1] = pds11_4hi[0];
          hicuta[0] = pds11_4lo[2];
          hicuta[1] = pds11_4hi[2];
          midpta[0] = pds11_4lo[1];
          midpta[1] = pds11_4hi[1];
          conf = conf_test_dble(m31_m22, locuta, hicuta, 1.0, midpta);
          cmin2 = min(cmin2, conf);
          ngtests[1]++;
//        printf("11-4: %f %f %f %f %f %f %f %f %f %f %d %d\n", pdsbt1[0], m31_m22, locuta[0], locuta[1],
//          midpta[0], midpta[1], hicuta[0], hicuta[1], conf, cmin2, num_tests, ngtests[1]);
...
//      printf("B1: %d %f %f %f %f %f %f %f\n", irtn, mvis, b1_locut, b1_midpt,
//        		    b1_hicut, b1_power, conf, cmin3);
//      printf("Vis: %f %f %f %f %f %f %d %d\n", m02, pdsref2[0],
//        pdsref2[1], pdsref2[2], conf, cmin3, num_tests, ngtests[2]);
...
/*  GEMI test. */
    ...
    if(rintf(m01) != rintf(bad_data) && rintf(m02) != rintf(bad_data) &&
      ...
      if(pxin.ndvibk < 0.1) {
        pdsgemi[0] = pdsgemi0[0];
        pdsgemi[1] = pdsgemi0[1];
        pdsgemi[2] = pdsgemi0[2];
      }
      else if (pxin.ndvibk < 0.2) {
        pdsgemi[0] = pdsgemi1[0];
        pdsgemi[1] = pdsgemi1[1];
        pdsgemi[2] = pdsgemi1[2];
      }
      else if (pxin.ndvibk < 0.3) {
        pdsgemi[0] = pdsgemi2[0];
        pdsgemi[1] = pdsgemi2[1];
        pdsgemi[2] = pdsgemi2[2];
      }

      if(gemi > pdsgemi[1]) (void) set_bit(21, pxout.testbits);
      conf = conf_test(gemi, pdsgemi[0], pdsgemi[2], 1.0, pdsgemi[1]);
      cmin3 = min(cmin3, conf);
      ngtests[2]++;

//    printf("gemi %f %f %f %f %f %f %f %f %f %d %d \n", m01, m02, pxin.ndvibk, gemi,
//      pdsgemi[0], pdsgemi[1], pdsgemi[2], conf, cmin3, ngtests[2],
//   	num_tests);
...
/*  Near-infrared (1.38 micron) high cloud test. */
    ...
    if (pxin.visusd && pxin.hi_elev == 0 && pxin.tpw > pds_ref3_tpw[0]) {
      if (rintf(m26) != rintf(bad_data)) {

        num_tests++;
        (void) set_bit(16, pxout.qabits);

        if (m26 <= pdsref3[1]) (void) set_bit(16, pxout.testbits);
        conf = conf_test(m26, pdsref3[0], pdsref3[2], 1.0, pdsref3[1]);
        cmin4 = min(cmin4, conf);
        ngtests[3]++;
//      printf("1.38: %f %f %f %f %f %f %d %d\n", m26,pdsref3[0],
//           pdsref3[1],pdsref3[2],conf,cmin4,num_tests,ngtests[3]);
