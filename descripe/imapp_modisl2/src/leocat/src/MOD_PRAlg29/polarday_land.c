...
!Revision History:
  ...
  at CIMSS, UW-Madison.
  Convertedd to C             R. Frey           10/2007
...
/*  Water vapor high cloud test. */
    ...
    if(rintf(m27) != rintf(bad_data)) {
      ...
      if(m27 >= pdlh20[1]) (void) set_bit(15, pxout.testbits);
      conf = conf_test(m27, pdlh20[0], pdlh20[2], pdlh20[3],
                                  pdlh20[1]);
...
/*  11 minus 4 micron BTDIF fog and low cloud test. */
    ...
    if (pxin.visusd) {
      if (rintf(m31) != rintf(bad_data) && rintf(m22) != rintf(bad_data)) {
        ...
        if (m31_m22 >= pdl11_4lo[1]) (void) set_bit(19, pxout.testbits);
        conf = conf_test(m31_m22, pdl11_4lo[0], pdl11_4lo[2], 1.0,
                         pdl11_4lo[1]);
        cmin2 = min(cmin2, conf);
        ngtests[1]++;
//      printf("11-4: %f %f %f %f %f %f %d %d\n", m31_m22, pdl11_4lo[0],
//        pdl11_4lo[1], pdl11_4lo[2], conf, cmin2, num_tests, ngtests[1]);
...
//      printf("B1: %d %d %d %f %f %f %f %f %f %f\n", pxin.elem, pxin.eco_type, irtn, m01, b1_locut,
//        		    b1_midpt, b1_hicut, pxin.sctang, pxin.ndvibk, conf);
//      printf("Vis: %f %f %f %f %f %f %d %d\n", m01, pdlref1[0],
//        pdlref1[1], pdlref1[2], conf, cmin3, num_tests, ngtests[2]);
...
/*  Near-infrared (1.38 micron) high cloud test. */
    ...
    if (pxin.visusd && pxin.hi_elev == 0 && pxin.tpw > pdl_ref3_tpw[0]) {
      if (rintf(m26) != rintf(bad_data)) {

        num_tests++;
        (void) set_bit(16, pxout.qabits);

        if (m26 <= pdlref3[1]) (void) set_bit(16, pxout.testbits);
        conf = conf_test(m26, pdlref3[0], pdlref3[2], 1.0, pdlref3[1]);
        cmin4 = min(cmin4, conf);
        ngtests[3]++;
//      printf("1.38: %f %f %f %f %f %f %d %d\n", m26,pdlref3[0],
//           pdlref3[1],pdlref3[2],conf,cmin4,num_tests,ngtests[3]);
