...
!Description:
  ...
  Integer function landday_coast.c
  Computes initial clear sky confidence for coastal, non-arid daytime
    surfaces.
...
/*  Perform cloud tests. */
//  printf("\nPerforming cloud tests in landday_coast.c\n");
...
/*  Co2 high cloud test. */
    ...
    if(rintf(m35) != rintf(bad_data)) {
      ...
      if(m35 >= dlco2_t2[1]) (void) set_bit(14, pxout.testbits);
      conf = conf_test(m35, dlco2_t2[0], dlco2_t2[2], dlco2_t2[3],
                                  dlco2_t2[1]);
...
/*  Water vapor high cloud test. */
    ...
    if(rintf(m27) != rintf(bad_data)) {
      ...
      if(m27 >= dlh20_t2[1]) (void) set_bit(15, pxout.testbits);
      conf = conf_test(m27, dlh20_t2[0], dlh20_t2[2], dlh20_t2[3],
                                  dlh20_t2[1]);
      cmin1 = min(cmin1, conf);
      ngtests[0]++;
...
/*  11 minus 4 micron BTDIF fog and low cloud test. */
    ...
    if (pxin.visusd) {
      if (rint(m31) != rint(bad_data) && rint(m22) != rint(bad_data)) {
        ...
        if(m31_m22 >= dl11_4lo_t2[1]) (void) set_bit(19, pxout.testbits);
        conf = conf_test(m31_m22, dl11_4lo_t2[0], dl11_4lo_t2[2], 1.0,
                         dl11_4lo_t2[1]);
        cmin2 = min(cmin2, conf);
        ngtests[1]++;
//      printf("11-4: %f %f %f %f %f %f %d %d\n", m31_m22, dl11_4lo_t2[0],
//       dl11_4lo_t2[1], dl11_4lo_t2[2], conf, cmin2, num_tests,
//       ngtests[1]);
...
//        dlref1_t2[1], dlref1_t2[2], conf, cmin3, num_tests,
//        ngtests[2]);
...
/*  Near-infrared (1.38 micron) high cloud test. */
    ...
    if (pxin.visusd && pxin.hi_elev == 0 && pxin.tpw > dl_ref3_tpw_t2[0]) {
      if (rint(m26) != rint(bad_data)) {

        num_tests++;
        (void) set_bit(16, pxout.qabits);

        if (m26 <= dlref3_t2[1]) (void) set_bit(16, pxout.testbits);
        conf = conf_test(m26, dlref3_t2[0], dlref3_t2[2], 1.0,
                    dlref3_t2[1]);
        cmin4 = min(cmin4, conf);
        ngtests[3]++;
//      printf("1.38: %f %f %f %f %f %f %d %d\n", m26,dlref3_t2[0],
//        dlref3_t2[1],dlref3_t2[2],conf,cmin4,num_tests,ngtests[3]);
