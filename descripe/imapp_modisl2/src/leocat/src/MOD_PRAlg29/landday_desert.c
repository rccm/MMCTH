...
!Description:
  ...
  Integer function landday_desert.c
  Computes initial clear sky confidence for non-coastal, arid daytime
    land surfaces.
  Called from land_day.c
...
!Revision History:
  ...
  at CIMSS, UW-Madison.
  Convertedd to C             R. Frey           06/2007
...
//  float btc22, btc31, btc32;
    ...
    float fac;
    float gemi;
    float mvis, m01, m02, m08, m22, m26, m27, m31, m32, m35;
...
/*  Perform cloud tests. */
//  printf("\nPerforming cloud tests in landday_desert.c\n");
...
/*  Co2 high cloud test. */
    ...
    if(rintf(m35) != rintf(bad_data)) {
      ...
      if(m35 >= ldsco2[1]) (void) set_bit(14, pxout.testbits);
      conf = conf_test(m35, ldsco2[0], ldsco2[2], ldsco2[3],
                                  ldsco2[1]);
      cmin1 = min(cmin1, conf);
      ngtests[0]++;
//    printf("co2 %f %f %f %d %d\n", m35,conf,cmin1,num_tests,
//            ngtests[0]);
    }
...
/*  Water vapor high cloud test. */
    ...
    if(rintf(m27) != rintf(bad_data)) {
      ...
      if(m27 >= ldsh20[1]) (void) set_bit(15, pxout.testbits);
      conf = conf_test(m27, ldsh20[0], ldsh20[2], ldsh20[3],
                                  ldsh20[1]);
      cmin1 = min(cmin1, conf);
      ngtests[0]++;
...
//    printf("Ci thr desert: %f %f %f %f %f %f %f %f %f\n", btd_thr, m31, m32, m31_m32, pxin.emissfc31, pxin.emissfc32,
//    		  btc22, btc31, btc32);
    ...
    if (pxin.visusd) {
      if (rintf(m31) != rintf(bad_data) && rintf(m22) != rintf(bad_data)) {
    	if(m31 <= ldsbt1[0]) {
          ...
          if (m31_m22 >= lds11_4lo[1] && m31_m22 <= lds11_4hi[1])
...
//        printf("11-4: %f %f %f %f %f %f %f %f %f %f %d %d\n", ldsbt1[0],
//          m31_m22, locuta[0], locuta[1], midpta[0], midpta[1],
//          hicuta[0], hicuta[1], conf, cmin2, num_tests, ngtests[1]);
...
//      printf("B1: %d %f %f %f %f %f %f %f %f %f\n", irtn, lds_ndvi[0], mvis,
//         b1_locut, b1_midpt, b1_hicut, pxin.ndvibk, pxin.sctang, conf, cmin3);
//      printf("Vis: %f %f %f %f %f %f %d %d\n", m02, ldsref2[0],
//        ldsref2[1], ldsref2[2], conf, cmin3, num_tests, ngtests[2]);
...
/*  GEMI test. */
    ...
    if(rintf(m01) != rintf(bad_data) && rintf(m02) != rintf(bad_data) &&
          pxin.visusd) {

      num_tests++;
      (void) set_bit(21, pxout.qabits);

      r1 = 2.0 * ( powf(m02, 2.0) - powf(m01, 2.0) ) + (1.5 * m02) + (0.5 * m01);
      r2 = m02 + m01 + 0.5;
      r3 = r1 / r2;
      gemi = r3 * ( 1.0 - 0.25*r3) - ( (m01 - 0.125) / (1.0 - m01) );

      if(pxin.ndvibk < 0.1) {
        ldsgemi[0] = ldsgemi0[0];
        ldsgemi[1] = ldsgemi0[1];
        ldsgemi[2] = ldsgemi0[2];
      }
      else if (pxin.ndvibk < 0.2) {
        ldsgemi[0] = ldsgemi1[0];
        ldsgemi[1] = ldsgemi1[1];
        ldsgemi[2] = ldsgemi1[2];
      }
      else if (pxin.ndvibk < 0.3) {
        ldsgemi[0] = ldsgemi2[0];
        ldsgemi[1] = ldsgemi2[1];
        ldsgemi[2] = ldsgemi2[2];
      }

      if(gemi > ldsgemi[1]) (void) set_bit(21, pxout.testbits);
      conf = conf_test(gemi, ldsgemi[0], ldsgemi[2], 1.0, ldsgemi[1]);
      cmin3 = min(cmin3, conf);
      ngtests[2]++;

//    printf("gemi %f %f %f %f %f %f %f %f %f %d %d \n", m01, m02, pxin.ndvibk, gemi,
//      ldsgemi[0], ldsgemi[1], ldsgemi[2], conf, cmin3, ngtests[2],
//    	num_tests);
...
/*  Near-infrared (1.38 micron) high cloud test. */
    ...
    if (pxin.visusd && pxin.hi_elev == 0 && pxin.tpw > lds_ref3_tpw[0]) {
      if (rintf(m26) != rintf(bad_data)) {

        num_tests++;
        (void) set_bit(16, pxout.qabits);

        if (m26 <= ldsref3[1]) (void) set_bit(16, pxout.testbits);
        conf = conf_test(m26, ldsref3[0], ldsref3[2], 1.0, ldsref3[1]);
        cmin4 = min(cmin4, conf);
        ngtests[3]++;
//      printf("1.38: %d %f %f %f %f %f %f %f %d %d\n", pxin.elem, pxin.tpw, m26,ldsref3[0],
//           ldsref3[1],ldsref3[2],conf,cmin4,num_tests,ngtests[3]);
