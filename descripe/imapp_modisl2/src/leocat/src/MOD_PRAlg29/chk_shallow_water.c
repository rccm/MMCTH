...
!Description:
   ...
   Routine which checks for extreme values of NDVI in shallow water
   conditions.  Sets confidence to "confident clear" if very low or
...
//  printf("Executing chk_shallow_water \n");
...
//  Ckeck IR SST test.
    ...
    if( qa_test && !bit_test ) sst_clr = 0;
    bit_test = check_bits(16, pxout.testbits);
    qa_test = check_bits(16, pxout.qabits);
    if( qa_test && !bit_test ) nir_clr = 0;
    bit_test = check_bits(20, pxout.testbits);
    qa_test = check_bits(20, pxout.qabits);
    if( qa_test && !bit_test ) nir_clr = 0;

    if( irclr && nir_clr && !sst_clr && !pxin.sh_ocean ) {
    	thr_low = -0.35;
    	thr_hi = swc_ndvi[1];
    }
    else if(irclr && pxin.sh_ocean) {
    	thr_low = swc_ndvi[0];
    	thr_hi = swc_ndvi[1];
    }

    if( (irclr && nir_clr && !sst_clr && !pxin.sh_ocean) || (irclr && pxin.sh_ocean) ) {
...
//  printf("CSW NDVI: %d %f %f %f %f %f %f %f %f\n", irclr, m01, m02, ndvi,
//  		swc_ndvi[0], swc_ndvi[1], thr_low, thr_hi, conf);
