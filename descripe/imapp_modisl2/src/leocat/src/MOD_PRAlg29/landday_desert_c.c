...
!Description:
  ...
  Integer function landday_desert_c.c
  Computes initial clear sky confidence for coastal, arid daytime land
    surfaces.
...
/*  Perform cloud tests. */
//  printf("\nPerforming cloud tests in landday_desert_c.c\n");
...
/*  Co2 high cloud test. */
    ...
    if(rintf(m35) != rintf(bad_data)) {
      ...
      if(m35 >= ldsco2_c[1]) (void) set_bit(14, pxout.testbits);
      conf = conf_test(m35, ldsco2_c[0], ldsco2_c[2], ldsco2_c[3],
                                  ldsco2_c[1]);
...
/*  Water vapor high cloud test. */
    ...
    if(rintf(m27) != rintf(bad_data)) {
      ...
      if(m27 >= ldsh20_c[1]) (void) set_bit(15, pxout.testbits);
      conf = conf_test(m27, ldsh20_c[0], ldsh20_c[2], ldsh20_c[3],
                                  ldsh20_c[1]);
      cmin1 = min(cmin1, conf);
      ngtests[0]++;
    ...
    if (pxin.visusd) {
      if (rint(m31) != rint(bad_data) && rint(m22) != rint(bad_data)) {
        if(m31 <= ldsbt1_c[0]) {
          ...
          if (m31_m22 >= lds11_4lo_c[1] && m31_m22 <= lds11_4hi_c[1])
            (void) set_bit(19, pxout.testbits);

          locuta[0] = lds11_4lo_c[0];
          locuta[1] = lds11_4hi_c[0];
          hicuta[0] = lds11_4lo_c[2];
          hicuta[1] = lds11_4hi_c[2];
          midpta[0] = lds11_4lo_c[1];
          midpta[1] = lds11_4hi_c[1];
          conf = conf_test_dble(m31_m22, locuta, hicuta, 1.0, midpta);
          cmin2 = min(cmin2, conf);
          ngtests[1]++;
//        printf("11-4: %f %f %f %f %f %f %f %f %f %f %d %d\n", ldsbt1_c[0],
//         m31_m22, locuta[0], locuta[1], midpta[0], midpta[1], hicuta[0],
...
/*  Visible reflectance threshold test. */
    ...
    if (pxin.visusd) {
      ...
      if(irtn != 0) {
        b1_locut = ldsref2_c[0];
        b1_midpt = ldsref2_c[1];
        b1_hicut = ldsref2_c[2];
        b1_power = ldsref2_c[3];
        mvis = m02;
      }
...
//      printf("B1: %d %f %f %f %f %f %f%f %f\n", irtn, mvis, lds_ndvi_c[0],
//       	    b1_locut, b1_midpt, b1_hicut, b1_power, conf, cmin3);
//      printf("Vis: %f %f %f %f %f %f %d %d\n", m02, ldsref2_c[0],
//        ldsref2_c[1], ldsref2_c[2], conf, cmin3, num_tests,
//        ngtests[2]);
...
/*  Near-infrared (1.38 micron) high cloud test. */
    ...
    if (pxin.visusd && pxin.hi_elev == 0 && pxin.tpw > lds_ref3_tpw_c[0]) {
      if (rint(m26) != rint(bad_data)) {

        num_tests++;
        (void) set_bit(16, pxout.qabits);

        if (m26 <= ldsref3_c[1]) (void) set_bit(16, pxout.testbits);
        conf = conf_test(m26, ldsref3_c[0], ldsref3_c[2], 1.0,
                         ldsref3_c[1]);
        cmin4 = min(cmin4, conf);
        ngtests[3]++;
//      printf("1.38: %f %f %f %f %f %f %d %d\n", m26,ldsref3_c[0],
//           ldsref3_c[1],ldsref3_c[2],conf,cmin4,num_tests,ngtests[3]);
