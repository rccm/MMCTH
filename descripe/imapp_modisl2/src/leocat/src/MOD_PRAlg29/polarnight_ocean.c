...
!Description:
  ...
  Integer function polarnight_ocean.c
  Computes initial clear sky confidence for polar nighttime water surfaces.
...
/*  11 micron brightness temperature threshold test. */
    ...
    if(rintf(m31) != rintf(bad_data)) {
      ...
      if(m31 >= pnobt11[1]) (void) set_bit(13, pxout.testbits);
      conf = conf_test(m31, pnobt11[0], pnobt11[2], pnobt11[3],
                                  pnobt11[1]);
...
/*  Water vapor high cloud test. */
    ...
    if(rintf(m27) != rintf(bad_data)) {
      ...
      if(m27 >= pnoh20[1]) (void) set_bit(15, pxout.testbits);
      conf = conf_test(m27, pnoh20[0], pnoh20[2], pnoh20[3],
                                  pnoh20[1]);
      cmin1 = min(cmin1, conf);
      ngtests[0]++;
...
//      if(sfct > 295.0) sfct = 295.0;
        ...
        if(sfcdif < pnosst[1]) (void) set_bit(27, pxout.testbits);
        conf = conf_test(sfcdif, pnosst[0], pnosst[2], pnosst[3], pnosst[1]);
        cmin1 = min(cmin1, conf);
        ngtests[0]++;
        
//      printf("SST %f %f %f %f %f\n", m31c_m32c,sstc,mu,modsst,sfct);
//      printf("SST %f %f %f %f %f %f %f %d %d\n", pnosst[0], pnosst[1],        
//        pnosst[2], pnosst[3], sfcdif, conf, cmin1, ngtests[0], num_tests);
...
/*  11-8.6 micron BTD test. */
    ...
    if(rintf(m31) != rintf(bad_data) && rintf(m32) != rintf(bad_data) &&
      ...
      if(m29_m31 < pno11_86[1]) (void) set_bit(24, pxout.testbits);
      conf = conf_test(m29_m31, pno11_86[0], pno11_86[2], pno11_86[3], pno11_86[1]);
      cmin2 = min(cmin2, conf);
      ngtests[1]++;
//    printf("11-8.6 %f %f %f %f %f %f %d %d\n", m29_m31, pno11_86[0], pno11_86[1],
//         pno11_86[2], conf, cmin2, num_tests, ngtests[1]);
...
/*  11 minus 4 micron BTDIF oceanic stratus (low emissivity water cloud) test. */
    ...
    if (rintf(m31) != rintf(bad_data) && rintf(m22) != rintf(bad_data)) {
      ...
      if (m31_m22 <= pno11_4lo[1]) (void) set_bit(31, pxout.testbits);
      conf = conf_test(m31_m22, pno11_4lo[0], pno11_4lo[2], 1.0,
                       pno11_4lo[1]);
      cmin2 = min(cmin2, conf);
      ngtests[1]++;
//    printf("11-4 lo em: %f %f %f %f %f %f %d %d\n", m31_m22, pno11_4lo[0],
//       pno11_4lo[1], pno11_4lo[2], conf, cmin2, num_tests, ngtests[1]);
...
//  11 minus 4 micron BTDIF cloud test.    
    ...
    if (rint(m31) != rint(bad_data) && rint(m22) != rint(bad_data)) {
      ...
      thr = b1 + pno_intadj[0] + (b2 * pxin.tpw) + (b3 * pxin.tpw * pxin.tpw);
      hicutr[0] = thr - 1.5;
      hicutr[1] = thr + 1.5;
...
//           pno_intadj[0], thr, locutr[0], midptr[0], hicutr[0], locutr[1],
...
/*  Water vapor cloud test (warmer water only). */

    if(pxin.sfctmp >= pnobt1[0]) {
      if (rintf(m28) != rintf(bad_data) && rintf(m29) != rintf(bad_data)) {

        num_tests++;
        (void) set_bit(29, pxout.qabits);
        m29_m28 = m29 - m28;

        if (m29_m28 > pno86_73[1]) (void) set_bit(29, pxout.testbits);
        conf = conf_test(m29_m28, pno86_73[0], pno86_73[2], 1.0,
                         pno86_73[1]);
        cmin2 = min(cmin2, conf);
        ngtests[1]++;
//      printf("8.6-7.3: %f %f %f %f %f %f %f %d %d\n", pnobt1[0], m29_m28, pno86_73[0],
//        pno86_73[1], pno86_73[2], conf, cmin2, num_tests, ngtests[1]);
...
//     no_11var[1], no_11var[2], conf, cmin2, num_tests, ngtests[1]);
