/*$Id: clavrx_beta_cloud_top_temp_debug.c,v 1.1.1.1 2006/10/02 22:40:37 mpav Exp $*/
			         ...
			         rtm_toa rclr, rad_utils rutil)
{
  ...
  float tsfc_est, trans_ac_11, rad_ac_11, trans_ac_12, rad_ac_12,
        rad_bnd_11, rad_clear_11, rad_bnd_12, rad_clear_12;
  ...
  for (i=0; i<imgr1->npts; i++) {
	...
	break;
      case CIRRUS_TYPE:
        x_ap[0] = nwp.map[i_nwp].t_tropo + 20.0;
	tau_ap = 0.7; /*ec=0.50*/
	Sa[0][0] = (25.0*25.0);
	Sa[1][1] = (0.5*0.5);
	imgr2->cldbeta[i] = 1.08;
    ...
    /*----------------------------------------------------------------------------
      Print some information used in debugging if DEBUG is set.
    ----------------------------------------------------------------------------*/
    
    if (DEBUG && (i % 100000) == 0) {
      printf("l=%d, tsfc_est=%f, rad_bnd_11=%f, rad_clear_11=%f, rad_bnd_12=%f, rad_clear_12=%f\n",
        l,tsfc_est,rad_bnd_11,rad_clear_11,rad_bnd_12,rad_clear_12);
    }
	...
	precomputed radiance and transmittance in Z.
      ...
      printf("rad_ac_11=%f, trans_ac_11=%f, rad_clear_11=%f\n",rad_ac_11,trans_ac_11,rad_clear_11);
      printf("rad_ac_12=%f, trans_ac_12=%f, rad_clear_12=%f\n",rad_ac_12,trans_ac_12,rad_clear_12);
      
      imgr2->cldbeta[i] = 1.30;
      x[1] = 0.8457643;
      Tc_temp = 238.6760;
      rad_ac_11 = rutil.planck_rad_fast_index_ptr(31, 157.0730, rutil.T_planck, rutil.B_table, &dB_dTc_11);
      trans_ac_11 = 0.9966368;
      rad_clear_11 = rutil.planck_rad_fast_index_ptr(31, 248.6760, rutil.T_planck, rutil.B_table, &dB_dTc_11);
      rad_ac_12 = rutil.planck_rad_fast_index_ptr(32, 154.9425, rutil.T_planck, rutil.B_table, &dB_dTc_11);
      trans_ac_12 = 0.9947537;
      rad_clear_12 = rutil.planck_rad_fast_index_ptr(32, 248.5549, rutil.T_planck, rutil.B_table, &dB_dTc_11);
      ...
      printf("demiss12_demiss11=%f\n",demiss12_demiss11);
      printf("emiss_12=%f\n",emiss_12);
      printf("Bc_11=%f, dB_dTc_11=%f\n",Bc_11,dB_dTc_11);
      printf("Bc_12=%f, dB_dTc_12=%f\n",Bc_12,dB_dTc_12);
	...
	rad = sfc(toa) + below_cld_atmos(toa) + cld(toa) + above_cld_atmos(toa)
      ...
      printf("f[0]=%f, dB_dT_11=%f\n",f[0],dB_dT_11);
      printf("f[1]=%f, dB_dT_12=%f\n",f[1],dB_dT_12);
	...
	counter-intuitive considering the normal column major fortran convention.
      ...
      K[1][0] = -7.7459872E-02;
      K[1][1] = -2.764819;
      Sa_inv[0][0] = 3.9999999E-04;
      Sa_inv[0][1] = 0.0000000E+00;
      Sa_inv[1][0] = 0.0000000E+00;
      Sa_inv[1][1] = 4.000000;
      Sy_inv[0][0] = 0.3873985;
      Sy_inv[0][1] = 0.0000000E+00;
      Sy_inv[1][0] = 0.0000000E+00;
      Sy_inv[1][1] = 1.298759;
