/*$Id: clavrx_cloud_top_temp.c,v 1.1.1.1 2006/10/02 22:40:37 mpav Exp $*/
...
void clavrx_cloud_top_temp (unsigned char verbose,
			    ...
			    rtm_toa rclr, rad_utils rutil)
{
  ...
  unsigned char DEBUG = 0, two_layer_flg = 1;
  ...
  int l, ilev, imin=0, nprof;
  float *btd1112, bt11_uni, btd1112_uni, mintmp, maxtmp, tau_ap, Tc_temp, xmin, PTOP = 100.0;
  float tsfc_est, trans_ac_11, rad_ac_11, trans_ac_12, rad_ac_12,
        rad_bnd_11, rad_clear_11, rad_bnd_12, rad_clear_12;
  float layer_lapse_rate, layer_lapse_rate_p, *tt, *pp, *zz, *rad_atm31, *rad_atm32, *trans_atm31, *trans_atm32, beta;
  ...
  for (i=0; i<imgr1->npts; i++) {
	...
	break;
      case TICE_TYPE:
        x_ap[0] = max(220.0,imgr1->bt31[i]-10.0);
	tau_ap = 2.3; /*ec=0.90*/
	Sa[0][0] = (10.0*10.0);
	Sa[1][1] = (0.2*0.2);
	break;
	...
	precomputed radiance and transmittance in Z.
      ...
      if (imgr1->cldtype[i] == TICE_TYPE || imgr1->cldtype[i] == CIRRUS_TYPE ||
          imgr1->cldtype[i] == OVERLAP_TYPE) {
        beta = 1.08;
      }
      else {
        beta = 1.3;
      }
      /*if (imgr1->cldphase[i] == ICE_PHASE && btd1112[i] < 4.0) {
        beta = 1.08;
      }
      else {
        beta = 1.3;
      ...
      emiss_11 = x[1];
      demiss12_demiss11 = beta * pow((1.0-emiss_11),(beta-1.0));
      emiss_12 = 1.0 - pow((1.0-emiss_11),beta);
