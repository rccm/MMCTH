...
MODULE SpcCoeff_Define
  ...
  '$Id: SpcCoeff_Define.f90,v 1.1.1.1 2006/10/02 22:40:37 mpav Exp $'
  ...
  INTEGER, PRIVATE, PARAMETER :: SPCCOEFF_RELEASE = 5  ! This determines structure and file formats.
  ...
  INTEGER( Long ), PUBLIC, PARAMETER :: N_SPCCOEFF_ITEMS = 17_Long
...
!--------------------------------------------------------------------------------
  ...
  CONTAINS
    ...
    ! --------------------------------------------------------
    ! Internal subprogram to construct a standard message when
    ! a floating point number comparison fails.
    ! --------------------------------------------------------

    FUNCTION Construct_Message( Label, l, x1, x2 ) RESULT( Message )
      ...
      CHARACTER( LEN(SPCCOEFF_DATA_NAME) ) :: Data_Name
      CHARACTER( 256 ) :: Format_String
      INTEGER :: n
      REAL( fp_kind ) :: dx
      ...
      Format_String = '( "'//Data_Name//' difference, ", '//Real_Format//&
                      ', " vs. ", '//Real_Format//', "(", es9.2, "% diff), channel index # ", i4 )'

      ! -- Create the message
      dx = HUNDRED * ( x1 - x2 ) / x1
      WRITE( Message, FMT = TRIM( Format_String ) ) x1, x2, dx, l
...
!--------------------------------------------------------------------------------
  ...
  SUBROUTINE Count_SpcCoeff_Sensors( SpcCoeff,   &  ! In/Output
    ...
    ELSE ! Use WMO ID
      ...
      ! -- Check that all the values are valid 
      IF ( ANY( SpcCoeff%WMO_Satellite_ID == INVALID_WMO_SATELLITE_ID ) .OR. &
           ANY( SpcCoeff%WMO_Sensor_ID    == INVALID_WMO_SENSOR_ID    )      ) RETURN
...
! Revision 1.1.1.1  2006/10/02 22:40:37  mpav
! LEOCAT
!
