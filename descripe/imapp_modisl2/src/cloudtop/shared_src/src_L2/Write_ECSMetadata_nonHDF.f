      Integer Function Write_ECSMetadata_nonHDF( LUN_Of_QCFile_MCF, 
     1                                           LUN_Of_QCFile_RP,
     2                                           LUN_Of_NUM_INV_RP,
     3                                           LUN_Of_NUM_ARCH_RP,
     4                                           FileNameSuffix )
...
C
C !PURPOSE:
C   Write EOSDIS Core System (ECS) inventory metadata to a non-HDF 
C   product file.  These metadata are written to a separate (*.met) 
C   ascii file as provided by the SDPTK for non-HDF product files.  
C   They are intended to satisfy ECS's requirements for ingest and
C   search capability on non-HDF products. 
C
C
C !DESCRIPTION:
C
C   The individual ECS metadata fields written by function
C   Write_ECSMetadata_nonHDF, the source of the field values, and the
...
C   internal - computed internally within function Write_ECSMetadata_nonHDF
C   static   - predefined value set within function Write_ECSMetadata_nonHDF
C
...
C      4    LocalGranuleId                  internal        PGE
C      5    RangeBeginningTime                PCF           PGE
C      6    RangeEndingTime                   PCF           PGE
C      7    RangeBeginningDate                PCF           PGE
C      8    RangeEndingDate                   PCF           PGE
C      9    AssociatedSensorShortName         MCF           MCF
C     10    AssociatedPlatformShortName       MCF           MCF
C     11    AssociatedInstrumentShortName     MCF           MCF
C
C  Additional ECS metadata under the "Archive" classification are also
C  written to the MOD *.met.  They serve to identify the product and 
C  track revisions to its metadata configuration. 
C
C
C
C !INPUT PARAMETERS:
...
C !REVISION HISTORY:
c 
c Revision 1.8  2005-01-14  gfireman
c Call Set_RP_Data_Atmos only with valid LUN_Of_NUM_ARCH_RP.
c Set LUN_Of_NUM_ARCH_RP <= 0 to avoid writing ArchiveMetadata Runtime Parameters.
c
c Revision 1.7  2004/11/01  rhucek
c Added call to function Set_RP_Data_Atmos to permit storing 
c ArchiveMetadata fields defined as Runtime Parameters in the PCF.
c 
...
C        Get_LUN_Of_LclVrsnID_Atmos (src_L2 shared code library)
...
C        Set_LclGranID_Sfx          (src_L2 shared code library)
C        String_Loc                 (src_L2 shared code library)
C
C
C    Named Constant:
C        MCORE_*               (mapi.inc)
...
C PARAMETER declarations
      character*(*)  FUNCNAME
      PARAMETER     (FUNCNAME = 'Write_ECSMetadata_nonHDF')
      ...
      integer        LUN_COLLECTION_START,       LUN_COLLECTION_STOP
      parameter     (LUN_COLLECTION_START=10258, LUN_COLLECTION_STOP=10259)
...
C Declaration of function arguments
      character*(*) FileNameSuffix
      integer LUN_OF_LCLVRSNID,  LUN_Of_NUM_INV_RP,  LUN_Of_NUM_ARCH_RP, 
     1        LUN_Of_QCFile_MCF, LUN_Of_QCFile_RP


C Local Variable Declarations
      character*25  msg25, msg25_LUN
      ...
      integer    fbyte_LUN,lbyte_LUN,iparm,rtn,rtn_flag,rtn_loc

      integer    pgs_met_init,pgs_met_setattr_s,
     1           PGS_PC_GetConfigData,pgs_met_write,String_Loc,
     2           Set_LclGranID_Sfx, Set_RP_Data_Atmos
...
c-----Check for valid LUN of QC file MCF  
     ...
     1   // '" is out of bounds. '
     2   // char(10) //'PCF LUN numbers must be greater than zero.' 
...
c-----Check for valid LUN of QC file RP
     ...
     1   // msg25(fbyte:lbyte) // '" is out of bounds. '
     2   // char(10) //'PCF LUN numbers must be greater than zero.' 
     3   // char(10) // 'Operator Action:  Notify SDST.'
...
C-------------------------------------------------------------------------------
      ...
      rtn = pgs_met_init(LUN_Of_QCFile_MCF,Met_Handles)
...
c --- Read collection start datetime from PCF
     ...
     1   // char(10) // 'start datetime on LUN '//msg25(fbyte:lbyte) // '.'
     2   // char(10) // 'Operator Action:  Check for valid PCF Runtime Parameter (RP) '
     3   // char(10) // 'entry.  If LUN is nonexistent or RP value is blank, stage '
...
c --- check for blank string
     ...
     1   // msg25(fbyte:lbyte) // ' is blank.' 
     2   // char(10) // 'PGS_PC_GetConfigData() unable to set attributes '
     3   // char(10) // 'RangeBeginningDate and RangeBeginningTime.' 
     4   // char(10) // 'Operator Action:  Check for valid PCF Runtime Parameter '
     5   // char(10) // 'value.  If blank, stage correct PCF and rerun PGE. '
...
c ------ iparm=1, set start "date";  iparm=2, set start "time"
         Do 200 iparm  = 1,2
            ...
            rtn = PGS_MET_SetAttr_s(Met_Handles(INVENTORYMETADATA),
     1            AttrN,AttrV_s)
...
c --- Read collection stop datetime from PCF
      ...
      If (rtn .ne. PGS_S_SUCCESS) Then
         ...
         msgbuf = 'PGS_PC_GetConfigData() unable to read the collection '
     1   //char(10) //'stop datetime on LUN '//msg25(fbyte:lbyte) // '.'
     2   // char(10) // 'Operator Action:  Check for valid PCF Runtime Parameter (RP) '
     3   // char(10) // 'entry.  If LUN is nonexistent or RP value is blank, stage '
     4   // char(10) // 'correct PCF and rerun PGE.  Otherwise, notify SDST.'
...
c --- check for blank string
     ...
     1   // msg25(fbyte:lbyte) // ' is blank.' 
     2   // char(10) // 'PGS_PC_GetConfigData() unable to set attributes '
     3   // char(10) // 'RangeEndingDate and RangeEndingTime.' 
     4   // char(10) // 'Operator Action:  Check for valid PCF Runtime Parameter '
     5   // char(10) // 'value.  If blank, stage correct PCF and rerun PGE. '
     6   // char(10) // 'Otherwise, notify SDST.'
...
C-------------------------------------------------------------------------------
C Set "LocalGranuleID".  This is the product file name in the MODIS convention.  
C-------------------------------------------------------------------------------
     ...
     4   // char(10) // 'messages originating in routine Get_LUN_Of_LclVrsnID_Atmos.'
      ...
      Else
         ...
         rtn = Set_LclGranID_Sfx( Met_Handles,LUN_Of_LclVrsnID,FileNameSuffix )

         If (rtn .eq. FAIL) Then
            error_flag = .true.

            msgbuf = 'Set_LclGranID_Sfx detected error setting ECS LocalGranuleID.'
...
C-------------------------------------------------------------------------------
      If (LUN_Of_NUM_ARCH_RP .gt. 0) Then
         
         rtn = Set_RP_Data_Atmos( MET_Handles,
     1                            LUN_Of_NUM_ARCH_RP,
     2                            ARCHIVEMETADATA )
...
C --- Write inventory Metadata to QC file.
      rtn = pgs_met_write(Met_Handles(ODL_IN_MEMORY),' ',LUN_Of_QCFile_RP)
     ...
     4   // char(10) // 'and rerun PGE. Otherwise, notify SDST.'
      ...
      If (.not.error_flag) Write_ECSMetadata_nonHDF = SUCCEED 
