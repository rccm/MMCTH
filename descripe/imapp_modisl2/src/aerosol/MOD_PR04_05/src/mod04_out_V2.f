...
C
     ...
     &     SDS_Aerosol_Cldmask_Land_Ocean(NUMCELLS*20*ILINE*2)
...
C
     ...
     &    SDS_NUMPIXELS(NUMCELLS,NWAV_S+3), SDS_CLDFRC_ocean(NUMCELLS),
...
C
     ...
     *SDS_Tau_Land_Ocean,SDS_Aerosol_Cldmask_Land_Ocean,SDS_Quality_Land_Ocean)
...
C
     ...
     *SDS_Aerosol_Cldmask_Land_Ocean,SDS_Quality_Land_Ocean)
...
c
     ...
     &            SDSTAU_average_0p55um(NUMCELLS),
     &            SDS_AOT_model(NUMCELLS,num_model_index),
     &           SDS_NUMPIXELS(NUMCELLS,NWAV_S+3),SDS_MTHET0(NUMCELLS),
...
c Set up BUFfer for the arrays to write
     ...
     &         BUF_mass_conc(NUMCELLS*Lines_Per_Write*NUM_solutions)
      ...
      INTEGER*2 BUF_REF(NUMCELLS*Lines_Per_Write*(NWAV_S+3)),
     &         BUF_REF_STD(NUMCELLS*Lines_Per_Write*(NWAV_S+3)),
     ...
     &         BUF_RefF_average(NUMCELLS*Lines_Per_Write*NWAV_S) 
           ...
           INTEGER*2
     &    BUF_small_weighting(NUMCELLS*Lines_Per_Write),
     ...
     &    BUF_angs_coeff1(NUMCELLS*Lines_Per_Write),
     &    BUF_angs_coeff2(NUMCELLS*Lines_Per_Write), 
     &    BUF_AOT_model(NUMCELLS*Lines_Per_Write*num_model_index),
     &    BUF_NUMPIXELS(NUMCELLS*Lines_Per_Write*(NWAV_S+3))
           INTEGER*2
     &    BUF_MTHET0(NUMCELLS*Lines_Per_Write),
     &    BUF_MTHET(NUMCELLS*Lines_Per_Write),
     &    BUF_MPHI0(NUMCELLS*Lines_Per_Write),
     ...
     &    BUF_Aerosol_cldmask(NUMCELLS*20*Lines_Per_Write*20),
     &    BUF_Quality(NUMCELLS*Lines_Per_Write),
     &    BUF_TAU_average_0p55um(NUMCELLS*Lines_Per_Write)
...
C Dimension 2 is BUFfer (scan) line counter.
   ...
   15  continue
       ...
       do 25 i = 1, Ncell
         ...
         BUF_angs_coeff1(LL)     = SDS_angs_coeff1(i)
         BUF_angs_coeff2(LL)     = SDS_angs_coeff2(i)
         BUF_small_weighting(LL) = SDS_small_weighting(i) 
         BUF_TAU_average_0p55um(LL) = SDSTAU_average_0p55um(i)
   25  continue 
...
c
        ...
        BUF_NUMPIXELS(LL)     =    SDS_NUMPIXELS(i,j)
        Enddo
        Enddo
...
C 2/2006
     ...
     &   'MOD04_SEA_OUT')
C1/2002
cs      RTN = PMAR(MODFIL,Fid_Ratio_Small_Land_And_Ocean,' ', 
cs     & START2,DIM2_BUF,BUF_small_wgt)
cs         if (RTN.NE.MAPIOK) CALL MODIS_SMF_SETDYNAMICMSG
cs     &   (MODIS_W_GENERIC,'PMAR for small mode ratio Ocean and land',
cs    &   'MOD04_SEA_OUT')
...
c /12/2012 moved to single dimension for L3 easy processing     
           RTN = PMAR(MODFIL,Fid_angs_coff1,' ',START2,DIM2_BUF,
     &             BUF_angs_coeff1)
         if (RTN.NE.MAPIOK) CALL MODIS_SMF_SETDYNAMICMSG
     &      (MODIS_W_GENERIC,'PMAR for Angstrom_Exponent_1_Ocean',
     &   'MOD04_SEA_OUT')
c
      RTN = PMAR(MODFIL,Fid_angs_coff2,' ',START2,DIM2_BUF,
     &             BUF_angs_coeff2)
         if (RTN.NE.MAPIOK) CALL MODIS_SMF_SETDYNAMICMSG
     &      (MODIS_W_GENERIC,'PMAR for Angstrom_Exponent_2_Ocean',
     &   'MOD04_SEA_OUT') 
        
        RTN = PMAR(MODFIL,Fid_AOT_O_55um,' ',START2,DIM2_BUF,
     &             BUF_TAU_average_0p55um)
         if (RTN.NE.MAPIOK) CALL MODIS_SMF_SETDYNAMICMSG
     &    (MODIS_W_GENERIC,'PMAR for TAUA(0.55um) OCEAn for L3',
     &   'MOD04_SEA_OUT') 
        
           RTN = PMAR(MODFIL,Fid_smallmode,' ',START2,DIM2_BUF,
     &               BUF_small_weighting)
         if (RTN.NE.MAPIOK) CALL MODIS_SMF_SETDYNAMICMSG
...
c
       RTN = PMAR(MODFIL,Fid_numpix,' ',START3,DIM3_BUF,BUF_NUMPIXELS)
         if (RTN.NE.MAPIOK) CALL MODIS_SMF_SETDYNAMICMSG
...
c
     ...
     &      (MODIS_W_GENERIC,'PMAR for REF_STD','MOD04_SEA_OUT')
       ...
       RTN = PMAR(MODFIL,Fid_numpix,' ',START3,DIM3_BUF,BUF_NUMPIXELS(LL))
         if (RTN.NE.MAPIOK) CALL MODIS_SMF_SETDYNAMICMSG
     &   (MODIS_W_GENERIC,'PMAR for NUMBER OF PIXELS USED',
     &    'MOD04_SEA_OUT')
C
...
C
     ...
     &         BUF_REF(NUMCELLS*Lines_Per_Write*(NWAV_S+3)),
     &         BUF_REF_STD(NUMCELLS*Lines_Per_Write*(NWAV_S+3)),
     &         BUF_small_wgt(NUMCELLS*Lines_Per_Write),
     &         BUF_refflux(NUMCELLS*Lines_Per_Write)
      INTEGER *2 
...
c
cs        RTN = PMAR(MODFIL,SDS1_NE6,' ',START2,DIM2_BUF,BUF_CldMskQA)
cs         if (RTN.NE.MAPIOK) CALL MODIS_SMF_SETDYNAMICMSG
cs     &      (MODIS_W_GENERIC,'PMAR for Cloud_Mask_QA','MOD04_Land_OUT')
C
...
c
     ...
     &    'MOD04_Land_OUT')
c
cs        RTN = PMAR(MODFIL,Fid_angs_coff_land,' ',START2,DIM2_BUF,
cs     &          BUF_angs_coeff)
cs         if (RTN.NE.MAPIOK) CALL MODIS_SMF_SETDYNAMICMSG
cs     &   (MODIS_W_GENERIC,'PMAR forAng Coeff Land',
cs     &    'MOD04_Land_OUT')
c
...
CJuly/2008
         ...
         START2(2) = (Nline-1) * Lines_Per_Write*20 
         DIM2_BUF(1)   = Ncell*20
         DIM2_BUF(2)   = Lines_Per_Write*20
...
c
cs       DIM3_BUF_2(3) = Lines_Per_Write

cs        RTN = PMAR(MODFIL,Fid_qa_extra_land,' ',START3_2,DIM3_BUF_2,
cs     &      BUF_QCONTROL_crit)
cs         if (RTN.NE.MAPIOK) CALL MODIS_SMF_SETDYNAMICMSG
cs     &   (MODIS_W_GENERIC,'PMAR for QUALITY CONTROL','MOD04_Land_OUT')
cs
...
cs      DIM3_BUF(3) = Land_Sol4

cs        RTN = PMAR(MODFIL,Fid_AOT_small_land,' ',START3,DIM3_BUF,
cs     &              BUF_small_TAU)
cs         if (RTN.NE.MAPIOK) CALL MODIS_SMF_SETDYNAMICMSG
cs     &  (MODIS_W_GENERIC,'PMAR fo Tau small Land','MOD04_Land_OUT')

 
c Dimension (cells along swath,cells across swath,number of wavelength_land)
c

      DIM3_BUF(3) = Band_land+3
c
...
c
70        continue
cs         do 170 j = 1,Land_Sol4
cs            LL = Ncell*Lines_Per_Write*(j-1) + 1
cs            START3(3) = j - 1
cs          RTN = PMAR(MODFIL,Fid_AOT_small_land,' ',START3,DIM3_BUF,
cs     &          BUF_small_TAU(LL))
cs         if (RTN.NE.MAPIOK) CALL MODIS_SMF_SETDYNAMICMSG
