     ...
     &      Indx_wspped1,Indx_wspped2,WSPEED,wind,SDSTAU_average_0p55um)
C----------------------------------------------------------------------
C !F77
C
C !DESCRIPTION:
...
C     HDF array names..........
     ...
     &     SDS_mass_conc(NUMCELLS,NUM_solutions)
      INTEGER*2 SDS_ref(NUMCELLS,NWAV+3)
      INTEGER*2 SDS_ref_STD(NUMCELLS,NWAV+3)
...
C
     ...
     *     sd_W213,sd_w412,sd_w443,sd_w0p75,sd_W869o,IY
c
     ...
     *     W0P75_SYN(ISWATH,ILINE),W869o_SYN(ISWATH,ILINE) 
       ...
       real W138_SYN(ISWATH,ILINE),WSPEED 
       real Refl_4(2*ISWATH,2*ILINE) 
       INTEGER   High_Cloud_Flag_500(2*ISWATH,2*ILINE),index_wspeed
       Real Refl_3(2*ISWATH,2*ILINE) ,Refl_1(4*ISWATH,4*ILINE)

       INTEGER maskoption,num_resol,savecldmask(2*ISWATH,2*ILINE)
       INTEGER Save_index(NUMCELLS,MaxPixels_left),iist
...
c
     ...
     *  W124_SYN,W164_SYN,W213_SYN,W412_SYN,W443_SYN,W0P75_SYN,
     *  W869o_SYN,CLDMSK_syn,Sunglint_Flag,NCLDMSK_syn,
     *  START_1KM,END_1KM,NUMDATA_550,sd_W659,sd_W865,sd_W470,sd_W550,
     *  sd_W124,sd_W164,sd_W213,sd_w412,sd_w443,sd_w0p75,sd_W869o,
...
c          write(41,*) iscan,idata,(TAU_COMPUTED(IWAV),IWAV=1,Nwav)
C
...
C
!        write(36,*) 'storeref_data 1',iscan,idata
...
c
     ...
     *SDSTAU_best,SDSTAU_average,SDSTAUB_best,
     ...
     &SDS_Tau_Land_Ocean,SDS_correc_small_weighting,SDSTAU_average_0p55um)
...
C----------------------------------------------------------------------
     ...
     * NUMDATA_443,NUMDATA_0P75,NUMDATA_869o
     ...
     *     W0P75_SYN(ISWATH,ILINE),W869o_SYN(ISWATH,ILINE)
      real array(2*IGRIDX*2*IGRIDY),newarray(2*IGRIDX*2*IGRIDY)
       REAL  ref_interm(NWAV+3,2*IGRIDX*2*IGRIDY)
       REAL  array_interm(NWAV+3,2*IGRIDX*2*IGRIDY)
       REAL sd_W659,sd_W865, sd_W470,sd_W550,sd_W124,sd_W164,
     *     sd_W213,sd_w412,sd_w443,sd_w0p75,sd_W869o
      REAL  sd_allwav(NWAV+3),var_allwav(NWAV+3)
     ...
     * vFlag_w443,vFlag_w0P75, vFlag_w869o
      ...
      integer Save_index(NUMCELLS,MaxPixels_left)
...
c
     ...
     *  W0P75_SYN,W869o_SYN,ref_interm,NCLDMSK_syn,
     * NUMDATA_659,NUMDATA_865,NUMDATA_470,NUMDATA_550,
     * NUMDATA_124,NUMDATA_164,NUMDATA_213,NUMDATA_412,
     * NUMDATA_443,NUMDATA_0P75,NUMDATA_869o,START_1KM,
     * END_1KM,VFlag_w659,VFlag_w865,VFlag_w470,VFlag_w550,VFlag_w124,
     *VFlag_w164,VFlag_w213,vFlag_w412,vFlag_w443,vFlag_w0P75,
     *VFlag_w869o, cloudy,iscan, High_Cloud_Flag_500,W138_SYN,Ref_ray,Wave,
     *  CLD_FRAC,savecldmask,idata)
...
C
         elseif(iy .eq. 8)then
        REFW412=ref_allwav(iy)
         sd_w412=sd_allwav(iy)
       
C    
C   WAVE 0.443  UM
C
         elseif(iy .eq. 9)then
        REFW443=ref_allwav(iy)
        sd_w443=sd_allwav(iy)
      
     C
C   WAVE 0.75  UM
C
         elseif(iy .eq. 10)then
        REFW0p75=ref_allwav(iy)
        sd_w0p75=sd_allwav(iy)
...
c    GLINT...........
     ...
     &          ( REFW470/ REFW659  .le. 0.95)))then 
          ...
          if( Quality_dust_flag_glint .eq.1)
     &     print*,iscan,idata,(REFW470/REFW659)
...
!         write(36,*)'inside average',(ref_allwav(iy),iy=1,10)
!        write(36,*)'inside average',REFW470,REFW550,REFW659,REFW865,
!     *    REFW124,REFW164,REFW213,REFW412,REFW443,REFW0p75
           RETURN
           END
...
C----------------------------------------------------------------------
          ...
          REAL sd_W659,sd_W865,sd_W470,sd_W550,sd_W124,sd_W164,
     *     sd_W213,CLD_FRAC,sd_w412,sd_w443,sd_w0p75
           REAL SCAT_ANGLE_OCEAN
          INTEGER*2 SDS_ref(NUMCELLS,NWAV+3)
          INTEGER*2 SDS_ref_STD(NUMCELLS,NWAV+3)
          INTEGER * 2 SDS_NUMPIXELS(NUMCELLS,NWAV+3),
...
c                     HDF output file
c NPP WAVELENGTHS   refw412,refw443,refw0p75
         ...
         SDS_ref(IDATA,8) =  nint(REFW412*SCALE4+OFFSET4)
         SDS_ref(IDATA,9) =  nint(REFW443*SCALE4+OFFSET4)
         SDS_ref(IDATA,10) =  nint(REFW0p75*SCALE4+OFFSET4)
         ...
         SDS_ref_STD(IDATA,8) = nint(sd_w412*SCALE4+OFFSET4)
         SDS_ref_STD(IDATA,9) = nint(sd_w443*SCALE4+OFFSET4)
         SDS_ref_STD(IDATA,10) = nint(sd_w0p75*SCALE4+OFFSET4) 
         Do Num_Wav=1,NWAV+3 
...
C********************************************************************
     ...
     *EXTNORBIG,ISMALL,IBIG,Indx_wspped1,
...
c
      ...
      SDSTAU_average_0p55um(IDATA)= nint(AVE_ARRAY(4,2)*SCALE3+OFFSET3)  
...
C      12/2012.... CHanged to single solution for easy processing of L3      
      
       SDS_angs_coeff1(IDATA)=nint(AVE_ARRAY(40,2)*
     *                               SCALE3+OFFSET3)
       SDS_angs_coeff2(IDATA)=nint(AVE_ARRAY(41,2)*
     *                               SCALE3+OFFSET3)
       SDS_small_weighting(IDATA) = nint(AVE_ARRAY(57,2)*
     *                         SCALE3+OFFSET3)
C
...
C----------------------------------------------------------------------
     ...
     *     W0P75_SYN(ISWATH,ILINE),W869o_SYN(ISWATH,ILINE)
      ...
      REAL ref_interm(NWAV+3,2*IGRIDX*2*IGRIDY),CLD_FRAC
      INTEGER NCLDMSK_SYN(2*ISWATH,2*ILINE+1)
     ...
     * NUMDATA_443,NUMDATA_0P75, NUMDATA_869o
      ...
      INTEGER  High_Cloud_Flag_500(2*ISWATH,2*ILINE),idata
...
C
        ...
        vFlag_W412=vFlag_W412+1
          NUMDATA_412=NUMDATA_412+1
          ref_interm(8,NUMDATA_412)=(W412_SYN(IXX,IYY)*
     *       NCLDMSK_SYN(IX,IY) *sed_mask (IX,IY))
...
C
        ...
        vFlag_W443=vFlag_W443+1
          NUMDATA_443=NUMDATA_443+1
          ref_interm(9,NUMDATA_443)=(W443_SYN(IXX,IYY)*
     *       NCLDMSK_SYN(IX,IY) *sed_mask (IX,IY))
     
C     WAVE 0.75 UM
C
     
        vFlag_w0P75=vFlag_w0P75+1
         NUMDATA_0P75=NUMDATA_0P75+1
          ref_interm(10,NUMDATA_0P75)=( W0P75_SYN  (IXX,IYY)*
     *       NCLDMSK_SYN(IX,IY) *sed_mask (IX,IY))
       
       
        
         
Cc Endif for cldmask & Sedmask         
          ENDIF  
