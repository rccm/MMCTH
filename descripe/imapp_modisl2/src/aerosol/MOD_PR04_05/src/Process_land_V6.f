     ...
     &  SDSTAU_corrected_213,Quality_flag_forJoint,SDSTAU_small_land,Save_index,
     &  W412_SYN,W443_SYN,W0P75_SYN,snow_present_Flag,coastal_Pix)
...
C
     ...
     &        MODFIL_ANC(MODFILLEN)
      ...
      INTEGER  Good_pixels_Land(Band_land+3) 
...
C NPP channels      
      REAL W412_SYN(ISWATH,ILINE)
      REAL W443_SYN(ISWATH,ILINE)
      REAL W0P75_SYN(ISWATH,ILINE)
...
C
C Variables used for ECS core meta deta
C
...
c 9 lines added by SDST vlin    08/29/96
...
c retrieved products
      ...
      INTEGER AVE_COUNT
      INTEGER FTABLE_NL
      INTEGER ETA_FLAG,snow_present_Flag
...
C
     ...
     * SDW412_L,SDW443_L,SDW0P75_L,iscan,Good_pixels_Land)
...
C
     ...
     * REFW412_L,REFW443_L,REFW0P75_L,SDW412_L,SDW443_L,SDW0P75_L,
     * Good_pixels_Land,snow_present_Flag,coastal_Pix)
...
c                   to BYTE.
      ...
      INTEGER  Good_pixels_Land(Band_land+3),num_wav,coastal_Pix
...
c compute fraction instead of % for outpt
      ...
      IF (cloud_num_land .ge.0) THEN
        New_cloud_num=real(cloud_num_land/400.)
      ENDIF
...
C set it to 12 to indicate No retrivals        
        ...
        Qcontrol_special_land=0
c        Quality_flag_forJoint=QA_Flag_Land(8)
...
C Quality will be degraded based on the number of coastal pixels in % in 10*10 box.
        
         if(coastal_Pix .gt. 50)then
         QA_Flag_Land(8)=0  
         Quality_Flag_for_retr =12
         endif
         
C Changed the Order of Quality Cirrus Ret_Quality_cirrus =0 if thin cirrus detection 
...
C,,, ,
        ...
        SDS_ref_land(IDATA,8) = NINT(REFW412_L * SCALE4 + OFFSET4)
        SDS_ref_land(IDATA,9) = NINT(REFW443_L * SCALE4 + OFFSET4)
        SDS_ref_land(IDATA,10) =NINT(REFW0P75_L * SCALE4 + OFFSET4)
c
        ...
        SDS_ref_STD_land(IDATA,8) =NINT(SDW412_L * SCALE4 + OFFSET4)
        SDS_ref_STD_land(IDATA,9) =NINT(SDW443_L * SCALE4 + OFFSET4)
        SDS_ref_STD_land(IDATA,10)=NINT(SDW0P75_L * SCALE4 + OFFSET4)
...
C Byte 3( ist bit = snow  2nd and 3rd bit information about ancillary data.      
c Snow Flag  based on Aerosol snow detection( Rong-rong) and MOD35 (snow/ice flag)
C Quality Flag for land( Byte 3) is filled to give indication of at least 10% of snow pixels
C in  10 * 10 KM box. Aerosol snow mask is quite strict and seems to work well but may fail
C sometimes. It is just diagnostics.  Shana Mattoo 4/2013
       if( snow_present_Flag .lt. 10)snow_present_Flag=0      
       if( snow_present_Flag .ge. 10)snow_present_Flag=1  
        QA_Flag_Land(3) = snow_present_Flag 
          QA_Temp=0  
       CALL BYTE_SET(QA_Flag_Land(3),0,QA_Temp) 
       CALL BYTE_SET(QA_Flag_Land(1),1,QA_Temp) 
       CALL BYTE_SET(QA_Flag_Land(2),5,QA_Temp)  
       SDS_QCONTROL_land(3,IDATA)=QA_Temp
       
C   4 Byte is emptyEmpty byte  
      SDS_QCONTROL_land(4,IDATA)=0
C   5 and 6th Byte  will be   USED BY DEEP BLURE ALGORITHM       
...
c MVI dependence
            ...
            ELSE !(MVI around 0.5)
               yint_644 = 0.0
               slope_644 = 0.58 - (0.10/0.50)*(MVI-0.25)
               yint_466 = 0.005
               slope_466 = 0.49
            ENDIF
...
C NPP channels      
      ...
      INTEGER NUMRED(2*ISWATH,2*ILINE),NUMBLUE(2*ISWATH,2*ILINE)
      INTEGER CLDMSK_250(4*ISWATH,4*ILINE),I,Idata,iscan 
      REAL THR213MIN,THR213MAX
      INTEGER Num_Pixels_Used,MODIS_waves,NPP_waves
       parameter( MODIS_waves=7, NPP_waves=3)
      REAL   Ref_Inter_Land(MODIS_waves+NPP_waves,Max_Pixels_L)
      REAL   Array(Max_Pixels_L),Array_new(Max_Pixels_L) 
      INTEGER  Good_pixels_Land(MODIS_waves+NPP_waves) 
      REAL     array_interm(MODIS_waves+NPP_waves,Max_Pixels_L)
       REAL     ref_allwav(MODIS_waves+NPP_waves)
      REAL REFW466_L,REFW550_L,sd_allwav(MODIS_waves+NPP_waves)
      REAL REFW644_L,REFW124_L,REFW164_L,REFW866_L,REFW212_L
      REAL SDW466_L,SDW550_L
      REAL SDW644_L,SDW124_L,SDW164_L,SDW866_L,SDW212_L
      REAL ref_val,sd_val
      ...
      Integer Save_index(NUMCELLS_L,MaxPixels_left_L)
       integer Save_Array_index(MaxPixels_left_L)  
C
C Initialization
C
...
C NPP channels          
     ...
     *  START_1KM,END_1KM,MODIS_waves,NPP_waves,idata,iscan)
...
C
         elseif(iy .eq. 8)then
         REFW412_L=ref_allwav(iy)
         SDW412_L=sd_allwav(iy)  
                  
         
C   WAVE 0.443 um  
C
         elseif(iy .eq. 9)then
        REFW443_L=ref_allwav(iy)
        SDW443_L=sd_allwav(iy) 
        

C  Wave 0.75
         elseif(iy .eq. 10)then
         REFW0P75_L=ref_allwav(iy)
         SDW0P75_L=sd_allwav(iy)  
         endif
...
! NPP channels      
      ...
      REAL W443_SYN(ISWATH,ILINE)
      REAL W0P75_SYN(ISWATH,ILINE)
      INTEGER MODIS_waves,NPP_waves,idata,iscan
      INTEGER CLDMSK_250(4*ISWATH,4*ILINE),IXX,IYY 
...
C
       ...
       NUM_indx_Q=0
         ...
         ENDDO
         ENDDO
         do iwav=1,(MODIS_waves+NPP_waves)
         do j=1,Max_Pixels_L
         Ref_Inter_Land(Iwav,j)=0
         ENDDO
         ENDDO
CLoop around 1km to add NPP wavelengths         
...
C
       ...
       DO 200 I = JBLUE,2*IXX 
            IMASK=2*I-1
         DO 201 J = IBLUE,2*IYY
           W865_SYN_ForAve=0 
           W659_SYN_ForAve=0 
           ICLDRED=0
           NUMBLUE(I,J)=0
...
C
           ...
           IF(NUMCFR .GE. 4 .AND. NOWATER .EQ. 4) THEN
             IF(W470_SYN(I,J ) .GT. 0.0) THEN
              ...
              if( W412_SYN(IXX,IYY ).GT. 0.0) THEN
              NUM_indx_z=NUM_indx_z+1
              Ref_Inter_Land(8,NUM_indx_z)=W412_SYN(IXX,IYY ) 
              Endif
              
              if(W443_SYN(IXX,IYY ).GT. 0.0) THEN
               NUM_indx_P=NUM_indx_P+1
              Ref_Inter_Land(9, NUM_indx_P)=W443_SYN(IXX,IYY )  
              Endif
              
              if(W0P75_SYN(IXX,IYY )  .GT. 0.0) THEN
               NUM_indx_Q=NUM_indx_Q+1
              Ref_Inter_Land(10,NUM_indx_Q)=W0P75_SYN(IXX,IYY )   
              endif
...
C    endif for   NUMCFR .GE. 4 .AND. NOWATER .EQ. 4) THEN        
       ENDIF
