...
C
C  WRITTEN BY:
C  SHANA MATTOO                E-MAIL:shana.mattoo@nasa.gov
...
C
     ...
     &           SLOPE_MEAN_LAND(3),SLOPE_MEAN_OCEAN(3)
      ...
      BYTE       SDS_CldMskQA(Iswath*2),QA_Temp
...
C
     ...
     &             SDS_NUMPIXELS_land(NUMCELLS,Band_land+3),
     &             SDSTAU_corrected(NUMCELLS,Land_Sol3),
     &             SDS_ref_land(NUMCELLS,(Band_land+3)),
     &             SDS_ref_STD_land(NUMCELLS,(Band_land+3)),
...
C
     ...
     &             SDS_mass_conc(NUMCELLS,NUM_solutions)
      ...
      INTEGER *2   SDS_ref(NUMCELLS,(NWAV_S+3)),
     &             SDS_ref_STD(NUMCELLS,(NWAV_S+3)),
     &             SDSTAU_best(NUMCELLS,NWAV_S),
     &             SDSTAU_average(NUMCELLS,NWAV_S),
     &             SDSTAU_average_0p55um(NUMCELLS),
     ...
     &             SDS_angs_coeff1(NUMCELLS),
     &             SDS_angs_coeff2(NUMCELLS),
     &             SDS_AOT_model(NUMCELLS,num_model_index),
     &            SDS_NUMPIXELS(NUMCELLS,NWAV_S+3),SDS_CLDFRC(NUMCELLS),
...
C
     ...
     2        Aerosol_cloud_land(ISWATH*2,ILINE*2),
     3         Qcontrol_special_land,Quality_to_pass
C
C
...
C      INTEGER CLDMSK_500(2*ISWATH,2*ILINE),kk,jj
C New
...
C
     ...
     *     Refl_7 (2*ISWATH,2*ILINE),Refl_8 (ISWATH,ILINE),
     ...
     *     Rad_32(ISWATH,ILINE),Refl_15(ISWATH,ILINE) 
     ...
     *     Unc_32(ISWATH,ILINE),Unc_15(ISWATH,ILINE) 
     ...
     &     Vflag_32(ISWATH,ILINE),Vflag_15(ISWATH,ILINE)
     ...
     *     W213_SYN(2*ISWATH,2*ILINE),W12_SYN(ISWATH,ILINE),
     *     W412_SYN(ISWATH,ILINE),W443_SYN(ISWATH,ILINE),
     *     W0P75_SYN(ISWATH,ILINE),W869o_SYN(ISWATH,ILINE),
...
C
      ...
      Real    Glint_angle
      integer index_wave,quality_land,ix,iy,IL,IS,coastal_Pix
c    New cldmask
      REAL RMED(ISWATH*2 ),RMEDSQ(ISWATH*2 ),Multi_factor(NWAV_S+3)
      REAL RMED_1km(ISWATH),RMEDSQ_1km(ISWATH)
      integer CldMsk_500_Ocean(2*ISWATH,2*ILINE+1)
      integer Joint_Cld_mask_500(iswath*2,Iline*2),ii,num,jj,kk
      integer savecldmask(2*ISWATH,2*ILINE)
      integer num_joint,Full_Cld_mask_500(2*ISWATH,2*ILINE*Tot_scan)
      integer num_Sea_land,Full_Sea_Land_mask(ISWATH,ILINE*Tot_scan)
      integer xsize,ysize,Aerosol_Cldmask_land(iswath*2,Iline*2)
      integer xsize1,ysize1
       real  New_distance(Iswath*2,Iline*2*Tot_scan)
      integer *2 
     *     SDS_Aerosol_Clddis_Land_Ocean(2*ISWATH,2*ILINE*Tot_scan)
      integer *2 
     * SDS_Aerosol_Cldmask_Land_Ocean(2*ISWATH,2*ILINE*Tot_scan)
      integer *2 SDS_Ave_Clddis_Land_Ocean(ISWATH,Tot_scan)
      integer Save_index_ocean(NUMCELLS,Max_Pixels_O),index_wspeed
      integer Save_index_land(NUMCELLS,Max_Pixels_O)
      integer Save_index_Land_Ocean(NUMCELLS,Tot_scan,Max_Pixels_O)
      integer save_numpix_Land_Ocean(NUMCELLS,Tot_scan),Iyy
       Real  new_Average_dis(NUMCELLS,Tot_scan),Wind(Lut_indx)  
      integer set_counter_for_Gread,snow_present_Flag,Pure_Land
C
...
C rhucek 11/27/06: commented out create_mod04()
c      rtn = create_mod04() 
...
C rhucek 11/27/06: status message not needed  
C     IF(rtn.ne.SUCCEED) CALL MODIS_SMF_SETDYNAMICMSG(MODIS_E_GENERIC,
C     &    'create_mod04 failed','MOD_PR04_V2.f')
...
C
     ...
     &      Rad_20,Rad_31,Rad_32,Refl_15,
     &      Unc_1,Unc_2,Unc_3,Unc_4,Unc_5,Unc_6,UNc_7,UNc_8,Unc_9,
...
C Vanderlie  Cloud mask For Land >>>>>>>>>>>>   
     ...
     &  Aerosol_Cldmask_land)
     
     
C ...    Vanderlie  Cloud mask  OCEAN >>>>>>>>>>>>       
               Call cldMsk_Ocean(Refl_3 ,Refl_4,Refl_1,
     &         Refl_26,Refl_5,CldMsk_500_Ocean,High_Cloud_Flag,
     &         data_size,savecldmask) 
         
c  Call to make joint cloud mask 500 meter resolution for land and ocean together based on LandSea_Flag( MOD03)  
          Call Make_joint_aerosolCldmask (LandSea_Flag,iscan,
     &    CldMsk_500_Ocean,Aerosol_Cldmask_land,Joint_Cld_mask_500)
      ...
      sds_ave_clddis_land_ocean(Idata,iscan)=-9999
...
C tailored to PGE04 
C  Reads ugrd,vgrd,pwat,ozone from NCEP/GDAS
C
...
c Call subroutine to compute Gas correction factors
...
C      Computes Windspeed 
C
            
          
      CALL indx_wspeed(ugrd,vgrd,Indx_wspeed1,Indx_wspeed2,WSPEED,Wind)
        
C Applies Gas correction factors and computes Snow mask ( Rong_rong)            
     ...
     *       Refl_5,Refl_6,Refl_7,Refl_8,Refl_9,Refl_12,Refl_13,Refl_16,
     *       Refl_26,Rad_20,Rad_31,Rad_32,LandSea_Flag,CldMsk_1km,
     *       SnowMsk_500m,SnowMsk_Ratio,Refl_15,
     *       pwat,ozone,SLOPE_MEAN_LAND,SLOPE_MEAN_OCEAN,W659_SYN,
     *       W865_SYN,W470_SYN,W550_SYN,W124_SYN,W164_SYN,W213_SYN,
     *       W412_SYN,W443_SYN,W0P75_SYN,W869o_SYN,W395_SYN,
     *       W138_SYN,W1100_SYN,W1100_SYN_500m,W1200_SYN,G_factor,
     *       iscan,idata,Multi_factor)
               
               
C
C Call to determine counts of land, water, Clouds for Land only
C 
 
       
       CALL LAND_WATER_CLOUD(DET_Flag,CldMsk_1km,CldMsk_500,CldMsk_250,
     &   High_Cloud_Flag,High_Cloud_Flag_500,LandSea_Flag,
     &   SunGlint_Flag,SnowIce_Flag,SnowMsk_Ratio,
     &   Shadow_Flag,Iscan,idata,cloud_num,
     &   cloud_num_land,Water,Land,Glint,Snowice,START_1KM,END_1KM,
     &   QA_Flag_Land,QA_Flag_Ocean,QA_Temp, 
     &   Quality_cirrus, Ret_Quality_cirrus,snow_present_Flag,coastal_Pix,
     &   Pure_Land)
...
C        Geolocation and angles for 10*10 bin size.
     ...
     &         SDS_Scattering_Angle,SDS_CldMskQA,iscan)
     
         
C
C Compute Glint angle
...
C algorithm, else  processing land algorithm;  
...
C 
     ...
     &        W164_SYN,W213_SYN,W412_SYN,W443_SYN,W0P75_SYN,
     ...
     &  savecldmask,Save_index_ocean,Quality_to_pass,Indx_wspeed1,Indx_wspeed2,
     &  WSPEED,Wind,SDSTAU_average_0p55um)
     
!       Fill Array for TAU for 0.55um for L3 easy processing.              
               
         
               if((SDSTAU_average(IDATA,2)/(SCALE3+OFFSET3)).GT.-0.01) then
                do iyy=1,SDS_NUMPIXELS(idata,2) 
          Save_index_Land_Ocean(IDATA,Iscan,IYY)= Save_index_ocean(IDATA,IYY)
          save_numpix_Land_Ocean(IDATA,iscan)=SDS_NUMPIXELS(idata,2)
           
                Enddo
                Endif
                 
c If retreival populate Quality flg  
...
! Populate TAu_image with all Quality   

          CALL POPULATE_TAU_LAND_OCEAN(IDATA,SDS_Tau_Land_Ocean_img,
     *    SDSTAU_average,NUMCELLS,NWAV_S)
     
     
! Populate TAu with Quality flag for Ocean  1,2 or 3 
...
! if there are more than 20% of water pixels degrade the Quality of land            
               if( water .gt.20) quality_land=0  
! if  for land_sea flag pure_land=(land+intermittent water) coastal = 50% of coastal and if less then 50% it is set back to land              
               if(Pure_Land .gt.0)SDS_Sea_Land_Flag(Idata,iscan)=1
               if(coastal_Pix .le.50)SDS_Sea_Land_Flag(Idata,iscan)=1
               if(coastal_Pix .gt.50)SDS_Sea_Land_Flag(Idata,iscan)=2 
     ...
     &  Save_index_land,W412_SYN,W443_SYN,W0P75_SYN,snow_present_Flag,coastal_Pix)
                  
     
C1/2002   Fill small weighting for land 
 
             if((SDSTAU_corrected(Idata,2)/SCALE3+OFFSET3) .ge.-0.05) then
                do iyy=1,SDS_NUMPIXELS_land(idata,2)
              Save_index_Land_Ocean(IDATA,Iscan,IYY)= Save_index_land(IDATA,IYY)
              save_numpix_Land_Ocean(IDATA,Iscan)=SDS_NUMPIXELS_land(idata,2)
...
! Populate Tau_image for land with All quality Flags
            CALL POPULATE_TAU_LAND_OCEAN(IDATA,SDS_Tau_Land_Ocean_img,
     *     SDSTAU_corrected,NUMCELLS,Land_Sol3) 
     
! Populate Tau_ for land with  quality Flags =3
...
C
     ...
     *        SDS_correc_small_weighting,SDSTAU_average_0p55um)
          
C endif for Land pixels
          ENDIF
C endif for 100 pixel water at ocean
          ENDIF

C
c endif for (no water no land ... problem pixels maybe)
          ELSE
           NO_Ret_Land=NO_Ret_Land+1
             SDS_CLDFRC_land(IDATA)=-99
            CALL FILLVALUE_LAND(IDATA,SDS_Tau_Land_Ocean_img,
...
C this else is for alll nighttime or other bad Scan data
           Else 
          NUMSQ=EV_Frames(iscan)/10
     ...
     *SDS_ratio_small_Land_Ocean,NUMSQ,SDS_Sea_Land_Flag,iscan,SDSTAU_average_0p55um)
          
     
C  Save cloud mask for whole granule to get cloud Distance 
            
           Do ii=1,Iline*2 
           Do jj=1,NUMSQ*20  
           Joint_Cld_mask_500(jj,ii)=-9999
        enddo
        enddo  
         
C endif for Day scan line
C
         ENDIF
...
C Save cloud mask for whole granule to get cloud Distance 
           ...
           Do jj=1,NUMSQ*20  
           Full_Cld_mask_500(jj,num_joint)=Joint_Cld_mask_500(jj,ii)
        enddo
        enddo 
...
C Call to MOD04_OUT writes the SDS HDF arrays and writes the output file
       ...
       CALL MOD04_OUT(MODFIL_MOD04,NUMSQ,NUMSCAN,SDSLAT,SDSLON,
     ...
     *SDS_Quality_Land_Ocean,SDSTAU_average_0p55um)
C
...
9999  CONTINUE
      ...
      Xsize=NUMSQ*20
      Ysize=num_joint
c write sea-land_flag to sds      
c               Do ii=1,num_Sea_land
c               Do jj=1,Iswath
c        SDS_Sea_Land_Flag(jj,ii)= Full_Sea_Land_mask(jj,ii)
c                Enddo
c          Enddo 
      
C Call to subroutine to compute cloud distance........      
      Call Cloud_distance (Full_Cld_mask_500,Xsize,Ysize,New_distance) 
...
C Write Cloud mask and cloud distance to HDF file........      
          Do ii=1,Ysize
               Do jj=1,Xsize 
        SDS_Aerosol_Clddis_Land_Ocean(jj,ii)= Nint(New_distance(jj,ii))
        SDS_Aerosol_Cldmask_Land_Ocean(jj,ii)=Full_Cld_mask_500(jj,ii)
                Enddo
          Enddo
            call Average_cloud_distance(Save_index_Land_Ocean,
     +   save_numpix_Land_Ocean,New_distance,Xsize,Ysize,new_Average_dis) 
          Do ii=1,Ysize/20
               Do jj=1,Xsize/20 
         sds_ave_clddis_land_ocean(jj,ii)=Nint(new_Average_dis(jj,ii)) 
        ...
        Xsize=NUMSQ*20
        Ysize=num_joint
        CALL MOD04_OUT_Extra_500(MODFIL_MOD04,Xsize,Ysize,
     *  SDS_Aerosol_Clddis_Land_Ocean,Fid_CLDdis_Land_Ocean)
      
        CALL MOD04_OUT_Extra_500(MODFIL_MOD04,Xsize,Ysize,
     *   SDS_Aerosol_Cldmask_Land_Ocean,Fid_CLDMSK_Land_Ocean)
     
     
           Xsize1 =Xsize/20 
           Ysize1= Ysize/20
           
         CALL MOD04_OUT_Extra(MODFIL_MOD04,Xsize1,Ysize1,
     *   sds_ave_clddis_land_ocean,Fid_Avedis_Land_Ocean) 
          CALL MOD04_OUT_Extra(MODFIL_MOD04,Xsize1,Ysize1,
     *    SDS_Sea_Land_Flag,Fid_Land_sea_Flag) 
         CALL MOD04_OUT_Extra(MODFIL_MOD04,Xsize1,Ysize1,
     *    SDS_Sea_Sunglint_Flag,Fid_Glint_Angle)
         CALL MOD04_OUT_Extra(MODFIL_MOD04,Xsize1,Ysize1,
     *   SDS_NCEP_Wspeed,Fid_Wind_Speed) 
         CALL MOD04_OUT_Extra(MODFIL_MOD04,Xsize1,Ysize1,
     *   SDS_topo_Altitude,Fid_Altitude_Land)
 
      DO I=1,No_PSA
        QA_Metadata_MOD04(I)=0.0
      ENDDO

      Total_Grids=NUMSCAN*NUMSQ 
...
! Commented out MOD05 read 18 June R. Cintineo 
      file_version = 1
      HDF_FILENAME = ' '
      rtn = -1
...
C*********************************************************************
     ...
     *         Shadow_Flag,Iscan,idata,
     *         cloud_num,cloud_num_land,Water,Land,Glint,Snowice,
     *         START_1KM,END_1KM,QA_Flag_Land,QA_Flag_Ocean,QA_Temp,
     *        Quality_cirrus, Ret_Quality_cirrus,snow_present_Flag,
     *        coastal_Pix,Pure_Land)

C---------------------------------------------------------------------
C!F77
C
C!DESCRIPTION: This subroutine counts water, land, cloudy,  and
...
C----------------------------------------------------------------------
     ...
     &        High_Cloud_Flag(ISWATH,ILINE),Pure_Land,
     &        High_Cloud_Flag_500(2*ISWATH,2*ILINE),idata,iscan 
      INTEGER QA_Flag_Land(19),QA_Flag_Ocean(12),cloud_num2
      integer   Ret_Quality_cirrus,Quality_cirrus(ISWATH,ILINE)
      integer  Newwater,NewLand,Nothing,snow_present_Flag,coastal_Pix
      BYTE QA_Temp
      ...
      snow_present_Flag = 0
       coastal_Pix =0
       Pure_Land =0
C cloud_num and cloud_num _land after cloud mask at 1km  before ice,snow flag for Land only
         DO IYY = 1,IGRIDY
       ...
       ENDDO
          ENDDO
C calculate number of cloudy pixels to compute cloud fraction.          
...
C Calculating number of land, water 
C pixels
C
          DO IYY = 1,IGRIDY
          DO IXX=START_1KM,END_1KM
 
C   Changed flag 11/26/2012    Shana Mattoo
...
C----------------------------------------------------------------------
     ...
     &          SDS_Scattering_Angle(NUMCELLS)
      ...
      SDS_MTHET0(IDATA) = int(MTHET0*SCALE2+OFFSET1)
      SDS_MTHET(IDATA) = int( MTHET*SCALE2+OFFSET1)
      SDS_MPHI0(IDATA) = int(MPHI0*SCALE2+OFFSET1)
      SDS_MPHI(IDATA) = int(MPHI*SCALE2+OFFSET1)
      SDS_Scattering_Angle(IDATA)=int(MSCATT*SCALE2+OFFSET1)
...
C************************************************************************
     ...
     *   Refl_6,Refl_7,Refl_8,Refl_9,Refl_12,Refl_13,Refl_16,Refl_26,Rad_20,
     *   Rad_31,Rad_32,LandSea_Flag,CldMsk_1km,SnowMsk_500m,
     *   SnowMsk_Ratio,Refl_15,Total_H2O,Total_O3,
     *   SLOPE_MEAN_LAND,SLOPE_MEAN_OCEAN,W659_SYN,W865_SYN,W470_SYN,
     *   W550_SYN,W124_SYN,W164_SYN,W213_SYN,W412_SYN,W443_SYN,
     *   W0P75_SYN,W869o_SYN,W395_SYN,W138_SYN,W1100_SYN,W1100_SYN_500m,
     *   W1200_SYN,G_factor,iscan,idata,Multi_factor)
...
C *\ Global variables
     ...
     *     Refl_9 (ISWATH,ILINE),Refl_12(ISWATH,ILINE),
     *     Refl_13(ISWATH,ILINE),Refl_16(ISWATH,ILINE),
     *     Refl_26(ISWATH,ILINE),Refl_15(ISWATH,ILINE),
     *     Rad_20(ISWATH,ILINE),Rad_31(ISWATH,ILINE),
     *     Rad_32(ISWATH,ILINE)
...
C Intialize the data    
           ...
           W412_SYN(IS,IL)=-999.0
           W443_SYN(IS,IL)=-999.0
           W0P75_SYN(IS,IL)=-999.0 
...
C  Added chaneels 412 & 445 and 0.75 NPP channels

        
          IF(Refl_8(IS,IL).GT.0.0) THEN
            W412_SYN(IS,IL)=Refl_8(IS,IL)*Multi_factor(8)  
          ENDIF
            IF(Refl_9(IS,IL).GT.0.0) THEN
            W443_SYN(IS,IL)=Refl_9(IS,IL)*Multi_factor(9)  
          ENDIF
            IF(Refl_15(IS,IL).GT.0.0) THEN
             W0P75_SYN(IS,IL)=Refl_15(IS,IL)*Multi_factor(10)  
            ENDIF
...
C
      ...
      INTEGER CldMsk_1km(ISWATH,ILINE),quality_cirrus(ISWATH,ILINE)
       Integer Aerosol_Cldmask_land(ISWATH*2,ILINE*2)
       ...
       DO IX=1,Data_Size(1)
          ...
          DO p = 1,2 
           ...
           IF(CldMsk_1km(IX,IY).EQ.1 .and.CldMsk_500(p2,l2).EQ.1)THEN
            CldMsk_500(p2,l2)=1
            Aerosol_Cldmask_land(p2,l2)=1
           ELSE
            CldMsk_500(p2,l2)=0
            Aerosol_Cldmask_land(p2,l2)=0
            ENDIF
          ENDDO
          ENDDO
          Y4_offset = (IY-1)*4
...
C----------------------------------------------------------------------
      ...
      INCLUDE 'mod04.inc'
        REAL diff, sumdif
      INTEGER START_1KM
      INTEGER IX1,IX2,IY1,IY2,I,iscan,idata
...
C endif for lon_max.........     
      ...
      RETURN
      END
           Subroutine Make_joint_aerosolCldmask (LandSea_Flag,iscan,
     &     CldMsk_500_Ocean,CldMsk_500,Joint_Cld_mask_500)
           include 'mod04.inc'  
          integer Joint_Cld_mask_500(iswath*2,Iline*2),iscan
           INTEGER       LandSea_Flag(ISWATH,ILINE)
           integer CldMsk_500_Ocean(2*ISWATH,2*ILINE+1)
           integer CldMsk_500(2*ISWATH,2*ILINE)
          Integer ixx,iyy,y2,x2,X2_offset,Y2_offset ,l,p,JX,JY
! set the bad data to 9   
            Do  IYY=1,ILINE 
            Do  IXX=1,ISWATH
           if(LandSea_Flag(IXX,IYY) .lt. 0 
     &     .or.LandSea_Flag(IXX,IYY) .gt.7)then  
           LandSea_Flag(IXX,IYY)=9    
           endif
           Enddo
           Enddo
              ...
              DO JY=1,ILINE*2 
              DO JX=1,ISWATH*2 
                Joint_Cld_mask_500(JX,JY) = -9999
               enddo
               enddo 
c Mzke a joint cloud mask for Land & ocean together             
                Do  IYY=1,ILINE 
                Do  IXX=1,ISWATH	
               Y2_offset = (IYY-1)*2
               X2_offset = (IXX-1)*2
 !        IF(LandSea_Flag(IXX,IYY).EQ.0 )THEN
     ...
     &        LandSea_Flag(IXX,IYY) . eq. 7) then
              ...
              Y2 = Y2_offset + l
              X2 = X2_offset + p
            Joint_Cld_mask_500(X2,Y2) = CldMsk_500_Ocean(x2,y2)
             ENDDO
             ENDDO  
!         ELSEIF(LandSea_Flag(IXX,IYY).GT.0 .And.
!     &              LandSea_Flag(IXX,IYY).LE.3)Then
     ...
     &            LandSea_Flag(IXX,IYY) . eq. 4)THEN
              ...
              Y2 = Y2_offset + l
              X2 = X2_offset + p
            Joint_Cld_mask_500(X2,Y2) = CldMsk_500(x2,y2)
                   ENDDO
                   ENDDO
             ENDIF
             ENDDO
             ...
             Return
             End
       Subroutine Cloud_distance (mask,nx,ny,New_distance)
      Implicit None
        include 'mod04.inc' 
      integer mask(Iswath*2,Iline*2*Tot_scan)
      real  north (Iswath*2,Iline*2*Tot_scan),dmax
      real  south (Iswath*2,Iline*2*Tot_scan)
      real  east (Iswath*2,Iline*2*Tot_scan)
      real  west (Iswath*2,Iline*2*Tot_scan)
      real  dist(Iswath*2,Iline*2*Tot_scan),dtest,dedge,dhorz 
      real New_distance(Iswath*2,Iline*2*Tot_scan)
      integer  k,j,nx,ny,kmark,jmark,k1,k2,j1,j2
      integer  Kfile,iwrite,lll,kkk,Jy,is,ip,jx,iks
 
*Intialize
      do k=1,ny
      do j=1,nx
      New_distance(j,k)=-9999
      enddo
      enddo
* top edge of north array      
      k=1
      do j=1,nx
        north(j,k)=0.
        if(mask(j,k).eq.1) north(j,k)=1.
      enddo
...
* rest of north array
      ...
      do j=1,nx
        north(j,k)=0.
        if(mask(j,k).eq.1) north(j,k)=north(j,k-1)+1.
      enddo
      enddo

* bottom edge of south array

      k=ny
      do j=1,nx
        south(j,k)=0.
        if(mask(j,k).eq.1) south(j,k)=1.
      enddo
...
* rest of south array
      ...
      do j=1,nx
        south(j,k)=0.
        if(mask(j,k).eq.1) south(j,k)=south(j,k+1)+1.
      enddo
      enddo

* left edge of west array

      j=1
      do k=1,ny
        west(j,k)=0.
        if(mask(j,k).eq.1) west(j,k)=1.
      enddo
...
* rest of west array
      ...
      do k=1,ny
        west(j,k)=0.
        if(mask(j,k).eq.1) west(j,k)=west(j-1,k)+1.
      enddo
      enddo

* right edge of east array

      j=nx
      do k=1,ny
        east(j,k)=0.
        if(mask(j,k).eq.1) east(j,k)=1.
      enddo
...
* rest of east array
      ...
      do k=1,ny
        east(j,k)=0.
        if(mask(j,k).eq.1) east(j,k)=east(j+1,k)+1.
      enddo
      enddo

 

* square all the arrays

      do k=1,ny
      do j=1,nx
        north(j,k) = north(j,k) * north(j,k)
        south(j,k) = south(j,k) * south(j,k)
        west(j,k) = west(j,k) * west(j,k)
        east(j,k) = east(j,k) * east(j,k)
      enddo
      enddo

* use jmark,kmark as test point with east and west arrays

      dmax=float(nx)*float(nx)+float(ny)*float(ny)
      do kmark=k1,k2
      do jmark=j1,j2
         dist(jmark,kmark)=dmax
      do k=1,ny
         dtest=min(west(jmark,k),east(jmark,k))
     *        +float(kmark-k)*float(kmark-k)
         dist(jmark,kmark)=min(dist(jmark,kmark),dtest)
      enddo
      enddo
      enddo
...
* use jmark,kmark as test point with north and south arrays

      dmax=float(nx)*float(nx)+float(ny)*float(ny)
      do kmark=1,ny
      do jmark=1,nx
         dedge=float(jmark)*float(jmark)
         dist(jmark,kmark)=dedge
         dedge=float(nx+1-jmark)*float(nx+1-jmark)
         dist(jmark,kmark)=min(dist(jmark,kmark),dedge)
      do j=jmark,nx
         dhorz=float(jmark-j)*float(jmark-j)
         if(dhorz.gt.dist(jmark,kmark)) goto 91
         dtest=min(north(j,kmark),south(j,kmark))+dhorz
         dist(jmark,kmark)=min(dist(jmark,kmark),dtest)
      enddo
 91   continue
      do j=jmark,1,-1
         dhorz=float(jmark-j)*float(jmark-j)
         if(dhorz.gt.dist(jmark,kmark)) goto 92
         dtest=min(north(j,kmark),south(j,kmark))+dhorz
         dist(jmark,kmark)=min(dist(jmark,kmark),dtest)
      enddo
 92   continue 
       New_distance(jmark,kmark)=sqrt(dist(jmark,kmark)) 
        if(New_distance(jmark,kmark) .gt.60)
     &    New_distance(jmark,kmark)=60.0
        enddo
        enddo 
!Write cld distance 
  1   continue 
       Return 
           ...
           Subroutine Average_cloud_distance(Save_index_Land_Ocean,
     &     save_numpix_Land_Ocean,cloud_distance,XX,YY,New_Average_dis) 
           implicit None
           include 'mod04.inc'
           Real cloud_distance(Iswath*2,Iline*2*Tot_scan)
           integer  XX,YY,iwrite,is,ip,kkk,jx,jy,num,data_size(2)
           Real array(Max_Pixels_O) 
           integer Nfile,START_y,End_y,START_X,End_X,Idata,iscan
           integer lll,iks,number,kfile,Gfile,Tfile,Cfile,Afile,Bfile
           Integer Save_index_Land_Ocean(NUMCELLS,Tot_scan,Max_Pixels_O) 
           Integer  iXX,IYY,save_numpix_Land_Ocean(NUMCELLS,Tot_scan)
           real new_Average_dis(NUMCELLS,Tot_scan)  
              START_y=1
               End_y=20 
            DO  Iscan = 1,YY/20
              START_X=1
               End_X=20 
            DO   IDATA = 1,XX/20 
             New_Average_dis(Idata,iscan)=0 
              num=0
              do Jy=START_y,End_y
              do Jx=START_X,End_X
               num=num+1
              array(num)=cloud_distance(JX,JY)    
              enddo
              enddo
           iyy=save_numpix_Land_Ocean(idata,Iscan)  
           if( Iyy .gt.0) then
               number=0
            do iks=1,iyy
            Jx=Save_index_Land_Ocean(Idata,Iscan,iks) 
            if( array(jx) .gt.0) then
             number=number+1 
           New_Average_dis(Idata,iscan)=New_Average_dis(Idata,iscan)+
     &     array(jx) 
             endif
             Enddo
            
             if( number.gt.0) then
        New_Average_dis(Idata,iscan)=New_Average_dis(Idata,iscan)/number 
               Else
              New_Average_dis(Idata,iscan)=-9999
               Endif
           Else
            New_Average_dis(Idata,iscan)=-9999
              Endif 
...
C----------------------------------------------------------------------
       ...
       INTEGER*2  SDS_ref(NUMCELLS,NWAV+3),SDS_ref_STD(NUMCELLS,NWAV+3)
     ...
     &            SDS_Sea_Land_Flag(ISWATH,Tot_scan) 
       ...
       INTEGER*2  SDS_small_weighting(NUMCELLS),
     &            SDS_correc_small_weighting(NUMCELLS),
     &            SDS_Least_error(NUMCELLS,NUM_solutions),
     &            SDS_effrad(NUMCELLS,NUM_solutions),
     &            SDS_sol_INDX_small(NUMCELLS,NUM_solutions),
     ...
     &            SDS_TranF_land(NUMCELLS,Land_Sol1)
      ...
      INTEGER  INT_Fill_value,Qcontrol_special_land,Num_Wav
     ...
     &      SDS_ccn(NUMCELLS,NUM_solutions)
       ...
       INTEGER*2 SDS_NUMPIXELS(NUMCELLS,NWAV+3),
     &   SDS_SCAT_ANGLE_OCEAN(NUMCELLS)
c       
          INT_Fill_value=-9999
         ...
         Do Idata = 1,NumSq 
          SDS_SCAT_ANGLE_OCEAN(IDATA)=INT_Fill_value 
           ...
           SDS_ratio_small_Land_Ocean(IDATA)=INT_Fill_value 
            SDS_angs_coeff1(IDATA) =INT_Fill_value
            SDS_angs_coeff2(IDATA) =INT_Fill_value
            SDS_small_weighting(IDATA) = INT_Fill_value
            SDSTAU_average_0p55um(IDATA) = INT_Fill_value
...
c         SDS_CLDFRC_land(IDATA)=INT_Fill_value
         ...
         Do Num_Wav=1,Band_land+3
         SDS_NUMPIXELS_land(IDATA, Num_Wav)=INT_Fill_value 
         SDS_ref_land(IDATA, Num_Wav) =INT_Fill_value 
         SDS_ref_STD_land(IDATA, Num_Wav)=INT_Fill_value
         Enddo
...
C Write Sea_Land_mask from Wisconsin Cloud mask.......  
     ...
     &      (MODIS_W_GENERIC,'Problem picking sds ,MOD04_v2.f')  
           Return
           End   
       subroutine MOD04_OUT_Extra_500(MODFIL,Ncell,Nrow,SDS_Value,name) 
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
C MOD04_OUT : Interface to two subroutines which populate HDF arrays
C              fort land and ocean aerosol retrival algorithm.
...
C
     ...
     &     SDS_Value(2*ISWATH,2*ILINE*Tot_scan) 
