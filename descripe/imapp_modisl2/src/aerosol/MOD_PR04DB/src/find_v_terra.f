...
! TODO: Convert all of this to F95
      subroutine find_v_terra(realbuf, outbuf, tmpvg, qa_flag)
...
Cc-------------------------------------------------------------
Cc
C      subroutine new_intep(x1a,x2a,x3a,ya,m,n,l,ia,x1,x2,x3,y,dy,
C     1                     mbeg,nbeg,frac)
C      dimension x1a(m),x2a(n),x3a(l),ya(4,4,2,10)
C      dimension xx2a(4), xx1a(4)
C      dimension yntmp(4),ymtmp(4),yltmp(2)
C 
C      do 12 j=1,4
C        do 11 k=1,4
C          yltmp(1)=ya(j,k,1,ia)
C          yltmp(2)=ya(j,k,2,ia)
C          yntmp(k) = yltmp(1)*(1.-frac) + yltmp(2)*frac
C          xx2a(k) = x2a(k+nbeg)
C11      continue
C        call polint(xx2a,yntmp,4,x2,ymtmp(j),dy)
C        xx1a(j) = x1a(j+mbeg)
C12    continue
C      call polint(xx1a,ymtmp,4,x1,y,dy)
C      return
C      end
Cc
Cc-------------------------------------------------------------
Cc
C      subroutine new_intepw(x1a,x2a,x3a,ya,m,n,l,ia,x1,x2,x3,y,dy,
C     1                     mbeg,nbeg,frac)
C      dimension x1a(m),x2a(n),x3a(l),ya(4,4,2,8)
C      dimension xx2a(4), xx1a(4) 
C      dimension yntmp(4),ymtmp(4),yltmp(2)
C       
C      do 12 j=1,4 
...
C          w=c(i+1)-d(i)
C          den=ho-hp
Cc          if(den.eq.0.) print *,'den=',den
Cc          if(den.eq.0.)pause
C          den=w/den
C          d(i)=hp*den
C          c(i)=ho*den
C12      continue
...
Cc     --------------
Cc
C      subroutine search(dflag,xbar,x,n,i)
Cc
Cc     purpose
Cc       locate position in table of point at which interpolation is
Cc       required
Cc
Cc     usage
Cc       call  search (xbar, x, n, i)
Cc
Cc     description of parameters
Cc       xbar   - point at which interpolation is required
Cc       x      - array containing independent variable
Cc       n      - number of points in x array
Cc       i      - index specifying segment containing xbar
Cc
C      real*8 com1(6),com2(6)
C      logical dflag
C      dimension x(n)
C      data b/.69314718/
C      data com1/8hsearch  , 8h n is le, 8hss than ,
C     1          8h2.      , 8h        , 8h        /
C      data com2/8hsearch  , 8h table i, 8hs not in,
C     1          8h increas, 8hing orde, 8hr       /
C      icnt = 0
C      if (n.lt.2) go to 15
C      if(x(1).gt.x(2)) go to 17
C      m = int((log(float(n)))/b)
C      i=2**m
C      if (i .ge. n) i = n-1
C      k=i
C   10 k=k/2
C      if (k .eq. 0) icnt = icnt + 1
C      if (icnt .ge. 2) goto 27
C      if (xbar.ge.x(i).and.xbar.lt.x(i+1))return
C      if (xbar.gt.x(i)) go to 12
C      i = i-k
C      go to 10
C   12 i = i+k
C      if (i.lt.n) go to 10
C      i=n-1
C      go to 10
C   15 print 100,com1
C      return
...
C   27 continue
C      do 22 i=1,n-1
C         if (xbar.ge.x(i).and.xbar.le.x(i+1)) return
C   22 continue           
C      write(6,*) "setting dflag = true"
C      dflag = .true.
C      return
C
C  100 format(5x,6a8)
C      end
C
C      subroutine search2(dflag,xbar,x,n,i,fx)
Cc
Cc       call  search (xbar, x, n, i)
Cc
Cc     description of parameters
...
Cc       n      - number of points in x array
Cc       i      - index specifying segment containing xbar
Cc       fx     - fraction from i (between i and i+1)
Cc
C      dimension x(n)
C      logical dflag
C
C      call search(dflag,xbar,x,n,i)
C
C      fx   = (xbar-x(i))/ (x(i+1)-x(i))
C
C      end
C
Cc--------------------------------------------------------
C      subroutine aero_412_abs(dflag,refl,x1,x2,x3,mm,nn,ll,
C     1    r412,index_ia,frac_ia,w0_x)
C
C      include 'aottbl.inc'
C      include 'newaottbl.inc'
C
C      real nvalxw(10,46,30,8), nnvalxw(4,4,2,8), yyw(8)
C      logical dflag
C      data pi   /3.14159/
C
C      dflag = .false.
...
C     1    print *,'aero_412_abs frac on sfc=', frac
C 
...
C       do i = 1, 4
C        do j = 1, 4
C        dd1 = nvalx412(mbeg+i,nbeg+j,ii,index_ia,iw,index_ii)*
C     1  (1.-frac) + 
C     1  nvalx412(mbeg+i,nbeg+j,ii,index_ia,iw,index_ii+1)*frac
C        dd2= nvalx412(mbeg+i,nbeg+j,ii,index_ia+1,iw,index_ii)*
C     1  (1.-frac)+
C     1   nvalx412(mbeg+i,nbeg+j,ii,index_ia+1,iw,index_ii+1)
C     2   *frac   
C 
C         nnvalxw(i,j,1,iw) = dd1* (1.-frac_ia) + dd2*frac_ia   
C 
C        dd1 = nvalx412(mbeg+i,nbeg+j,ii+1,index_ia,iw,index_ii)*
C     1  (1.-frac) +
C     1   nvalx412(mbeg+i,nbeg+j,ii+1,index_ia,iw,index_ii+1)*frac 
C        dd2= nvalx412(mbeg+i,nbeg+j,ii+1,index_ia+1,iw,index_ii)* 
C     1  (1.-frac)+
C     1   nvalx412(mbeg+i,nbeg+j,ii+1,index_ia+1,iw,index_ii+1)
C     2   *frac  
C 
C        nnvalxw(i,j,2,iw) = dd1* (1.-frac_ia) + dd2*frac_ia
C 
C        enddo
C       enddo
C      enddo
C 
Cc---     interpolating W0 tables
C 
C      do 800 iw = 1, 8
C 
C      call new_intepw(theta0, theta, phi, nnvalxw, mm, nn, ll, iw,
C     1          x1,x2,x3,y,dy,mbeg,nbeg,xfrac)
C 
C      yyw(iw) = y/pi
C 800  continue
C 
C      if (refl.le.yyw(1)) then
C        w0_x = 0.82
C        return
C      endif
C 
C      if (refl.ge.yyw(8)) then
C        w0_x = 1.0
C        return
C      endif
C 
C      call search2(dflag,refl,yyw,8,index_ii,frac)
C      if (dflag) return
C      w0_x = frac*w0(index_ii+1) + (1.-frac)*w0(index_ii)
C
C      return
C      end
C
Cc--------------------------------------------------------
C      subroutine aero_470_abs(dflag2,refl,x1,x2,x3,mm,nn,
C     1    ll,r470,index_ia,frac_ia,w0_x)
C 
C      include 'aottbl.inc'
C      include 'newaottbl.inc'
C 
C      real nvalxw(10,46,30,4), nnvalxw(4,4,2,4), yyw(4)
C      logical dflag2
C      data pi   /3.14159/
C 
C      dflag2 = .false.
...
C     1    print *,'index_ii r470abs = ', index_ii
C      if (frac.lt.0.0.or.frac.gt.1.0) 
C     1    print *,'frac on sfc=', frac
C 
C      call search(dflag2,x3,phi,ll,ii)
C      if (dflag2) return
C      xfrac = (x3-phi(ii))/(phi(ii+1)-phi(ii))
C 
C      nsm=1
...
C       do i = 1, 4
C        do j = 1, 4
C        dd1 = nvalx470(mbeg+i,nbeg+j,ii,index_ia,iw,index_ii)*
C     1  (1.-frac) + 
C     1  nvalx470(mbeg+i,nbeg+j,ii,index_ia,iw,index_ii+1)*frac
C        dd2= nvalx470(mbeg+i,nbeg+j,ii,index_ia+1,iw,index_ii)*
C     1  (1.-frac)+
C     1   nvalx470(mbeg+i,nbeg+j,ii,index_ia+1,iw,index_ii+1)
C     2   *frac   
C 
C         nnvalxw(i,j,1,iw) = dd1* (1.-frac_ia) + dd2*frac_ia   
C 
C        dd1 = nvalx470(mbeg+i,nbeg+j,ii+1,index_ia,iw,index_ii)*
C     1  (1.-frac) +
C     1   nvalx470(mbeg+i,nbeg+j,ii+1,index_ia,iw,index_ii+1)*frac 
C        dd2= nvalx470(mbeg+i,nbeg+j,ii+1,index_ia+1,iw,index_ii)* 
C     1  (1.-frac)+
C     1   nvalx470(mbeg+i,nbeg+j,ii+1,index_ia+1,iw,index_ii+1)
C     2   *frac  
C 
C        nnvalxw(i,j,2,iw) = dd1* (1.-frac_ia) + dd2*frac_ia
C 
...
C      endif
C 
C      if (refl.ge.yyw(4)) then
C        w0_x = 1.0
C        return
C      endif
C 
C      call search2(dflag2,refl,yyw,4,index_ii,frac)
C      if (dflag2) return
C      w0_x = frac*w0_470(index_ii+1) + (1.-frac)*w0_470(index_ii)
C 
C      return
C      end
C
...
C     1    imod,r470,tau_x470,tau_x470_flag,trflg,model_frac,debug)
C     
C        implicit none
C        
C        include 'aottbl.inc'
C        include 'newaottbl.inc'
C        
C        logical, intent(inout)      ::  dflag
C        real, intent(in)            ::  refl
C        real, intent(in)            ::  x1
C        real, intent(in)            ::  x2
C        real, intent(in)            ::  x3
C        integer, intent(in)         ::  mm
C        integer, intent(in)         ::  nn
C        integer, intent(in)         ::  ll
C        integer, intent(in)         ::  ma
C        integer, intent(in)         ::  imod
C        real, intent(in)            ::  r470
C        real, intent(inout)         ::  tau_x470
C        integer, intent(inout)      ::  tau_x470_flag
C        real, intent(inout)         ::  trflg
C        real, intent(in)            ::  model_frac
C        logical, intent(in)         ::  debug
C        
C        
C        
C!       -- NOTE: if model_frac = 0.0, the aerosol model = imod
C!       -- if model_frac > 0.0, the aerosol model will be interpolated between
C!       --  imod and imod + 1, using the value of model_frac
C
C        real, dimension(4,4,2,10) ::  nnvalx, nnvalx1, nnvalx2 
C        real, dimension(10)       ::  yy
C        real, dimension(8)        ::  yy2
C      
C        real      ::  frac_ia, w0_x
C        integer   ::  index_ia
C        real      ::   tau_x
C      
C        integer ::  ia, i, ii, index_ii, nsm, nsn, mbeg, nbeg, iw, j
C        real    ::  dif, dift, frac, xfrac, dd1, dd2, y, dy
C        real    ::  xxrat
C      
C        real, parameter   ::  pi = 3.14159
C        
C        tau_x470_flag = -999  
C        
C        if (debug) print *, 'aero_470, in: ', refl, x1, x2, x3, r470, imod, model_frac
C
C        index_ii = r470
C 
C        frac = (r470-sfc_ref470(index_ii)) / (sfc_ref470(index_ii+1)-sfc_ref470(index_ii))
C        if (debug) print *, 'aero_470, sfc indx: ', r470, index_ii
C 
C        if (index_ii < 1 .or. index_ii > 24) print *,'index_ii = ', index_ii, r470
C        if (frac < 0.0 .or. frac > 1.0) print *,'aero_470 frac on sfc=', frac
C 
C        call search(dflag,x3,phi,ll,ii)
C        if (dflag) return
C        xfrac = (x3-phi(ii))/(phi(ii+1)-phi(ii))
C        if (debug) print *, 'aero_470, raa: ', x3, ii, phi(ii), phi(ii+1)
C        
C        nsm = 1
C        dif = x1 - theta0(1)
C        do i = 1, mm
C          dift = x1 - theta0(i)
C          if (dift > 0.0 .and. dift < dif) then
C            nsm = i
C            dif = dift
C          end if
C        end do
...
C        if (nbeg <= 0) then
C          nbeg = 0
C        else if (nbeg > nn-4) then
C          nbeg = nn-4
C        end if
C        if (debug) print *, 'aero_470, mbeg, nbeg: ', mbeg, nbeg
C        
C!       -- interpolate between models if requested.
C !       if (present(model_frac)) then
C!       -- temporary check to avoid out-of-bounds condition with imod=4
C!       -- change to above when this file is converted to a proper module
C        if (imod < 4) then
C          nnvalx1(:,:,:,:)  = -999.0
C          nnvalx2(:,:,:,:)  = -999.0
C          nnvalx(:,:,:,:)   = -999.0
...
C            do i = 1, 4
C              do j = 1, 4
C                nnvalx1(i,j,1,ia) = nvalx470(mbeg+i,nbeg+j,ii,ia,imod,index_ii)*
C     1             (1.0-frac) + nvalx470(mbeg+i,nbeg+j,ii,ia,imod,index_ii+1)*frac
C                nnvalx1(i,j,2,ia) = nvalx470(mbeg+i,nbeg+j,ii+1,ia,imod,index_ii)*
C     1             (1.0-frac) + nvalx470(mbeg+i,nbeg+j,ii+1,ia,imod,index_ii+1)*frac
C  
C                nnvalx2(i,j,1,ia) = nvalx470(mbeg+i,nbeg+j,ii,ia,imod+1,index_ii)*
C     1             (1.0-frac) + nvalx470(mbeg+i,nbeg+j,ii,ia,imod+1,index_ii+1)*frac
C                nnvalx2(i,j,2,ia) = nvalx470(mbeg+i,nbeg+j,ii+1,ia,imod+1,index_ii)*
C     1             (1.0-frac) + nvalx470(mbeg+i,nbeg+j,ii+1,ia,imod+1,index_ii+1)*frac
C  
C                nnvalx(i,j,1,ia) = (1.0-model_frac) * nnvalx1(i,j,1,ia) +   
C     1                                 model_frac * nnvalx2(i,j,1,ia)
C                nnvalx(i,j,2,ia) = (1.0-model_frac) * nnvalx1(i,j,2,ia) +  
C     1                              model_frac * nnvalx2(i,j,2,ia)
C              end do
C            end do
C          end do
C          
C!       -- just use imod, no interpolation
C        else      
C          do ia = 1, 10
C            do i = 1, 4
C              do j = 1, 4
...
C        if (refl <= yy(1)) then
C          tau_x470 = 0.06 
C          if (trflg > 0.0) tau_x470 = 0.02
C          tau_x470_flag = -10
C          if (debug) print *, 'aero_470, hit low bound: ', refl, yy(1)
C          return
C        end if
C 
C! Reflc off the charts!  Set AOT to max and set flag.
C        if (refl >= yy(10)) then
C          xxrat = 0.8
C          tau_x470 = 3.5
C          tau_x470_flag = 1
C          if (debug) print *, 'aero_412, hit hi bound: ', refl, yy(10)
C          return
C        endif
C
C!
...
C!
C 
C        if (yy(1) < yy(2)) go to 650
C 
C        if (refl < yy(4)) return
C      
C        yy2(:) = -999.0
C        do i = 1, 7
C          yy2(i) = yy(i+3)
...
C        tau_x470 = tau_x
C        tau_x470_flag = 0
C        if (debug) print *, 'aero_470, exit 2371, aot: ', tau_x470
C!      print *,'tau_x470 =', tau_x470
C 
C        return
C      end subroutine aero_470
C      
CC      subroutine aero_470(dflag,refl,x1,x2,x3,mm,nn,ll,ma,
CC     1    imod,r470,tau_x470,tau_x470_flag,trflg,model_frac)
CC 
CC      include 'aottbl.inc'
CC      include 'newaottbl.inc'
CC      
CC      logical, intent(in), optional ::  model_frac
CC      
CC      real nnvalx(4,4,2,10), yy(10), yy2(8)
CC      integer tau_x470_flag, imod
CC      logical dflag
CC      data pi   /3.14159/
CC
CC      index_ii = r470
CC 
CC      frac = (r470-sfc_ref470(index_ii))/
CC     1       (sfc_ref470(index_ii+1)-sfc_ref470(index_ii))
CC 
CC      if (index_ii.lt.1.or.index_ii.gt.24)
CC     1    print *,'index_ii = ', index_ii,xlat,xlong
CC      if (frac.lt.0.0.or.frac.gt.1.0)
CC     1    print *,'aero_470 frac on sfc=', frac, r470, imod
CC 
CC      call search(dflag,x3,phi,ll,ii)
CC      if (dflag) return
CC      xfrac = (x3-phi(ii))/(phi(ii+1)-phi(ii))
CC 
CC      nsm=1
CC      dif=x1-theta0(1)
CC      do i=1,mm
CC        dift=x1-theta0(i)
CC        if (dift.gt.0. .and. dift.lt.dif) then
CC          nsm=i
CC          dif=dift
CC        endif
CC      enddo
CC      mbeg = nsm - 2
CC      if (mbeg.le.0) then
CC        mbeg = 0
CC      else if (mbeg.gt.mm) then
CC        mbeg = mm-4
CC      endif
CC 
CC      nsn=1
...
CC      enddo
CC      nbeg = nsn - 2
CC      if (nbeg.le.0) then
CC        nbeg = 0
CC      else if (nbeg.gt.nn) then
CC        nbeg = nn-4
CC      endif
CC 
CC      do ia = 1, 10
CC       do i = 1, 4
CC        do j = 1, 4
CC       nnvalx(i,j,1,ia) = nvalx470(mbeg+i,nbeg+j,ii,ia,imod,index_ii)*
CC     1  (1.-frac) + nvalx470(mbeg+i,nbeg+j,ii,ia,imod,index_ii+1)*frac
CC       nnvalx(i,j,2,ia) = nvalx470(mbeg+i,nbeg+j,ii+1,ia,imod,index_ii)*
CC     1  (1.-frac) + nvalx470(mbeg+i,nbeg+j,ii+1,ia,imod,index_ii+1)*frac
CC        enddo
CC       enddo
CC      enddo
CC 
CCc---     interpolating AOT tables
CC
CC      do 105 ia = 1, 10
CC 
CC      call new_intep(theta0, theta, phi, nnvalx, mm, nn, ll, ia,
CC     1          x1,x2,x3,y,dy,mbeg,nbeg,xfrac)
CC
CC      yy(ia) = y/pi
CCc      print *,'tau, i/f=', tau(ia), y/pi, dy
CC 105  continue
CC      if (refl.le.yy(1)) then
CC       tau_x470 = 0.06
CC       if (trflg.gt.0.0) tau_x470 = 0.02
CC       tau_x470_flag = -10
CC       return
CC      endif
CC      if (refl.ge.yy(10)) then
CC       xxrat = 0.8
CC       tau_x470_flag = 1
CC       return
CC      endif
CCc
CCc
CCc     Check if the reflectance increase with AOT
CCc
CC 
CC      if (yy(1).lt.yy(2)) go to 650
CC 
CC      if (refl.lt.yy(4)) return
CC 
CC      do i = 1, 7
CC      yy2(i) = yy(i+3)
CC      enddo
CC 
CC      if (yy2(2).lt.yy2(1)) return
CC      call search2(dflag,refl,yy2,7,index_ii,frac)
CC      if (dflag) return
CC      tau_x = frac*tau(index_ii+1+3) + (1.-frac)*tau(index_ii+3)
CC      tau_x470 = tau_x
CC      return
CC 
CC650   continue
CCc
CCc     Pass the monotonic order check
CCc
CC      call search2(dflag,refl,yy,10,index_ii,frac)
CC      if (dflag) return
CCc      print *,'after 2nd search'
CC      tau_x = frac*tau(index_ii+1) + (1.-frac)*tau(index_ii)
CC 
CC      tau_x470 = tau_x
CC
CCc      print *,'tau_x470 =', tau_x470
CC      return
CC      end
C
Cc--------------------------------------------------------
C! TODO: check/use/add status flag      
C      subroutine aero_650(dflag,refl,x1,x2,x3,mm,nn,ll,ma,r650,tau_x650,  
C     1                    tau_x650_flag,tau_x470_flag,tau_x412, tau_x470, 
C     1                    tau_x412_flag_91,trflg)
C        implicit none
C        
C        include 'aottbl.inc'
C        include 'newaottbl.inc'
...
C        integer, intent(in)           ::  ll
C        integer, intent(in)           ::  ma
C        real, intent(in)              ::  r650
C        real, intent(inout)           ::  tau_x650
C        integer, intent(inout)        ::  tau_x650_flag
C        integer, intent(in)           ::  tau_x470_flag
C        real, intent(in)              ::  tau_x412
C        real, intent(in)              ::  tau_x470
C        integer, intent(in)           ::  tau_x412_flag_91
C        real, intent(in)              ::  trflg
C        
C        real, dimension(4,4,2,10)     ::  nnvalx
C        real, dimension(10)           ::  yy
C        real, dimension(8)            ::  yy2
C        real, dimension(4)            ::  yy3
C        real, dimension(6)            ::  yy5
C     
C        real, parameter               ::  pi = 3.14159
C
C        real                          ::  frac, xfrac, dif, dift
C        real                          ::  y, dy, w0_x, tau_x
C        integer                       ::  index_ii, ii, nsm, nsn, mbeg, nbeg
C        integer                       ::  ia,i, j
C        
C        tau_x650_flag = -999
C             
C        index_ii = (r650 + 1.0) / 2.0
C 
C        frac = (r650-sfc_ref650(index_ii)) / (sfc_ref650(index_ii+1)-sfc_ref650(index_ii))
C 
C        if (index_ii < 1.or.index_ii > 24) print *,'index_ii = ', index_ii
C        if (frac < 0.0.or.frac > 1.0) print *,'aero_650 frac on sfc=', frac
C 
C        if (index_ii < 1) then
C          index_ii = 1
C          frac = 0.0
C        end if
C 
...
C      end do
C      
C      if (refl <= yy(1) .and. yy(1) < yy(2)) then
C        tau_x650 = 0.002
C        if (trflg > 0.0) tau_x650 = 0.02
C        tau_x650_flag = -10
C        return
C      end if
C 
...
C      end if
C
C!     -- check for absorbing dust over bright surface
C      if (refl >= yy(7)) then
C         do i = 1, 4
C         yy3(i) = yy(i+6)
C         enddo
...
C         call search2(dflag,refl,yy3,4,index_ii,frac)
C         if (dflag) return
C         tau_x = frac*tau(index_ii+1+6) + (1.-frac)*tau(index_ii+6)
C         tau_x650 = tau_x
C         tau_x650_flag = 0
C         return
C      endif
C      
C!     -- check if 470 or 412 retrievals were off the charts (refl > yy(10))
C      if (tau_x470_flag > 0) go to 670
C      if (tau_x412_flag_91 > 0) go to 680
C
C!
C!     Check if the reflectance increase with AOT
C!
...
C
C670   continue
Cc      if (refl < yy(8)) return
Cc
Cc      do i = 1, 3
Cc        yy3(i) = yy(i+7)
Cc      end do
Cc
Cc      if (yy3(2) < yy3(1)) return
Cc      call search2(dflag,refl,yy3,3,index_ii,frac)
Cc      if (dflag) return
Cc      tau_x = frac*tau(index_ii+1+7) + (1.-frac)*tau(index_ii+7)
Cc      tau_x650 = tau_x
Cc      tau_x650_flag = 0
Cc      return
C
C680   continue
C      if (refl < yy(5)) return
C
C      do i = 1, 6
C        yy5(i) = yy(i+4)
C      end do
C
C      if (yy5(2) < yy5(1)) return
C      call search2(dflag,refl,yy5,6,index_ii,frac)
C      if (dflag) return
C!      print *,'after 1st search'
C!      if (yy3(1) > yy3(2)) print *,'yy=',refl,(yy(i),I=1,10)
...
C      return
C      
C      end subroutine aero_650
C
C
CC      subroutine aero_650(dflag,refl,x1,x2,x3,mm,nn,ll,ma,
CC     1    r650,tau_x650,tau_x650_flag,tau_x470_flag,
CC     2    tau_x412, tau_x470,tau_x412_flag_91,trflg)
CC 
CC      include 'aottbl.inc'
CC      include 'newaottbl.inc'
CC
CC      real nnvalx(4,4,2,10), yy(10), yy2(8), yy3(3), yy5(6)
CC      real tau_x650, tau_x412, tau_x470
CC      real refl,x1,x2,x3,r650
CC
CC      integer tau_x650_flag, tau_x470_flag, tau_x412_flag_91
CC      integer mm,nn,ll,ma
CC      logical dflag
CC      data pi   /3.14159/
CC
CC      index_ii = (r650+1.)/2.
CC 
CC      frac = (r650-sfc_ref650(index_ii))/
CC     1       (sfc_ref650(index_ii+1)-sfc_ref650(index_ii))
CC 
CC      if (index_ii.lt.1.or.index_ii.gt.24) 
CC     1    print *,'index_ii = ', index_ii,xlat,xlong
CC      if (frac.lt.0.0.or.frac.gt.1.0) 
CC     1    print *,'aero_650 frac on sfc=', frac
CC 
CC      if (index_ii.lt.1) then
CC        index_ii = 1
CC        frac = 0.
CC      endif
CC 
...
CC       do i = 1, 4
CC        do j = 1, 4
CC          nnvalx(i,j,1,ia) = nvalx650(mbeg+i,nbeg+j,ii,ia,index_ii)*
CC     1       (1.-frac) + nvalx650(mbeg+i,nbeg+j,ii,ia,index_ii+1)*frac   
CC          nnvalx(i,j,2,ia) = nvalx650(mbeg+i,nbeg+j,ii+1,ia,index_ii)*
CC     1       (1.-frac) + nvalx650(mbeg+i,nbeg+j,ii+1,ia,index_ii+1)*frac   
CC        enddo
CC       enddo
CC      enddo
CC 
...
CC 600  continue
CC 
CC      if (refl.le.yy(1).and.yy(1).lt.yy(2)) then
CC       tau_x650 = 0.06
CC       if (trflg.gt.0.0) tau_x650 = 0.02
CC       return
CC      endif
CC 
CC      if (refl.ge.yy(10)) then
...
CC      endif
CC
CC      if (tau_x470_flag.gt.0) go to 670
CC      if (tau_x412_flag_91.gt.0) go to 680
CC
CCc
CCc     Check if the reflectance increase with AOT
CCc
...
CC
CC670   continue
CC      if (refl.lt.yy(8)) return
CC
CC      do i = 1, 3
CC      yy3(i) = yy(i+7)
CC      enddo
CC
CC      if (yy3(2).lt.yy3(1)) return
CC      call search2(dflag,refl,yy3,3,index_ii,frac)
CC      if (dflag) return
CC      tau_x = frac*tau(index_ii+1+7) + (1.-frac)*tau(index_ii+7)
CC      tau_x650 = tau_x
...
CC
CC680   continue
CC      if (refl.lt.yy(5)) return
CC
CC      do i = 1, 6
CC      yy5(i) = yy(i+4)
CC      enddo
CC
CC      if (yy5(2).lt.yy5(1)) return
CC      call search2(dflag,refl,yy5,6,index_ii,frac)
CC      if (dflag) return
CCc      print *,'after 1st search'
CCc      if (yy3(1).gt.yy3(2)) print *,'yy=',refl,(yy(i),I=1,10)
CC      tau_x = frac*tau(index_ii+1+4) + (1.-frac)*tau(index_ii+4)
CC      tau_x650 = tau_x
CC      return
CC 
CC650   continue
...
C
Cc--------------------------------------------------------
C      subroutine aero_412(dflag,refl,x1,x2,x3,mm,nn,ll,ma,imod,r412, 
C     1                    tau_x412,tau_x412_flag,trflg,model_frac,debug)
C        
C        implicit none
C        
C        include 'aottbl.inc'
...
C        integer, intent(in)         ::  ma
C        integer, intent(in)         ::  imod
C        real, intent(in)            ::  r412
C        real, intent(inout)         ::  tau_x412
C        integer, intent(inout)      ::  tau_x412_flag
C        real, intent(in)            ::  trflg
C        real, intent(in)            ::  model_frac
C        logical, intent(in)         ::  debug
C        
...
C        integer                     ::  index_ii, ii, nsm, nsn, mbeg, nbeg
C        integer                     ::  ia, j, i
C        real                        ::  frac, xfrac, dif, dift, y, dy
C        real                        ::  xxrat, tau_x
C                
C        if (debug) print *, 'aero_412, in: ', refl, x1, x2, x3, r412, imod, model_frac
C
C        tau_x412 = -999.0
C        tau_x412_flag = -999
C                
C        index_ii = r412
C        if (r412 < 0.0) return
C        frac = (r412-sfc_ref412(index_ii)) / (sfc_ref412(index_ii+1)-sfc_ref412(index_ii))
C        if (debug) print *, 'aero_412, sfc indx: ', r412, index_ii
C        if (index_ii < 1 .or. index_ii > 20) then
C          print *,'index_ii = ', index_ii
C          dflag = .false.
C          return
C        end if
C        
C        if (frac < 0.0 .or. frac > 1.0) then
C          print *,'aero_412 frac on sfc=', frac, r412, sfc_ref412(index_ii)
C          dflag = .false.
C          return
C        end if
C      
...
C        if (dflag) return
C        xfrac = (x3-phi(ii))/(phi(ii+1)-phi(ii))
C        if (debug) print *, 'aero_412, raa: ', x3, ii, phi(ii), phi(ii+1)
C
C        nsm = 1
C        mbeg = -999
C        dif = x1 - theta0(1)
...
C          nbeg = nn-4
C        end if
C        if (debug) print *, 'aero_412, mbeg, nbeg: ', mbeg, nbeg
C
C!       -- interpolate between models if requested.
C!        if (present(model_frac)) then
C        if (imod < 8) then              ! temp check to avoid out-of-bounds issue with imod=8
C          nnvalx1(:,:,:,:) = -999.0
C          nnvalx2(:,:,:,:) = -999.0
C          do ia = 1, 10
C            do i = 1, 4
C              do j = 1, 4
C                nnvalx1(i,j,1,ia) = nvalx412(mbeg+i,nbeg+j,ii,ia,imod,index_ii)*
C     1            (1.0-frac) + nvalx412(mbeg+i,nbeg+j,ii,ia,imod,index_ii+1)*frac
C                nnvalx1(i,j,2,ia) = nvalx412(mbeg+i,nbeg+j,ii+1,ia,imod,index_ii)*
C     1             (1.0-frac) + nvalx412(mbeg+i,nbeg+j,ii+1,ia,imod,index_ii+1)*frac
C  
C                nnvalx2(i,j,1,ia) = nvalx412(mbeg+i,nbeg+j,ii,ia,imod+1,index_ii)*
C     1             (1.0-frac) + nvalx412(mbeg+i,nbeg+j,ii,ia,imod+1,index_ii+1)*frac
C                nnvalx2(i,j,2,ia) = nvalx412(mbeg+i,nbeg+j,ii+1,ia,imod+1,index_ii)*
C     1             (1.0-frac) + nvalx412(mbeg+i,nbeg+j,ii+1,ia,imod+1,index_ii+1)*frac
C  
C                nnvalx(i,j,1,ia) = (1.0-model_frac) * nnvalx1(i,j,1,ia) +   
C     1                                 model_frac * nnvalx2(i,j,1,ia)
C                nnvalx(i,j,2,ia) = (1.0-model_frac) * nnvalx1(i,j,2,ia) +   
...
C        if (refl <= yy(1)) then
C          tau_x412 = 0.06 
C          if (trflg > 0.0) tau_x412 = 0.02
C          tau_x412_flag = -10       
C          if (debug) print *, 'aero_412, hit low bound: ', refl, yy(1)
C          return
C        end if
C
C! Reflc off the charts!  Set AOT to max and set flag.
...
C        tau_x412 = tau_x
C        tau_x412_flag = 0
C        if (debug) print *, 'aero_412, exit 2355, aot: ', tau_x412
C        return
C 
C650     continue
C!
...
C        tau_x412 = tau_x
C        tau_x412_flag = 0
C        if (debug) print *, 'aero_412, exit 2367, aot: ', tau_x412
C        return
C 
C      end subroutine aero_412
C      
CC      subroutine aero_412(dflag,refl,x1,x2,x3,mm,nn,ll,ma,
CC     1    imod,r412,tau_x412,tau_x412_flag,trflg,debug)
CC 
CC      include 'aottbl.inc'
CC      include 'newaottbl.inc'
CC      
CCc      logical, intent(in), optional ::  model_frac
CC      logical, intent(in) ::  debug
CC      
CC      real nnvalx(4,4,2,10), yy(10), yy2(8)
CC      integer tau_x412_flag
CC      logical dflag
CC      data pi   /3.14159/
CC 
CC      if (debug) print *, 'aero_412_old, in: ', refl, x1, x2, x3, r412, w0_x
CC
CC      index_ii = r412
CC 
CC      frac = (r412-sfc_ref412(index_ii))/
CC     1       (sfc_ref412(index_ii+1)-sfc_ref412(index_ii))
CC 
CC      if (index_ii.lt.1.or.index_ii.gt.20)
CC     1    print *,'index_ii = ', index_ii
CC      if (frac.lt.0.0.or.frac.gt.1.0)
CC     1    print *,'aero_412 frac on sfc=', frac
CC      
CC      if (debug) print *, 'aero_412_old, sfc indx: ', r412, index_ii
CC      
CC      call search(dflag,x3,phi,ll,ii)
CC      if (dflag) return
CC      xfrac = (x3-phi(ii))/(phi(ii+1)-phi(ii))
CC      if (debug) print *, 'aero_412_old, raa: ', x3, ii, phi(ii), phi(ii+1)
CC      
CC      nsm=1
CC      dif=x1-theta0(1)
CC      do i=1,mm
...
CC      if (nbeg.le.0) then
CC        nbeg = 0
CC      else if (nbeg.gt.nn-4) then
CC        nbeg = nn-4
CC      endif
CC      if (debug) print *, 'aero_412_old, mbeg, nbeg: ', mbeg, nbeg
CC      
CC      do ia = 1, 10
CC       do i = 1, 4
CC        do j = 1, 4
CC        nnvalx(i,j,1,ia) = nvalx412(mbeg+i,nbeg+j,ii,ia,imod,index_ii)*
CC     1   (1.-frac) + nvalx412(mbeg+i,nbeg+j,ii,ia,imod,index_ii+1)*frac   
CC       nnvalx(i,j,2,ia) = nvalx412(mbeg+i,nbeg+j,ii+1,ia,imod,index_ii)*
CC     1  (1.-frac) + nvalx412(mbeg+i,nbeg+j,ii+1,ia,imod,index_ii+1)*frac   
CC        enddo
CC       enddo
CC      enddo
CC 
...
CC 
CC      do 100 ia = 1, 10
CC      if (debug) print *, mm, nn, ll, ia, x1, x2, x3, mbeg, nbeg, xfrac
CC      call new_intep(theta0, theta, phi, nnvalx, mm, nn, ll, ia,
CC     1          x1,x2,x3,y,dy,mbeg,nbeg,xfrac)
CC      if (debug) print *,'tau, i/f=', tau(ia), y/pi, dy
CC      yy(ia) = y/pi
CC 100  continue
CC 
CC      if (refl.le.yy(1)) then
CC       tau_x412 = 0.06
CC       if (trflg.gt.0.0) tau_x412 = 0.02
CC       tau_x412_flag = -10
CC       if (debug) print *, 'aero_412_old, hit low bound: ', refl, yy(1)
CC       return
CC      endif
CC 
CC      if (refl.ge.yy(10)) then
CC       xxrat = 0.8
CC       tau_x412_flag = 1
CC       if (debug) print *, 'aero_412_old, hit hi bound: ', refl, yy(10)
CC       return
CC      endif
CCc
CCc     Check if the reflectance increase with AOT
...
CC      tau_x = frac*tau(index_ii+1+3) + (1.-frac)*tau(index_ii+3)
CC      tau_x412 = tau_x
CC      if (debug) print *, 'aero_412_old, exit 2482, aot: ', tau_x412
CC      return
CC 
CC650   continue
CCc
...
CC 
CC      tau_x412 = tau_x
CC      if (debug) print *, 'aero_412_old, exit 2494, aot: ', tau_x412
CC      return
CC      end
C
Cc------------------------------------------------------------
C      subroutine aero_mod (tau_x412,tau_x470,tau_x650,
C     1           tau_x412_91,aot_mod)
C
C      real aot_mod(6)
C
C      aot_mod(1) = tau_x650
C      aot_mod(2) = tau_x470
C      aot_mod(3) = tau_x412_91
C      aot_mod(4) = tau_x412*1.9
C      aot_mod(5) = tau_x470*2.
C      aot_mod(6) = tau_x650*2.8
C      return
C      end
C      
C			integer function handle_lut_out_of_bounds(geo_zone, retr_flag, aot) result(status)
C				implicit none
C				
C				integer, intent(in)	:: geo_zone
C				integer, intent(in)	:: retr_flag
C				real, intent(inout)	:: aot
C				
C				status = 0
C												
C				if (retr_flag == -10) then      ! -10 = return value from aero_412, etc indicating
C					select case (geo_zone)        ! refl < yy(1) scenario.
C						case (5:11, 15, 16, 19, 20, 21, 23, 24, 100)
C							aot = 0.06
C						case (1:4, 12:14, 17, 18, 22, 25)
C							aot = 0.02
C						case (-999)
C							aot = 0.02
C						case default
C							print *, "ERROR: Invalid geographical zone in handle_lut_out_of_bounds: ", geo_zone
C							status = -1
C							return
C					end select
C				end if
...
C				return
C				
C			end function handle_lut_out_of_bounds
C			
C			
C			subroutine smoke_mod(view,scat_ang,dd412,dd470,dd650,  
C     1                     tau_x412ss,tau_x412ss_91,tau_smoke)
C        implicit none
C        
C        real, intent(in)        ::  view
C        real, intent(in)        ::  scat_ang
C        real, intent(in)        ::  dd412
C        real, intent(in)        ::  dd470
C        real, intent(in)        ::  dd650
C        real, intent(in)        ::  tau_x412ss, tau_x412ss_91
C        real                    ::  tau_smoke
C     
C        if (dd412 <1.0 .and. dd470 <1.0) then
C            tau_smoke = tau_x412ss * 2.85  
C          if (dd470 > dd412 .and. dd650 > dd412)   
C     1      tau_smoke = tau_x412ss_91 * 1.77
C          if (abs(dd412-dd470) < 0.25) then
C            if (dd412 <0.35) tau_smoke = tau_x412ss * 4.3
C            if (scat_ang <158.0 .and. dd412 <0.8) then
C            tau_smoke = tau_x412ss * 3.5
C            if (view >45.0 .and. dd470 <0.2 .and. abs(dd412-dd470) < 0.24)  
C     1        tau_smoke = tau_x412ss * 6.0
C            endif
C          endif
C          if (abs(dd412-dd650) < 0.1 .and. dd412 >0.7)    
C     1        tau_smoke = tau_x412ss_91
C          if (dd650 > 1.0 .and. abs(dd412-dd470) < 0.25) then
C              tau_smoke = tau_x412ss * 4.5
C          if (scat_ang <158.0 .and. dd412 >0.8)    
C     1        tau_smoke = tau_x412ss * 3.5
C          endif
C        endif
C     
C        if (dd412 >1.0 .and. dd470 <1.0) then
C             tau_smoke = tau_x412ss * 2.7
C          if (abs(dd412-dd470) < 0.25 .and. dd650 <0.)   
C     1      tau_smoke = tau_x412ss * 2.4
C        endif
C     
C        if (dd412 <1.0 .and. dd470 >1.0) then
C             tau_smoke = tau_x412ss * 2.7
C          if (abs(dd412-dd470) < 0.25 .and. dd650 <0.)    
C     1        tau_smoke = tau_x412ss * 2.4
C          if (dd650 - dd470 > -0.2) then
C             tau_smoke = tau_x412ss * 2.1
C          if (view < 45.0) tau_smoke = tau_x412ss_91
C          endif
C        endif
C     
C        if (dd412 >1.0 .and. dd470 >1.0) then
C          if (dd412 > dd470) tau_smoke = tau_x412ss_91 * 1.15
C          if (dd470 > dd650 .and. dd650 > dd412 .and. dd650 >1.0) then
C            if (view >= 45.0) tau_smoke = tau_x412ss * 2.1
C            if (view <  45.0) tau_smoke = tau_x412ss_91
C          endif
C          if (dd412 <2.0 .and. dd470 >2.0)   
C     1      tau_smoke = tau_x412ss * 1.4
C          if (dd650 > dd470 .and. dd470 > dd412) then
C            if (view <= 25.0) tau_smoke = (tau_x412ss+tau_x412ss_91)/2.
C            if (view > 25.0 .and. abs(dd412-dd470) < 0.2)  
C     1        tau_smoke = (tau_x412ss+tau_x412ss_91)/2.
C          endif
C          if (dd650 > 3.5) tau_smoke = tau_x412ss
C        endif
C     
C        if (dd650 <-1.0 .and. abs(dd412-dd650)>1.9 .and. view >45.) then
C          if (dd412 >= 0.88) tau_smoke = tau_x412ss * 4.0
C          if (dd412 <0.88 .and. dd412 >= 0.6) tau_smoke = tau_x412ss *6.0
C          if (dd412 <0.6 .and. dd412 >= 0.35) tau_smoke = tau_x412ss *8.0
C          if (dd412 <0.35) tau_smoke = tau_x412ss_91 *8.0
C          if (abs(dd412-dd470) < 0.14 .and. dd650 > -1.7 .and. dd412 <0.4)  
C     1        tau_smoke = tau_x412ss * 3.5
C          if (dd412 <0.) tau_smoke = tau_x412ss_91 *9.0
C        endif
C     
C        return
C 
C      end subroutine smoke_mod
C			
