...
Cc-----------failed to open or read met grib product: system problem, exit immediately
            if ( status .ne. 0 ) then
               level = 2
...
c--------ozone file entry exists in PCF
         if ( status .eq. PGS_S_SUCCESS ) then
            ESDT_name = 'OZ_DAILY'
            status    = modis_grib_driver( LUN_OZ_DAILY, LUN_TEMP_OZ_DAILY, 
     1                                     ESDT_name, errmsg, 
     2                                     ozn_year, ozn_month, 
     3                                     ozn_day, ozn_hour ) 
               
c-----------failed to open or read ozone grib product - system problem exit immediately
            if ( status .ne. 0 ) then
               level = 2
               call message( 'GetAncData_PGE04', errmsg, status, level ) 

c-----------open ozone temp file
            else    
               pcfnum = LUN_TEMP_OZ_DAILY
               reclen = 360*181*4
               status = openr_temp( pcfnum, reclen, lun )

c--------------failed to open ozone temp file - system problem, exit immediately
               if ( status .ne. 0 ) then

                  level = 2
                  write( errmsg,'(''Error opening OZONE temp file on LUN'',i12)') 
     &            pcfnum

                  call message( 'GetAncData_PGE04', errmsg //
     &            ' [OPERATOR ACTION: Contact SDST]', status, level ) 

c--------------read ozone temp file
               else    
                  read( lun, rec = 1, iostat = ios ) ozn_grid

c-----------------failed to read ozone temp file - system problem, exit immediately
                  if ( ios .ne. 0 ) then
                     level = 2
                     write( errmsg,'(''Error reading OZONE temp file on LUN'',i12)') 
     &               pcfnum
                     call message( 'GetAncData_PGE04', errmsg //
     &               ' [OPERATOR ACTION: Contact SDST]', ios, level )
                  else
                     ozn_success = .true.
                  endif ! failed to read ozone temp file

                  call modis_io_gen_closef( pcfnum, lun )
               endif ! failed to open ozone temp file  

            endif ! failed to open/read ozone grib file 
 
c--------no ozone file in PCF; write warning message and run without 
         else
            level = 1
            write( errmsg, '(''No OZONE file entry found on PCF LUN'',i12)') LUN_OZ_DAILY
            call message( 'GetAncData_PGE04', errmsg, status, level )

         endif ! no ozone file entry in PCF  


Ccc-----------------------------------------------------------------------
Ccc     Get SSM/I sea ice concentration 
Ccc-----------------------------------------------------------------------
Ccc ...   Unpack grib sea ice file and write to binary file
Cc        errmsg    = ' '
Cc        ESDT_name = 'SEA_ICE'
Cc        status    = modis_grib_driver( LUN_SEA_ICE, LUN_TEMP_SEA_ICE, 
Cc     1                                 ESDT_name, errmsg, 
Cc     2                                 ice_year, ice_month, 
Cc     3                                 ice_day, ice_hour ) 
Cc
Cc        if ( status .ne. 0 ) then
Cc          level = 2
Cc          call message( 'GetAncData_PGE04', errmsg, status, level ) 
Cc
Cc        else    
Ccc ......  Open unpacked ice file
Cc          pcfnum = LUN_TEMP_SEA_ICE
Cc          reclen = 720*360*4 
Cc          status = openr_temp( pcfnum, reclen, lun )
Cc
Cc          if ( status .ne. 0 ) then
Cc            level = 2
Cc            write( errmsg,'(''Error opening sea_ice temp file'',i12)') pcfnum
Cc            call message( 'GetAncData_PGE04', errmsg //
Cc     &           ' [OPERATOR ACTION: Contact SDST]', status, level ) 
Cc
Cc          else    
Ccc ........  Read the unpacked ice file
Cc            if ( status .eq. 0 ) then
Cc              read( lun, rec = 1, iostat = ios ) ice_grid
Cc
Cc              if ( ios .ne. 0 ) then
Cc                level = 2
Cc                write( errmsg,'(''Error reading sea_ice temp file'',i12)') pcfnum
Cc                call message( 'GetAncData_PGE04', errmsg //
Cc     &          ' [OPERATOR ACTION: Contact SDST]', ios, level ) 
Cc              else    
Cc                ice_success = .true.
Cc              endif   
Cc
Cc              call modis_io_gen_closef( pcfnum, lun )
Cc            endif   
Cc          endif  
Cc        endif   
C
