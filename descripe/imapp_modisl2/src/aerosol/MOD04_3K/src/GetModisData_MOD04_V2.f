     ...
     &        Refl_1,Refl_2,Refl_3,Refl_4,Refl_5,Refl_6,Refl_7,Refl_9,
     &        Refl_12,Refl_13,Refl_16,Refl_26,Rad_20,Rad_31,Rad_32,
     ...
     &        DayNight_Flag,SunGlint_Flag,SnowIce_Flag,LandSea_Flag,
     &        Non_CloudOb_Flag,Thin_CirNIR_Flag,Shadow_Flag,
     &        Thin_CirIR_Flag,Cloud_SimpIR_Flag,High_Cloud_Flag,
     &        Cloud_IRTemp_Flag,Cloud_3p75_11_Flag,Cloud_VisRat_Flag,
     &        Cloud_SpatVar_Flag,Cloud,QA_CLOUD)
...
C New
     ...
     8    Cloud_SpatVar_Flag(Buf_Size1,Buf_Size2)
...
C
      Do  J_flag = 1, Data_size(2)
        ...
        Enddo
      Enddo
c       NUMSQ=(Data_Size(1)/IGRIDX)*(Data_Size(2)/IGRIDY)
        NUMSQ=(Data_Size(1)/IGRIDX)
...
C Old setup
C
      Buf_Dims1 = 4*Buf_Size1
      Buf_Dims2 = 4*Buf_Size2
C
C      If (ReadCldMsk_MOD04(Modfil_CldMsk,ScanCube_No,
C     2                      Buf_Dims1,Buf_Dims2,SDS_DIM,
C     3                      Cloud,LandSea_Flag) .ne. 0)
C     4   CALL MODIS_SMF_SETDYNAMICMSG(MODIS_F_GENERIC,
C     5   'Read Cloud Mask failed','GetModisDat_MOD04')

C
C
C New setup - old - 18 bytes QA
C
C      CALL Read_CldMsk(Modfil_CldMsk,ScanCube_No,Buf_cldmsk,Buf_Size1,
C     1                 Buf_Size2,Buf_cldmsk_QA,Cloud,QA_Cloud,
C     2                 Error_Flag)
C
C New setup - new - 10 bytes QA
C
...
C Declarations
     ...
     &          LandSea_Flag(Buf_Size1,Buf_Size2)
      PARAMETER (Fmax=1500,Lmax=ILINE,NofLine_PerCube=ILINE,FILL_INT=-1000)
...
C Retrieve Satellite Azimuth and compute Relative Azimuth
         Else If (I.eq.6) Then
            ...
            Do 65 J = 1, DataSize(1)
               L = L + 1
               if (count(L).eq.FILL_INT) then
C                  Sol_Az(J,K) = FILL_INT
                  Sol_Az(J,K) = FILL_INT
                  Rel_Az(J,K) = FILL_INT
               else
C                  Sat_Az(J,K) = count(L)*0.018/pi
                  Sat_Az(J,K) = count(L)*0.01
c                  Sat_Az(J,K) = abs(Sat_Az(J,K) - Sol_Az(J,K))
...
C Declarations
     ...
     2             MaxScanCube_No,Dim_Size(3)
      ...
      PARAMETER    (No_byte=6,Fmax=1500,Lmax=ILINE)
...
C
C      PARAMETER    (No_Byte=6,No_Byte_QA=18,Fmax=1500,Lmax=ILINE)
C
...
c     3    SnowIce_Flag(Fmax,Lmax),LandSea_Flag(Fmax,Lmax),
     4    Non_CloudOb_Flag(Fmax,Lmax),Thin_CirNIR_Flag(Fmax,Lmax),
     5    Shadow_Flag(Fmax,Lmax),Thin_CirIR_Flag(Fmax,Lmax),
     6    Cloud_SimpIR_Flag(Fmax,Lmax),High_Cloud_Flag(Fmax,Lmax),
     7    Cloud_IRTemp_Flag(Fmax,Lmax),Cloud_3p75_11_Flag(Fmax,Lmax),
