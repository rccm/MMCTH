...
C
     ...
     &        MODFIL_ANC(MODFILLEN)
      ...
      INTEGER CLDMSK_500(2*ISWATH,2*ILINE),Quality_Flag_for_retr
...
C
     ...
     * New_QA_Flag_Land,snow_present_Flag,coastal_Pix)
c      write(37,*)'A',iscan,idata,IPROCE,ETA,Qcontrol_special_land
...
C
     ...
     * ERR644,Num_Pixels_Used,Qcontrol_special_land,Quality_flag_forJoint,
     * New_QA_Flag_Land,snow_present_Flag,coastal_Pix)
c       write(37,*)'B',iscan,idata,IPROCE,ETA,Qcontrol_special_land
...
c                   to BYTE.
      ...
      INTEGER Qcontrol_special_land,Quality_flag_forJoint,Quality_Flag_for_retr
...
C
     ...
     &            SDS_TranF_land(NUMCELLS,Land_Sol1)
      ...
      REAL AOD(NLWAV),AODF(NLWAV),AODC(NLWAV)
      REAL ERR644
      INTEGER ETA_FLAG,snow_present_Flag,coastal_Pix
      SAVE
...
C quality_land .eq.0 is if there is one single pixel of water 
        ...
        IF (quality_land .eq.0)then
         ...
         ENDIF
c If app 50% of pixels in 3*3 box are coastal degrade the Quality
         
           If( coastal_Pix .ge. 5) then
          QA_Flag_Land(8)=0 
...
c       if( Num_Pixels_Used .ge. 12 .and. Num_Pixels_Used .le.20)then   
c       QA_Flag_Land(8)=0 
c       Quality_Flag_for_retr=6
c       Endif
c       if( Num_Pixels_Used .ge. 21 .and. Num_Pixels_Used .le.30) then  
c       QA_Flag_Land(8)=1
c       Quality_Flag_for_retr=7
c       ENdif
c        if( Num_Pixels_Used .ge.31 .and. Num_Pixels_Used .le.50)then   
c        QA_Flag_Land(8)=2
c        Quality_Flag_for_retr=8
c       ENDIF
c        if( Num_Pixels_Used .ge.51)QA_Flag_Land(8)=3 
...
C in  3*3 KM box(9 pixels). Aerosol snow mask is quite strict and seems to work well but may fail
C sometimes. It is just diagnostics.  Shana Mattoo 4/2013
       if( snow_present_Flag .le. 1)snow_present_Flag=0      
       if( snow_present_Flag .gt. 1)snow_present_Flag=1  
...
C
c  New_QA_Flag_Land(I) just for testing  take it off later 

      DO I=1,19 
       New_QA_Flag_Land(I)= QA_Flag_Land(I)
        QA_Flag_Land(I)=0
      ENDDO
...
c enddo for wavelengths
        ENDDO
c      write(89,*)inum,REFW466_L,REFW550_L,REFW644_L,REFW212_L
