...
c      INTEGER * 2 TOTOZONE((ISWATH/5)*(ILINE/5))
      INTEGER * 2 TOTOZONE(ISWATH*ILINE)
      REAL  NEWWATER_VAPOR(ISWATH,ILINE),ADD_OFFSET,SCALE_FACTOR
      REAL  NEWTOT_OZONE(ISWATH*ILINE)
...
C Call GMFIN to get 'Number of Scans' from L1B dataset
     ...
     &'READ_AVERAGE_MODIS_DEPEN Get_Metatable for number of scans')
            write(37,*) 'from read_modisdep',numscan
             NUMSCAN=((NUMSCAN*10)/ILINE) 
             write(37,*) 'from read_modisdep_next',numscan
            grpnm=' '
...
c             ******* 1 lines and 2000 in one scan cube
...
C ********AVERAGEWATER_VAPOR  FOR EACH 10*10 SQUARE
      ...
      IGRIDYTOT=ILINE
      NUMSQ=(IGRIDXTOT/ILINE)*(IGRIDYTOT/ILINE)
      IFACTOR=1
      IDIMX=10*IFACTOR
      JDIMY=10*IFACTOR
      ISQ=0
