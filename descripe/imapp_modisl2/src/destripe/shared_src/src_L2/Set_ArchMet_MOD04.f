      Integer Function Set_ArchMet_MOD04( MET_Handles,
...
C !DESCRIPTION:
C
C   Set MOD04_L2 (MODIS L2 aerosol product) ECS "Archive" metadata objects
...
C   the MOD04_L2 metadata configuration file (MCF).  Function calls to the
...
C   Set_ArchMet_MOD04 - only the association of values to ODL objects is
...
C   PGS_MET_Write.  This call is made from outside Set_ArchMet_MOD04, after
C   all other ECS metadata relevant to the process have been set.
C
C   The specific metadata fields set by Set_ArchMet_MOD04, their
...
C                Set_ArchMet_MOD04 as formatted, F8.2 ASCII character strings.
C
C
...
C                 Update_InvMeta_MOD04.  Update_InvMeta_MOD04 returns the
...
C        Set_ArchPSA_MOD04         (science code)
C        Set_GRing                 (science code)
C        Set_LclInputGranID_MOD04  (science code)
C        Set_RP_Data_Atmos         (science code)
C        String_Loc                (science code)
C
C    Named Constant:
C        ARCHIVEMETADATA           (mod04_ECSMET.inc)
C        FAIL                      (mod04_ECSMET.inc)
C        LUN_OF_NUM_ARCH_RP_MOD04  (mod04_ECSMET.inc)
...
c-----PARAMETER declarations
      character*(*)  FUNCNAME
      parameter     (FUNCNAME = 'Set_ArchMet_MOD04')
...
c-----Declaration of local variables
      ...
      integer string_loc,
     1        Set_RP_Data_Atmos, Set_LclInputGranID_MOD04,
     2        Set_GRing, Set_ArchPSA_MOD04
...
C----------------------------------------------------------------------
     ...
     1                         LUN_OF_NUM_ARCH_RP_MOD04,
     2                         ARCHIVEMETADATA )

      If (rtn .eq. FAIL) Then
         error_flag = .true.

         msgbuf = 'Set_RP_Data_Atmos detected error setting MOD04 archive RP data.'
...
C-----------------------------------------------------------------
C Update MOD04 LocalInputGranuleID
C-----------------------------------------------------------------

      rtn = Set_LclInputGranID_MOD04( MET_Handles,
     1                                NUM_Of_MODIS_InputFiles,
     2                                LUN_MODIS_InputFiles,
     3                                VRSN_MODIS_InputFiles )
      ...
      If (rtn .eq. FAIL) Then
         ...
         msgbuf = 'Set_LclInputGranID_MOD04 detected error incrementing ECS '
...
C----------------------------------------------------------------------
      ...
      rtn = Set_ArchPSA_MOD04( MET_Handles,
     ...
     3                         Value_ArchivePSA_SC )
      ...
      If (rtn .eq. FAIL) Then
         ...
         msgbuf = 'Set_ArchPSA_MOD04 detected error setting archive PSA attribute.'
     ...
     3            // 'Set_ArchPSA_MOD04.'
      ...
      If (.not.error_flag) Set_ArchMet_MOD04 = SUCCEED
...
c---------------------------------------------------------------------------------------
      subroutine Copy_LclInputGranID_MOD04( NUM_MODIS_FileNames,
     1                                      MODIS_FileNames,
     2                                      rtn_flag)
...
C                from the MOD04_L2 product file to array storage.  
...
C        MAX_NUM_LCLPNTRS          (mod04_ECSMET.inc)
...
C PARAMETER declarations
      character*(*)   FUNCNAME
      PARAMETER      (FUNCNAME = 'Copy_LclInputGranID_MOD04')
...
c-------------------------------------------------------------------------------
c Copy existing MOD04 LocalInputGranuleID to internal memory, and count the
...
c--------count the number of MODIS file names retrieved from MOD04 LocalInputGranuleID 
...
c-----LocalInputGranuleID not yet set 
      Else If ( rtn.eq.PGSMET_E_NULL_PARAMETER .OR. rtn.eq.PGSMET_E_PARAMETER_NOT_SET) Then
         ...
         msgbuf = 
     1   MECS_ARCHIVE // ' in MOD04 product file, yet the required object ' 
...
c-----Failed to read LocalInputGranuleID 
     ...
     2   // char(10) // 'from MODIS Aerosol product (LUN = ' // msg25_LUN(fbyte_LUN:lbyte_LUN)
...
c---------------------------------------------------------------------------------------
      Integer Function Set_ArchPSA_MOD04( MET_Handles,
     1                                    NUM_NewArchPSA_SC,
...
C        NUM_ARCH_PSA_SC_MOD04     (mod04_ECSMET.inc)
C        PGSMET_E_NULL_PARAMETER   (PGS_MET_13.f)
C        PGS_S_SUCCESS             (PGS_SMF.f)
C        SUCCEED                   (mod04_ECSMET.inc)
C        VRSN_MOD04                (mod04_ECSMET.inc)
...
c-----PARAMETER declarations
      character*(*)  FUNCNAME
      parameter     (FUNCNAME = 'Set_ArchPSA_MOD04')
...
C------------------------
      ...
      Set_ArchPSA_MOD04  = SUCCEED
      arch_exists        = .FALSE.
      rtn_status         = 0
      error_flag         = .FALSE.

      write(msg25_a,'(i25)')   NUM_NewArchPSA_SC
      write(msg25_b,'(i25)')   NUM_ARCH_PSA_SC_MOD04
...
c--------------------------------------------------------------------------------
c If archive metadata are in file, loop over PSAs in MODIS Aerosol Product and 
c cotpy existing PSAs to internal memory.
c--------------------------------------------------------------------------------

      If (arch_exists) Then

         Do 100 i1 = 1, NUM_ARCH_PSA_SC_MOD04
            PSA_Name  = Name_ArchPSA_SC_MOD04(i1)
            rtn_loc   = string_loc(PSA_Name,fbyte,lbyte)

            rtn1 = PGS_MET_GetPCAttr_s( LUN_MOD04,
     1                                  VRSN_MOD04,
     2                                  MECS_ARCHIVE,
     3                                  PSA_Name,
     4                                  PSA_Value )
...
c-----------PSA read successful. Now copy PSA to memory
     ...
     2                                   PSA_Value )
               ...
               If (rtn2 .EQ. FAIL) Then
                  ...
                  msgbuf =
     1            'PGS_MET_SetAttr_s detected error setting MOD04 Archive PSA '
...
c-----------PSA is not in archive metadata.
     ...
     2            // ' unsuccessful - '
     3            // char(10) // PSA_Name(fbyte:lbyte) // ' not yet in MOD04 ' // MECS_ARCHIVE // '! '
...
c-----test for too many archive PSAs
      ...
      If (NUM_NewArchPSA_SC .gt. NUM_ARCH_PSA_SC_MOD04) Then
         error_flag = .true.
         Lcl_NUM_NewPSA = NUM_ARCH_PSA_SC_MOD04

         msgbuf =
     1   'Number of new archive PSAs (=' // msg25_a(fbyte_a:lbyte_a) // ') exceeds total '
     2   // char(10) // 'number in MOD04 product (' // msg25_b(fbyte_b:lbyte_b) // ').'
...
c--------check validity of new PSA name
         ...
         Do 300 i3 = 1, NUM_ARCH_PSA_SC_MOD04
            If (Name_ArchPSA_SC_MOD04(i3) .eq. PSA_Name) Name_Is_Valid = .TRUE.
300      Continue
...
c--------invalid PSA name
         If (.NOT. Name_Is_Valid) Then
            ...
            msgbuf = 'Archive PSA name ' // PSA_Name(fbyte:lbyte) // ' not on MOD04 metadata list.'
...
200   Continue   ! Loop on Input PSAs
      ...
      If (.not.error_flag) Set_ArchPSA_MOD04 = SUCCEED
...
c---------------------------------------------------------------------------------------
      Integer Function Set_LclInputGranID_MOD04( MET_Handles,
...
C        MODIS_SMF_SETDYNAMICMSG     (science code)
C        Query_HDF_Attr              (science code)
C        Copy_LclInputGranID_MOD04   (science code)
C        Update_LclInputGranID_MOD04 (science code)
C
C
C    Named Constant:
C        ARCHIVEMETADATA            (mod04_ECSMET.inc)
...
C PARAMETER declarations
      character*(*) FUNCNAME
      PARAMETER (FUNCNAME = 'Set_LclInputGranID_MOD04')
...
c-----Initialization
      Set_LclInputGranID_MOD04 = FAIL
      error_flag               = .FALSE.
      NUM_Old_LCLGRANID        = 0
      NUM_Total_LCLGRANID      = 0

c-----------------------------------------------------------------------
c Check if ECS archive metadata are in MOD04 product file
...
c-----------------------------------------------------------------------
     ...
     4                     archive_exists )
      ...
      If (rtn_status_query .eq. FAIL) then
         ...
         msgbuf = 
     1   'Query_HDF_Attr detected error searching MOD04 aerosol product file '
...
c-------------------------------------------------------------------------------
      ...
      If (archive_exists) Then
         ...
         Call Copy_LclInputGranID_MOD04( NUM_Old_LCLGRANID,
     1                                   LCLGRANID,
     2                                   rtn_status )
         ...
         If (rtn_status .eq. FAIL) Then
            ...
            msgbuf =
     1      'Copy_LclInputGranID_MOD04 detected error copying LocalInputGranuleID '
     2      // char(10) // 'from MOD04 product file. '
     3      // char(10) // 'Operator Action:  Refer to prior low level error LogStatus '
     4      // char(10) // 'messages generated by routine Copy_LclInputGranID_MOD04.'
...
c-----names used by current process.
      ...
      Call Update_LclInputGranID_MOD04( NUM_MODIS_InputFiles,
     ...
     6                                  rtn_status )
      ...
      If (rtn_status .eq. FAIL) Then
         ...
         msgbuf = 
     1   'Update_LclInputGranID_MOD04 detected error setting LocalInputGranuleID '
     2   // char(10) // 'from MOD04 product file. '
     3   // char(10) // 'Operator Action:  Refer to prior low level error LogStatus '
     4   // char(10) // 'messages generated by routine Update_LclInputGranID_MOD04.'
...
c-----If sufficient storage, add end-of-string marker to LCLGRANID array 
     ...
     4    // char(10) // 'correct files and rerun PGE.  Otherwise, notify SDST.'
      ...
      If (.NOT. error_flag) Set_LclInputGranID_MOD04 = SUCCEED
...
c---------------------------------------------------------------------------------------
      Subroutine Update_LclInputGranID_MOD04( NUM_MODIS_InputFiles,
     1                                        LUN_MODIS_InputFiles,
     2                                        VRSN_MODIS_InputFiles,
     3                                        NUM_Old_LCLGRANID,
     4                                        LCLGRANID,
...
C PARAMETER declarations
      character*(*) FUNCNAME
      PARAMETER (FUNCNAME = 'Update_LclInputGranID_MOD04')
...
c--------use different messages depending on whether core metadata exists
            ...
            If (NUM_Old_LCLGRANID .LE. 0) Then  ! implies core does not exist.
               ...
               msgbuf =
     1         'No preexisting ' // MCORE_LOCALINPUTGRANULEID // ' array in MOD04 product file - '
