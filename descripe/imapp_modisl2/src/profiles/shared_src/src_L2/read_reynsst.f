     ...
     &                          year, month, day, hour, sst, 
     &                          file_format, sst_success ) 
...
c-----Declaration of functions 
      integer  pgs_io_gen_openf, pgs_pc_getfilesize
      external pgs_io_gen_openf, pgs_pc_getfilesize
...
c ... file is formatted NCEP sst
      elseif (filesize .eq. SIZE_OF_REYNSST_FMT) then
         file_format = 'fmt'
         status = pgs_io_gen_openf( pcfnum, PGSd_IO_Gen_RSeqFrm, 
...
c ... file is unformatted NCEP sst
      elseif (filesize .eq. SIZE_OF_REYNSST_UNF) then
         file_format = 'unf'
         status = pgs_io_gen_openf( pcfnum, PGSd_IO_Gen_RSeqUnf,
     1                              reclen, lun, version )
