...
c ... Local variables
      ...
      INTEGER lun, i, j, k, level, ios,
     &  pcfnum, reclen, status, version
         
      REAL x, x0, dx, y, y0, dy, p( 0:15 ), satmix, xlon, 
     &  met_grid( 0:359, 0:180, 0:29 ),
     &  ozn_grid( 0:359, 0:180 ),
     &  ice_grid( 0:719, 0:359 ),
     &  sst_grid( 0:npoints_x-1, 0:npoints_y-1 )
     ...
     &  nise_success      
      ...
      CHARACTER*255 nise_file, grib_name
...
cc----------------------------------------------------------------------
c
c        call read_reynsst ( npoints_x, npoints_y, sst_year, sst_month, 
c     &                      sst_day, sst_hour, sst_grid, sst_file_fmt, 
c     &                      sst_success)
c
c       if( (.not. sst_success) ) then
c         level  =  1
c         status = -1
c         write( errmsg,'(''Problem in read_reynsst '')')
c         call message( 'get_ancillary', errmsg //
c     &      ' [OPERATOR ACTION: Contact SDST]', status, level )
c
c       end if
