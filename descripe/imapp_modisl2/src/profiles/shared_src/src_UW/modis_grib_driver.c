...
/* 05/10/1999 fhliang added the last argument to the interface. */
int modis_grib_driver_( int *inlun1, int *outlun1,
                        int *inlun2, int *outlun2,
                        int *inlun3, int *outlun3,
                        int *met_year, int *met_month, int *met_day, int *met_hour,
                        int *ozn_year, int *ozn_month, int *ozn_day, int *ozn_hour,
                        int *ice_year, int *ice_month, int *ice_day, int *ice_hour,
...
!Output Parameters:
 met_year    year of met. grib file data (YYYY)
 met_month   month of met. grib file data (MM)
 met_day     day of met. grib file data (DD)
 met_hour    two character field for hour of met. grib file data (hh)
 ozn_year    year of ozone grib file data (YYYY)
 ozn_month   month of ozone grib file data (MM)
 ozn_day     day of ozone grib file data (DD)
 ozn_hour    two character field for hour of ozone grib file data (hh)
 ice_year    year of nise ice file data (YYYY)
 ice_month   month of nise ice file data (MM)
 ice_day     day of nise ice file data (DD)
 ice_hour    two character field for hour of nise ice file data (hh)
...
!Revision History:
  06/03/1999 fhliang corrected an error: changed 'outlun' to 'inlun'
  around L.275.

  05/10/1999 fhliang added 'errmsg_size' to the interface.
...
/* 05/10/1999 fhliang modified the following line:
    int i, numberofchars, errmsg_size;
*/
    int i, numberofchars;
...
/* 05/10/1999 fhliang modified the following line to use errmsg_size:
   ...
   for (j = 1; j < 4; j++) {
      switch (j) {
         ...
         case 1: numofrecords = sizeof(met_records)/sizeof(grib_record_t);
                 ...
                 /*   Save date and time of MET data   */
                 
                 *met_year  = met_records[0].date.year;
                 *met_month = met_records[0].date.month;
                 *met_day   = met_records[0].date.day;
                 *met_hour  = met_records[0].date.hour;
         ...
         case 2: numofrecords = sizeof(ozn_records)/sizeof(grib_record_t);
                 ...
                 /*   Save date and time of OZN data   */
                 
                 *ozn_year  = ozn_records[0].date.year;
                 *ozn_month = ozn_records[0].date.month;
                 *ozn_day   = ozn_records[0].date.day;
                 *ozn_hour  = ozn_records[0].date.hour;
         ...
         case 3: numofrecords = sizeof(ice_records)/sizeof(grib_record_t);
                 ...
                 /*   Save date and time of ICE data   */
                 
                 *ice_year  = ice_records[0].date.year;
                 *ice_month = ice_records[0].date.month;
                 *ice_day   = ice_records[0].date.day;
                 *ice_hour  = ice_records[0].date.hour;
...
/* 06/03/1999 fhliang: changed following line
               outlun, ret_pgs);
*/
               inlun, ret_pgs);
      return (-1);
