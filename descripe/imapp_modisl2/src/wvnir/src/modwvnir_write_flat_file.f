...
C
C
      integer function modwvnir_write_flat_file(
     &			    	        req_scan,	 	
     &				        req_samples,
     &				        req_lines,
     &				        req_band,
...
c --- parameters
      ...
      character*(*)	platform_name    ! Aqua or Terra
...
c --- check datatype
      if( fil_datatype.ne.4 ) then
         modwvnir_write_flat_file = -1
         return
      endif

c --- check interleave
      if( fil_interleave(1:3).ne.'bil' ) then
         modwvnir_write_flat_file = -2
         return
      endif

c --- check resolution
      if( fil_resolution.ne.1 ) then
         modwvnir_write_flat_file = -3
         return
      endif

c --- check offset (currently can't do this)
      if( fil_offset.gt.0 ) then
         modwvnir_write_flat_file = -4
         return
      endif
...
c ... Write header file only if hdr_flg is set to one (1)
     ...
     &                      ''}'')') fil_desc(1:length)
          write(hdr_lun,'(''platform = '',A)') platform_name
...
c --- SUCCESS: all records for band were written
      ...
      modwvnir_write_flat_file = 0
      return

c --- FAILED: did not write all records 
9999  continue
      modwvnir_write_flat_file = -7
      return
