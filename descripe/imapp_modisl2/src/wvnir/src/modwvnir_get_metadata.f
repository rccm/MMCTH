...
C
C
      subroutine modwvnir_get_metadata(header_1km,header_geo,
...
c     data for the NIR water vapour MODIS Direct Broadcast algorithm.
C
C !DESCRIPTION:
...
C    platform_name   Platform name (currently Terra or Aqua)
C
C !REVISION HISTORY
C
...
c --- initalize strings
      ...
      parameter (routine_name = 'modwvnir_get_metadata')
      character*72 text
...
c --- platform  
     ...
     &                    platform_name) 
      if( status.lt.0 ) then
         call message( routine_name, 'FAILED - platform ('
     &                 // keyname // ' @ ' // filename // ')', 
     &                 0, 3)
         return
      endif
c --- datatype  
     ...
     &                    datatype_1km) 
      if( status.lt.0 ) then
         call message( routine_name, 'FAILED - 1km datatype ('
     &                 // keyname // ' @ ' // filename // ')', 
     &                 0, 3)
         return
      endif
...
c --- interleave  
     ...
     &                    interleave_1km) 
      if( status.lt.0 ) then
         call message( routine_name, 'FAILED - 1km interleave ('
     &                 // keyname // ' @ ' // filename // ')', 
     &                 0, 3)
         return
      endif
...
c --- offset  
     ...
     &                    offset_1km) 
      if( status.lt.0 ) then
         call message( routine_name, 'FAILED - 1km offset ('
     &                 // keyname // ' @ ' // filename // ')', 
     &                 0, 3)
         return
      endif
...
c --- error  
     ...
     &                    d_value) 
      if( status.lt.0 ) then
         call message( routine_name, 'FAILED - 1km error ('
     &                 // keyname // ' @ ' // filename // ')', 
     &                 0, 3)
         return
      endif
...
c --- get the number of scans in the image  
     ...
     &                    lines_1km) 
      if( status.lt.0 ) then
         call message( routine_name, 'FAILED - 1km lines ('
     &                 // keyname // ' @ ' // filename // ')', 
     &                 0, 3)
         return
      endif
...
c --- get the maximum number of pixels per scan
     ...
     &                    samples_1km) 
      if( status.lt.0 ) then
         call message( routine_name, 'FAILED - 1km samples ('
     &                 // keyname // ' @ ' // filename // ')', 
     &                 0, 3)
         return
      endif
...
c --- get the number of bands
     ...
     &                    bands_1km) 
      if( status.lt.0 ) then
         call message( routine_name, 'FAILED - 1km bands ('
     &                 // keyname // ' @ ' // filename // ')', 
     &                 0, 3)
         return
      endif
...
c --- get the band names and units
     ...
     &                    bandnames_1km(keyindex)) 
      if( status.lt.0 ) then
         call message( routine_name, 'FAILED - 1km band names ('
     &                 // keyname // ' @ ' // filename // ')', 
     &                 0, 3)
         return
      endif
     ...
     &                    bandunits_1km(keyindex)) 
      if( status.lt.0 ) then
         call message( routine_name, 'FAILED - 1km band units ('
     &                 // keyname // ' @ ' // filename // ')', 
     &                 0, 3)
         return
      endif
...
c --- datatype  
     ...
     &                    datatype_geo) 
      if( status.lt.0 ) then
         call message( routine_name, 'FAILED - Geo datatype ('
     &                 // keyname // ' @ ' // filename // ')', 
     &                 0, 3)
         return
      endif
...
c --- interleave  
     ...
     &                    interleave_geo) 
      if( status.lt.0 ) then
         call message( routine_name, 'FAILED - Geo interleave ('
     &                 // keyname // ' @ ' // filename // ')', 
     &                 0, 3)
         return
      endif
...
c --- offset  
     ...
     &                    offset_geo) 
      if( status.lt.0 ) then
         call message( routine_name, 'FAILED - Geo offset ('
     &                 // keyname // ' @ ' // filename // ')', 
     &                 0, 3)
         return
      endif
...
c --- error  
     ...
     &                    d_value) 
      if( status.lt.0 ) then
         call message( routine_name, 'FAILED - Geo error ('
     &                 // keyname // ' @ ' // filename // ')', 
     &                 0, 3)
         return
      endif
...
c --- get the number of scans in the image  
     ...
     &                    lines_geo) 
      if( status.lt.0 ) then
         call message( routine_name, 'FAILED - Geo lines ('
     &                 // keyname // ' @ ' // filename // ')', 
     &                 0, 3)
         return
      endif
...
c --- get the maximum number of pixels per scan
     ...
     &                    samples_geo) 
      if( status.lt.0 ) then
         call message( routine_name, 'FAILED - Geo samples ('
     &                 // keyname // ' @ ' // filename // ')', 
     &                 0, 3)
         return
      endif
...
c --- get the number of bands
     ...
     &                    bands_geo) 
      if( status.lt.0 ) then
         call message( routine_name, 'FAILED - Geo bands ('
     &                 // keyname // ' @ ' // filename // ')', 
     &                 0, 3)
         return
      endif
...
c --- get the band names and units
     ...
     &                    bandnames_geo(keyindex)) 
        if( status.lt.0 ) then
         call message( routine_name, 'FAILED - Geo band names ('
     &                 // keyname // ' @ ' // filename // ')', 
     &                 0, 3)
          return
        endif
...
c ---------------------------------------------------------------------
      ...
      call message( routine_name, text , 0, debug )
c ---------------------------------------------------------------------
