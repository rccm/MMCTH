...
contains
  ...
  subroutine getAlbedoEco (Lat, Long, JulianDay, AlbedoWavelengthFN, EcosystemFN,           &
                           SnowAlbedoFN, NISEFN, emissivity_name, Debug, WavelengthText, ProcessLandAlbedo,  &
                           icec, Wavelength, Albedo, emissivity, Ecosystem, Status                      )
...
!But it is not a coding issue. -- E. Moody
    ...
    character (len = *),                         intent(in ) :: NISEFN, emissivity_name
    ...
    real      (kind = single ),   &
              dimension(:),                      intent(out) :: Wavelength
    real      (kind = single ),   &
              dimension(:,:,:),                  intent(out) :: Albedo, emissivity
    integer   (kind = integer_fourbyte),   &
    ...
    integer  (kind = integer_fourbyte), parameter             :: NumMapCols        = 21600, &
                                                                 NumMapRows        = 10800, &
                                                                 NumSnowTypes      = 2
                                                                 !dry = 1, wet = 2
    ...
    integer  (kind = integer_twobyte),   &
             allocatable, dimension(:,:)                      :: AlbedoMapInt
    ...
    real     (kind = single), dimension(NumSeaIceAlbWaves)    :: meltSeaIceAlbedos,        &
                                                                 drySeaIceAlbedos
    real     (kind = single), allocatable, dimension(:)       :: ProcessMeltSeaIceAlbedos, &
    ...
    !*****************************************************************************
    ! Convert the text wavelength strings to real for future MOD06OD usage:  
    !*****************************************************************************
    do i = 1, NumAlbWavesProcess
      Wavelength(i) = CharToReal(trim(WavelengthText(i)))
    end do
...
!       return  
    ...
    !*****************************************************************************
    allocate( AlbedoMap    (startx:stopx, starty:stopy, 1:NumAlbWavesProcess) )
    allocate( AlbedoMapInt (startx:stopx, starty:stopy                      ) )
    ...
    do i = 1, NumAlbWavesProcess - 1
      ...
      if (ProcessLandAlbedo(i) .and.  index(AlbedoWavelengthFN(i),'notUsed') == 0 ) then
       ...
       !Read in the data:
       HDFstatus = SFrdata(AlbMapSDSID, hdfStart, hdfStride, hdfEdge, AlbedoMapInt)
       !Error checking:
       ...
       !Convert to real, with scale factor applied, and store in appropriate index of AlbedoMap:
...
!       return  
    ...
    if (errorLevel == FAIL) then
       status = error
       call local_message_handler('Problem reported from ReadSnowAlbStats',status,'getAlbedoEco')
       return
    end if
    ...
    do i = 1, NumAlbWavesProcess-1
      ...
      if (.not. FoundWave) then
         status = error
         call local_message_handler('Problem detected in wavelength loop',status,'getAlbedoEco')
         return
      end if
    end do
    ...
    !Allocate the processing albedos:
...
!         return
    ...
    !*****************************************************************************
    call GetNISEType(NISEFN, long, lat, snowIceType, errorLevel)  
    if (errorLevel == FAIL) then
       status = failure
...
! end emissivity additions
          ...
          !Compute the x,y indices from this pixel's lat/long values.
          ...
          if (      (snowIceType(i,j) >= 1)   &
              ...
              if (lat(i,j) >= 0.0) then
                ...
                SELECT CASE (JulianDay)
                  CASE ( (NHMeltBeg-TransitionDays):NHMeltBeg-1)
                    ...
                    Albedo(i,j,:) = (TransitionAlbedo(:) * real(snowIceType(i,j))/100.000)         &
                                  + (WaterAlbedo * real(100-snowIceType(i,j))/100.000)
                  CASE ( NHMeltBeg:NHMeltEnd )
                    !Melt Sea Ice Values:
                    Albedo(i,j,:) = (ProcessMeltSeaIceAlbedos(:) * real(snowIceType(i,j))/100.000) &
                                  + (WaterAlbedo * real(100-snowIceType(i,j))/100.000)
                  ...
                  CASE DEFAULT
                    !Dry Sea Ice values::
                    Albedo(i,j,:) = (ProcessDrySeaIceAlbedos(:) * real(snowIceType(i,j))/100.000) &
                                  + (WaterAlbedo * real(100-snowIceType(i,j))/100.000)
                END SELECT
              ...
              if ((Ecosystem(i,j) == 15) .and. &
                  (      AlbedoMap(xIndex,yIndex,1) > 0.00d0 &
                   .and. AlbedoMap(xIndex,yIndex,1) <= 1.00d0) ) then
                !This pixel is perm snow class and has a valid map value,
                ! so set to the map's value:
                Albedo(i,j,:) = AlbedoMap(xIndex,yIndex,:)
				Albedo(i,j,6) = 1.0 - emissivity(i,j,1)
              else
                !This pixel isn't flag as perm ice eco, so set it to the
                ! statistical value for perm ice:
                Ecosystem(i,j) = 15
                Albedo(i,j,:) = ProcessSnowAlbStats(:, Ecosystem(i,j), 1)
				Albedo(i,j,6) = 1.0 - emissivity(i,j,1)
              end if
