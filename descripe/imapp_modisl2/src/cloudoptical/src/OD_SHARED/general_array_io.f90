 ...
 subroutine read_byte_array(filedata, sdsname, start, stride, edge, dataholder, status) 
  ...
  subroutine read_uint_array(filedata, sdsname, start, stride, edge, use_offset, dataholder, status) 
	...
	do j=1, edge(2)
		do i=1, edge(1)
			...
			if (temp(i,j) /= fill_val) then 
				if (temp(i,j) < 0.) then 
					dataholder(i,j) = 32767 + abs(temp(i,j))
				else 
					dataholder(i,j) = temp(i,j)
				endif
				dataholder(i,j) = (dataholder(i,j) - offset) * scale_factor
			else
 ...
 end subroutine read_uint_array	 
...
! this subroutine is incorrect, but it works because the lat/lon arrays being read are actually 32-bit
 subroutine read_float64_array(filedata, sdsname, start, stride, edge, dataholder, status)
 ...
 end subroutine read_float64_array
...
!   Define dimension names for SDS (all scaled SDS's are two-dimensional)
	...
	deallocate(scaled_data_to_write)

 end subroutine write_scaled_array	 



 subroutine write_int_array2d(filedata, sdsname, start, stride, edge, data_to_write, status) 
