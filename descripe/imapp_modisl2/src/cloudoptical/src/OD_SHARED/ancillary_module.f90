...
!  include 'Atmos_AncData.f90.inc'
  ...
  external modis_grib_driver 
	...
	integer :: file_id, var_id, err_code, start(2), stride(2), edge(2), dummy
...
!-----------------------------------------------------------------------
#ifdef USE_TOAST
		call read_grib_file(trim(ozone_name), "OZONE", ozn_grid, dummy, dummy)	
#endif	
...
!-----------------------------------------------------------------------
		...
		call read_grib_file(trim(ice_name), "SEA_ICE", ice_grid, dummy, dummy)	
...
#ifdef USE_TOAST
		Ts_forint(1,1) = ozn_grid(idx_x(1)-1, idx_y(1)-1, 1)
		Ts_forint(1,2) = ozn_grid(idx_x(1)-1, idx_y(2)-1, 1)
		Ts_forint(2,1) = ozn_grid(idx_x(2)-1, idx_y(1)-1, 1)
		Ts_forint(2,2) = ozn_grid(idx_x(2)-1, idx_y(2)-1, 1)
...
! collection 4 albedo determination
  use GeneralAuxType
  use nonscience_parameters
  use modis_albedo, only : getAlbedoEco, init_snow_stats
  ...
  use MOD06AlbedoEcoModule, only: init_NISE_processing
  implicit none
...
! collection 4 albedo determination
  use GeneralAuxType
  use nonscience_parameters
  use seviri_albedo, only : getAlbedoEco, init_snow_stats
  use core_arrays, only: cloudsummary, cloudmask
  use names, only : MYDAY
  use mod06_run_settings
  use MOD06AlbedoEcoModule, only: init_NISE_processing
