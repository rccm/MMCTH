 ...
 subroutine check_datasources(l1bname,                   &
	...
	do i=1, 3
		...
		if (allstatus(23+i) /= success) then
			status = failure
			call local_message_handler ('Error noted: water sdev library file. Check PCF and input files.' ,&
                                   status, &
                                   'check_datasources')
		endif
		...
		if (allstatus(29+i) /= success) then
			status = failure
			call local_message_handler ('Error noted: ice sdev library file. Check PCF and input files.' ,&
                                   status, &
                                   'check_datasources')
		endif
    ...
    if (allstatus(34) /= success) then
      status = failure
      call local_message_handler ('Error noted: emissivity library file. Check PCF and input files.' ,&
                                   status, &
                                   'check_datasources')
    endif
