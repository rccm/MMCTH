 ...
 subroutine read_float_array(filedata, sdsname, start, stride, edge, dataholder, status) 
	...
	real*8 :: fill_val_double
 
	integer :: file_id, var_id, err_code, attr_id, dum, dum_arr(10), dtype
	...
	err_code = sfginfo(var_id, cdum, dum, dum_arr, dtype, dum)
	
	attr_id = sffattr(var_id, "_FillValue")

	if (dtype == DFNT_DOUBLE) then
	

		err_code = sfrattr(var_id, attr_id, fill_val_double)
		allocate(temp(edge(1), edge(2)))
		err_code = sfrdata(var_id, start, stride, edge, temp)
 ...
 subroutine set_processing_attributes(filedata)
  ...
  if (directive == 'open') then
	...
	do i=1, nbands
		l1b_filedata(i) = sfstart(l1bname(i), DFACC_READ)
	    if(l1b_filedata(i) == -1) then
	...
	if (nbands == 1) then 
		if (l1bname(2) /= "none") then 
			l1b_filedata(2) = sfstart(l1bname(2), DFACC_READ)
			if(l1b_filedata(2) == -1) then
				CALL MODIS_SMF_SETDYNAMICMSG(2,'MAPI open failed for HKM file, Check PCF.','openclose_files')
				status = failure
			else
				call MODIS_SMF_SETDYNAMICMSG(0,'MAPI open for L1B file OK','openclose_files')
			endif 
...
#ifdef AMS_INST
  geolocation_filedata(1) = mod06_filedata(1)
#else
  geolocation_filedata(1) = sfstart(geolocation_name, DFACC_READ)
	...
	do i=1, nbands
		err_code = sfend(l1b_filedata(i))
	    if (err_code /= 0) then
	      CALL MODIS_SMF_SETDYNAMICMSG(2,'MAPI close failed for L1B file, Check PCF.','openclose_files')
	    else
	      call MODIS_SMF_SETDYNAMICMSG(0,'MAPI close for L1B file OK','openclose_files')
	    endif
	end do
	
	if (nbands == 1) then 
		if (l1b_filedata(2) /= -1) then 
			err_code = sfend(l1b_filedata(2))
			if (err_code /= 0) then
				CALL MODIS_SMF_SETDYNAMICMSG(2,'MAPI close failed for HKM file, Check PCF.','openclose_files')
			else
				call MODIS_SMF_SETDYNAMICMSG(0,'MAPI close for HKM file OK','openclose_files')
			endif
...
#if MODIS_INST || SEVIRI_INST || MAS_INST || EMAS_INST
...
!    Quality Assurance 1KM, byte 7 ----------------------------------------------------------------------------
		...
		if (processing_information(i,j)%path_and_outcome_16 > 8) &
			quality_assurance_1km(7,i,j) = ibset(quality_assurance_1km(7,i,j), 0)			
		if (processing_information(i,j)%path_and_outcome_16_PCL > 8) &
			quality_assurance_1km(7,i,j) = ibset(quality_assurance_1km(7,i,j), 1)			

		if (processing_information(i,j)%path_and_outcome_37 > 8) &
			quality_assurance_1km(7,i,j) = ibset(quality_assurance_1km(7,i,j), 2)			
		if (processing_information(i,j)%path_and_outcome_37_PCL > 8) &
			quality_assurance_1km(7,i,j) = ibset(quality_assurance_1km(7,i,j), 3)			

		if (processing_information(i,j)%path_and_outcome_1621 > 8) &
			quality_assurance_1km(7,i,j) = ibset(quality_assurance_1km(7,i,j), 4)			
		if (processing_information(i,j)%path_and_outcome_PCL > 8) &
			quality_assurance_1km(7,i,j) = ibset(quality_assurance_1km(7,i,j), 5)			
