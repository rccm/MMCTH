...
!  get 1621 retrieval uncertainty estimate
			...
			if (nearest_used(re16) == 1 .and. failure_metric_16(i,j)%re > 0.) then 
				effective_radius_16_error(i,j) = nint (MAX_UNCERTAINTY / unc_scale_factor)
				liquid_water_path_16_error(i,j) = nint (MAX_UNCERTAINTY / unc_scale_factor)
			endif

			if (nearest_used(re21) == 1 .and. failure_metric(i,j)%re > 0.) then 
				effective_radius_21_error(i,j) = nint (MAX_UNCERTAINTY / unc_scale_factor)
				liquid_water_path_error(i,j) = nint (MAX_UNCERTAINTY / unc_scale_factor)
				optical_thickness_error(i,j) = nint (MAX_UNCERTAINTY / unc_scale_factor)
			endif

			if (nearest_used(re37) == 1 .and. failure_metric_37(i,j)%re > 0.) then 
				effective_radius_37_error(i,j) = nint (MAX_UNCERTAINTY / unc_scale_factor)
				liquid_water_path_37_error(i,j) = nint (MAX_UNCERTAINTY / unc_scale_factor)
			endif

			if (nearest_used(re1621) == 1 .and. failure_metric_1621(i,j)%re > 0.) then 
				effective_radius_1621_error(i,j) = nint (MAX_UNCERTAINTY / unc_scale_factor)
				liquid_water_path_1621_error(i,j) = nint (MAX_UNCERTAINTY / unc_scale_factor)
				optical_thickness_1621_error(i,j) = nint (MAX_UNCERTAINTY / unc_scale_factor)
			endif
