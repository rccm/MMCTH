...
contains
    ...
    !Define the Number of Points to read:
		...
		do i=1, num_channels
			...
			if (platform_name(1:4) .eq. 'EMAS') then
		
				if (channel_number == 26) sds_name = "emiss7" ! 3.7
				if (channel_number == 28) sds_name = "emiss10" ! 7.3
				if (channel_number == 30) sds_name = "emiss11" ! 8.5
				if (channel_number == 33) sds_name = "emiss14" ! 11
				if (channel_number == 34) sds_name = "emiss15" ! 12
				if (channel_number >= 36) sds_name = "emiss16"   ! any CO2 bands
...
! put the emissivity values where they belong 
	...
	include "dffunc.f90"
		...
		character(*), intent(in) :: emissivity_name
		integer, dimension(:), intent(in) :: channels
		integer*2, dimension(:,:,:), intent(inout) :: sfc_emis_array
	...
	! emissivity map variables
		...
		integer*2, parameter :: emissivity_fill = 985 ! according to Wisconsin
		real, parameter :: ScaleFactor = 0.001
!		integer*2, dimension(:,:), allocatable :: emissivity_map_int
...
!		allocate(emissivity_map_int(edge(1), edge(2)))
		...
		do i=1, num_channels
			...
			var_id = sfselect(file_id, sfn2index(file_id, trim(sds_name)))
			err_code = sfrdata(var_id, start, stride, edge, sfc_emis_array(:,:,i))
			err_code = sfendacc(var_id)
			
!			sfc_emis_array(:,:,i) = emissivity_map_int 
