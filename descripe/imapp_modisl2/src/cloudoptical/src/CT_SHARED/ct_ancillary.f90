...
subroutine read_ancillary_grids(  lat, lon, ncepgdas_name,ncepgdas_name2,  &
                                 sfctmp  )
  ...
  use names, only: MYTIME, MYMONTH, Atmp_dirname, MYDAY, MYYEAR
#ifndef AMS_INST  
  use rtm_support
#endif
...
!  include 'Atmos_AncData.f90.inc'
  ...
  character*500  temp_output_name, temp_input_name
  ...
  external modis_grib_driver 
	...
	real :: cs_rad_coarse(grid_xsize, grid_ysize, set_number_of_bands), cs_bt_coarse(grid_xsize, grid_ysize, set_number_of_bands)
	real :: sfc_emis(set_number_of_bands)
	...
	integer :: prev_i, prev_j, imodel, jmodel, icntx, icnty, dummy
...
#ifdef USE_GDAS
		call read_grib_file(trim(ncepgdas_name), "NCEP_GDAS", met_grid, met_date(4), MYMONTH)	
		call read_grib_file(trim(ncepgdas_name2), "NCEP_GDAS", met_grid2, met_date2(4), dummy)	
#endif
#ifdef USE_ECMWF
		call read_grib_file(trim(ncepgdas_name), "ECMWF", met_grid, met_date(4), MYMONTH)	
		call read_grib_file(trim(ncepgdas_name2), "ECMWF", met_grid2, met_date2(4), dummy)	
#endif
	...
	do j=1, grid_ysize
		...
		do i=1, grid_xsize
			...
			model_info(i,j)%wind_speed = sqrt( linearinterpolation( xx, yy, frac_time) **2 + &
									linearinterpolation( xx, yy2, frac_time*1.) **2 )
...
! if we have ECMWF this W2m is not an RH, but a dew point temperature.
			...
			Pmsl = linearinterpolation( xx, (/ met_grid(ii,jj,wind_start+5), &
											met_grid2(ii,jj, wind_start+5) /), frac_time)* 0.01
#else
			...
			model_info(i,j)%col_o3 = linearinterpolation( xx, (/ met_grid(ii,jj,wind_start+6), &
											met_grid2(ii,jj, wind_start+6) /), frac_time)
			...
			do k=1, 6
				o3_temp(k) = linearinterpolation( xx, (/ met_grid(ii,jj,o3_start+k), &
											met_grid2(ii,jj, o3_start+k) /), frac_time)			
			end do
...
!			if (i==4 .and. (j==116 .or. j==117)) then 
!				print*, i,j
!				do k=1, model_info(i,j)%surface_level
!					print*, model_info(i,j)%height_profile(k), model_info(i,j)%temp_profile(k)
!				end do			
!			endif
...
!******** begin test
#if !SEVIRI_INST && !AMS_INST
	prev_i = 0
	prev_j = 0

	PRN = .false.

	do icnty=1, data_ysize
		do icntx=1, data_xsize

			call get_model_idx(lat(icntx, icnty), lon(icntx, icnty), imodel, jmodel)

			if ( imodel /= prev_i .or. jmodel /= prev_j) then

				sfc_emis = surface_emissivity_land(icntx, icnty, :)

				call get_rtm_parameters(platform_name, sfc_emis, &
						sensor_zenith_angle(icntx, icnty), 0.0, imodel, jmodel, 1,1)


				call get_clear_toa_rad(platform_name, model_info(imodel, jmodel)%Ts, sfc_emis, &
							model_info(imodel, jmodel)%surface_level*1, &
							cs_rad_coarse(imodel, jmodel,:), cs_bt_coarse(imodel, jmodel,:), &
						   rtm_rad_atm_clr, rtm_trans_atm_clr, PRN)
...
!******** begin test
#ifndef SEVIRI_INST
		do k=1, set_number_of_bands
			
			Ts_forint(1,1) = cs_rad_coarse(idx_x(1), idx_y(1), k)
			Ts_forint(1,2) = cs_rad_coarse(idx_x(1), idx_y(2), k)
			Ts_forint(2,1) = cs_rad_coarse(idx_x(2), idx_y(1),k)
			Ts_forint(2,2) = cs_rad_coarse(idx_x(2), idx_y(2),k)

	        clear_sky_rad(grid_i, grid_j,k) = bilinear_interpolation(  lon_set, &
					lat_set, lon(grid_i, grid_j), lat(grid_i, grid_j), Ts_forint, 1)


			Ts_forint(1,1) = cs_bt_coarse(idx_x(1), idx_y(1), k)
			Ts_forint(1,2) = cs_bt_coarse(idx_x(1), idx_y(2), k)
			Ts_forint(2,1) = cs_bt_coarse(idx_x(2), idx_y(1),k)
			Ts_forint(2,2) = cs_bt_coarse(idx_x(2), idx_y(2),k)

	        clear_sky_btemp(grid_i, grid_j,k) = bilinear_interpolation(  lon_set, &
					lat_set, lon(grid_i, grid_j), lat(grid_i, grid_j), Ts_forint, 1)
