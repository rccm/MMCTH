...
C
C
      integer function read_int(lun_sst,iword,iret)
...
C   routine works for words of type INTEGER
C
C !INPUT PARAMETERS:
...
C
C !OUTPUT PARAMETERS:
C  iret           Function result 0=success, -1=fail
C  read_int       Output integer sst value read from file
C  
C
C !REVISION HISTORY:
C
...
c ...   external functions
        ...
        read(lun_sst,rec=iword,iostat=ios) ival
        if( ios.eq.0 ) then
           if(.not.big_endian()) ival = siflip(ival)
           read_int = ival
           iret = 0
        else
           iret = -1
