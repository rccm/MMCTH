...
c --- internal variables
      character*255	mod28_band
      character*255	mod28_bands
      integer		beg_record
      integer		end_record
      integer		record
      integer		i
...
c --- check if request band is contained in the file
      remove_all = .TRUE.
      mod28_band = req_bandname
      call strcompress( mod28_band, remove_all, len_req )
      do i = 1, fil_bands
      mod28_bands = fil_bandnames(i)
      call strcompress( mod28_bands, remove_all, len_fil )
      if(mod28_band(1:len_req).eq.mod28_bands(1:len_fil)) then
         iband = i
         goto 10
      endif
      enddo
