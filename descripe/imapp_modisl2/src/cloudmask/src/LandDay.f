...
c
c      For daytime land the groups are:
...
c
c!Revision History:
c Changed implementation of 11-12 um thin cirrus test (Key version)
...
c ... local scalars ..
     ...
     +     cmin2,cmin3,cmin4,Rel_equality_EPS,groups,fac,pre_confdnc,
...
c     H20 vapor channel (6.7 micron) high cloud test
      if (nint(masir65) .ne. nint(bad_data)) then
        ...
        call conf_test(masir65,dlh20(1),dlh20(3),dlh20(4),
     *                dlh20(2),1,c2)
        cmin1 = min(cmin1,c2)
        ngtests(1) = ngtests(1) + 1
      endif
