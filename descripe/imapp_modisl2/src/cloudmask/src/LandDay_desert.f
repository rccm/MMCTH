...
c!Description:
c
c      Routine for performing clear sky tests over desert and arid
c      surfaces during daylight hours.
c
c      For daytime desert the groups are:
...
c logical hi_elev    Logical variable indicating high elevation (> 2000 m)
c
c!Output Parameters:
...
c     H20 vapor channel (6.7 micron) high cloud test
      if (nint(masir65) .ne. nint(bad_data)) then
        ...
        call conf_test(masir65,ldsh20(1),ldsh20(3),ldsh20(4),
     *                 ldsh20(2),1,c2)
        cmin1 = min(cmin1,c2)
        ngtests(1) = ngtests(1) + 1
      endif
