...
then
	...
	echo $'\t\t'"3) Full path of level 1b input QKM file (can be 'MISSING')"
	echo $'\t\t'"4) Full path of output level 2 HDF file"
...
# Full path of input level 1b input QKM files
relPath=$6;
if [ "$relPath" != "MISSING" ]; then
  QKM_INPUT_FILE=$(basename $relPath);
  echo $'\n'"QKM input file ="$'\t'$QKM_INPUT_FILE
  QKM_INPUT_FULLPATH=$(cd ${relPath%$QKM_INPUT_FILE};pwd)"/"$QKM_INPUT_FILE;
  echo "QKM Full path ="$'\t'$QKM_INPUT_FULLPATH;
  QKM_INPUT_DIR=$(cd ${QKM_INPUT_FULLPATH%$QKM_INPUT_FILE};pwd);
  echo "QKM path ="$'\t\t'$QKM_INPUT_DIR;
else
  echo "QKM file is MISSING"
  QKM_INPUT_FILE="MISSING"
fi
...
echo "QKM directory       (-l1b_qkm_dir) :"	$QKM_INPUT_DIR
echo "QKM input file         (-qkm_file) :"	$QKM_INPUT_FILE
...
if [ "$QKM_INPUT_FILE" != "MISSING" ]; then

  echo "$ROOT/bin/leocat.exe -satType $SAT_NAME -satYYYDDD $SAT_DATE -satHHMM $SAT_TIME  -wd $LEOCAT_BIN_DIR -nol3 -a $ALG_NUMBER -geo_file $GEO_INPUT_FULLPATH -nise_file $NISE_FILE -sst_file $RSST_FILE -l2_dir $LEVEL2_OUTPUT_DIR -l2_name $LEVEL2_OUTPUT_FILE -nwp $NWP_TYPE -nwp_dir $NWP_DIR -l1b_dir $LEVEL1_INPUT_DIR -f $LEVEL1B_INPUT_FILE -l1b_qkm_dir $QKM_INPUT_DIR -qkm_file $QKM_INPUT_FILE -verbose -algData_dir $ALGDATA_DIR -staticData_dir $STATIC_DATA_DIR -sfc_dir $SFC_DATA_DIR -data $DATA_SRC -gdas_file1 $BASE_GDAS_FILE1 -gdas_file2 $BASE_GDAS_FILE2"$'\n\n'

  $ROOT/bin/leocat.exe -satType $SAT_NAME -satYYYDDD $SAT_DATE -satHHMM $SAT_TIME  -wd $LEOCAT_BIN_DIR -nol3 -a $ALG_NUMBER -geo_file $GEO_INPUT_FULLPATH -nise_file $NISE_FILE -sst_file $RSST_FILE -l2_dir $LEVEL2_OUTPUT_DIR -l2_name $LEVEL2_OUTPUT_FILE -nwp $NWP_TYPE -nwp_dir $NWP_DIR -l1b_dir $LEVEL1_INPUT_DIR -f $LEVEL1B_INPUT_FILE -l1b_qkm_dir $QKM_INPUT_DIR -qkm_file $QKM_INPUT_FILE -verbose -algData_dir $ALGDATA_DIR -staticData_dir $STATIC_DATA_DIR -sfc_dir $SFC_DATA_DIR -data $DATA_SRC -gdas_file1 $BASE_GDAS_FILE1 -gdas_file2 $BASE_GDAS_FILE2

else

  echo "$ROOT/bin/leocat.exe -satType $SAT_NAME -satYYYDDD $SAT_DATE -satHHMM $SAT_TIME  -wd $LEOCAT_BIN_DIR -nol3 -a $ALG_NUMBER -geo_file $GEO_INPUT_FULLPATH -nise_file $NISE_FILE -sst_file $RSST_FILE -l2_dir $LEVEL2_OUTPUT_DIR -l2_name $LEVEL2_OUTPUT_FILE -nwp $NWP_TYPE -nwp_dir $NWP_DIR -l1b_dir $LEVEL1_INPUT_DIR -f $LEVEL1B_INPUT_FILE -verbose -algData_dir $ALGDATA_DIR -staticData_dir $STATIC_DATA_DIR -sfc_dir $SFC_DATA_DIR -data $DATA_SRC -gdas_file1 $BASE_GDAS_FILE1 -gdas_file2 $BASE_GDAS_FILE2"$'\n\n'
...
if [ $? == 1 ]; then
  echo
  echo "********** WARNING: LEOCAT failed for cloud mask **********"
  echo
  exit 1
else
  echo
  echo "********** Successful completion of LEOCAT for cloud mask **********"
  echo
fi
