...
#define HDFEOSVERSION 2.18
#define HDFEOSVERSION1 "2.18"
	...
	*strbufsize = 0;
	...
	if (strcmp(class, type) == 0)
	{
	    ...
	    if ((intn) strlen(tempname) > VGNAMELENMAX)
	      {
		status = -1;
		HEpush(DFE_GENAPP, "EHinquire", __FILE__, __LINE__);
		HEreport("HDF-EOS VGroup \"%s\" name length is more than %d characters.\n", tempname, VGNAMELENMAX);
		return(status);
	      }
	    else
	      {
