...
plt.legend()
plt.grid(True)
plt.savefig('t_profile_org.png')


plt.figure(figsize=(8, 6))
plt.plot(w_org_profile, or_press, marker='o', color='blue',label='37 ERA5 Pressure Levels')
plt.plot(w_int_profile[w_int_profile>0], int_press[w_int_profile>0], marker='x', color='red', label='101 Pressure Levels')
plt.yscale('log') 
plt.gca().invert_yaxis()  # Invert y-axis to show higher pressure levels at the bottom
plt.xlabel('Water Vapor Mixing Ratio')
plt.ylabel('Pressure Level')
plt.title(f'Mixing Ratio Profile at Central Latitude and Longitude')
plt.legend()
plt.grid(True)
plt.savefig('w_profile_org.png')