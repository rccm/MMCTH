...
subroutine process_selected_pixels(wprof, tprof, hprof, psfc, pmsl, surftmp, &
  ...
  integer, intent(in), optional :: cs_idx(:), multi_idx(:)
  ...
  integer :: Processing_Mask_pix

  integer :: pix, i, k
  real    :: bias_w_um(nbct)
  integer :: n_used(nbct)
  logical :: use_bias, use_multi
  ...
  ! ===========================================================
  use_multi = present(multi_idx)

  if (use_multi) then
    do i = 1, size(multi_idx)
      pix = multi_idx(i)

      ! --- gather per-pixel inputs (unchanged) ---
      wprof_pix   = wprof(:, pix)
      tprof_pix   = tprof(:, pix)
      hprof_pix   = hprof(:, pix)
      psfc_pix    = psfc(pix)
      ...
      ! --- First pass with MISR-CTP set to surface pressure (unchanged) -------
      ...
      write(98, '(I8, 6F16.5)') pix, psfc_pix, misr_ctp_pix, Cloud_Top_Pressure(pix), &
                                 Cloud_Top_Height(pix), Cloud_Effective_Emissivity(pix), Cloud_Optical_Depth(pix)

      ! --- Second pass using MISR CTP (unchanged logic) -----------------------
      call co2cld_onepixel_misr( wprof_pix, tprof_pix, hprof_pix, psfc_pix, pmsl_pix, &
                                 surftmp_pix, view_pix, trad_adj, met_date_pix, &
