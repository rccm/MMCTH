  ...
  pair_loop: do id = 1, nsct
    ...
    !  write(*,'("Working on Pair:",i2)') id
     ...
     if (ok) then
        ...
        write(*,*) id, int( lev(id) ), nint( lev(id) )
...
!==============================================================================
  ...
  if (found_solution .and. imisr - lco2 > 1 ) then
    ...
    !rclr(5) = fm_modrad_emis( tpad(imisr), 1.0, pp(imisr), pp, tpad, taup(1,kwc), &
    !                          mbnds(5), imisr )
    !write(*,*) tpad(imisr-2), pp(imisr)
    !rclr = milliWatts per square meter per steradian per cm-1
    !rwarm = fm_modrad_emis( tpad(imisr), emisrw, pp(imisr), pp, tpad, T11,&
    !                        mbnds(kwc), imisr )          ! Ic(Pl)
    !write(*,*) fm_modrad_emis( tpad(imisr), emisrw, pp(imisr), pp, tpad, taup(1,kwc),&
    !                        mbnds(kwc), imisr ) 
    ...
    den = rwcld - rclr(5)          ! should be ≤ 0
    !write(*,*) delr(5), rwcld, rclr(5), tmisr, pp(imisr), imisr
    !rc1   = modis_planck_shift(rtmp1,mbnds(k),1)
    !rtmp2 = modis_bright_shift(rc1(k),mbnds(k),1)
    !rwarm(k) = modis_planck_shift(rtmp2,mbnds(k),0)
...
! Input arrays for selected pixels
  ...
  real, intent(in) :: hprof(plev, npix)              ! Geopotential height profiles
...
! Output arrays
  ...
  real, intent(out) :: Cloud_Top_Height(npix)      ! Upper cloud CTH
...
! Local variables for a single pixel
  ...
  open(unit=98, file='output_.txt', status='unknown', action='write') 
  open(unit=99, file='output.txt', status='unknown', action='write') 
...
! Loop over selected pixels
  do pix = 1, npix
    ...
    ! Call the subroutine for the current pixel with MISR-CTP set to SURF PRES 
    ! This is first run as a 1-LAYER SOLUTION
    ...
    write(98, '(I8, 6F16.5)') pix, psfc_pix, misr_ctp_pix, Cloud_Top_Pressure(pix), &
                              Cloud_Top_Height(pix), &
                              Cloud_Effective_Emissivity(pix), Cloud_Optical_Depth(pix)
    ...
    if (Cloud_Top_Pressure_pix .le. Cloud_Top_Pressure(pix)) then
        !write(*,*) Cloud_Top_Pressure_pix, Cloud_Top_Pressure(pix), Cloud_Top_Pressure(pix) - Cloud_Top_Pressure_pix
        Cloud_Top_Pressure(pix) = Cloud_Top_Pressure_pix
        Cloud_Top_Height(pix)   = Cloud_Top_Height_pix
        Cloud_Effective_Emissivity(pix) = Cloud_Effective_Emissivity_pix
        Cloud_Optical_Depth(pix) = Cloud_Optical_Depth_pix
    ...
    write(99, '(I8, 6F16.5)') pix, psfc_pix, misr_ctp_pix, Cloud_Top_Pressure(pix), &
                              Cloud_Top_Height(pix), &
                              Cloud_Effective_Emissivity(pix), Cloud_Optical_Depth(pix)
