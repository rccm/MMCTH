...
subroutine co2cld_onepixel(wprof, tprof, psfc,pmsl, surftmp, view, tcold,  &
              met_date, rlat, rlon, landsea, bias,ctp,eca)
...
subroutine co2cld_onepixel_misr(wprof, tprof, psfc,pmsl, surftmp, view, tcold, &
              met_date, jday,rlat, rlon, landsea, misr_ctp,ctp,eca)
  ...
  implicit none
  save
  real, intent(in) :: wprof(plev), tprof(plev) 
  real, intent(in) :: psfc, pmsl 
  real, intent(in) :: surftmp, view 
  real, intent(in) :: tcold(nbct) 
  
  integer(kind=4), intent(in) :: met_date(4)! doy of year
  integer, intent(in) :: jday ! doy of year
  real, intent(in) :: rlat, rlon 
  integer, intent(in) :: landsea   ! Changed to integer to match usage
  real, intent(in) ::  misr_ctp 
  
  
  ! Output arrays
  real, intent(out) :: ctp(nsct)
  real, intent(out) :: eca(2)
  ...
  write(*,*) psfc,pmsl, surftmp, view, tcold, met_date,jday, rlat, rlon, landsea,misr_ctp
  ...
  met_year = met_date(1)
  met_day = met_date(3)
  call clozo101(rlat, met_month, ozone) ! ozone return all zeros!!!
...
!c     Get geopotential height profile (km).
  ...
  z = 0.0
  nl = imslp
  call height(pp,tprof,wprof,zs,nl,z) !!! why z never gets used ????
...
!c     Calculate transmittance profiles (101 level fast model).
  do k = 1, ntbct
    kban = mbnds(k)
    call tran_modisd101(met_year, jday, tprof, wprof, ozone, view, &
                       kban, jdet, taup(1,k), plev) ! used to be taup(1,k) why??????
    print*, "transsmivi from surface : ", taup(1,k) 
                       ! why only the top of atmoshpere taup? always return 1 
  enddo
...
300  continue
  if(ltrp .eq. -99) ltrp = lmin
  ptrp = pp(ltrp)
  print*, "tropopause pressure: ", ptrp, ltrp
...
integer, intent(in) :: landsea(npix)  ! Changed to integer to match usage
...
real :: wprof_pix(plev), tprof_pix(plev), psfc_pix, pmsl_pix, surftmp_pix, view_pix
real :: tcold_pix(nbct) 
integer :: met_date_pix(4), landsea_pix  ! Changed to integer
