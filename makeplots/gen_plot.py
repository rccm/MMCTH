...
Fix included (lon in [-180, 180]):
- Longitude contours/ticks no longer create a spurious "constant longitude" line near the dateline.
- We unwrap longitude along cross-track, and also break residual >180° jumps with NaNs before contouring.
"""
...
def plot_mm_multipanel(
    ...
    # ------------------------------------------------------------------
    # FIX: Unwrap/break longitude to avoid dateline contour artifacts
    # lon in [-180, 180]
    # - unwrap along cross-track (axis=1) to make it continuous
    # - then break any residual >180 jumps by inserting NaNs
    # ------------------------------------------------------------------
    # --- robust dateline-safe longitude field for contouring ---
    lon_for_contour = lon_m.astype(np.float64).copy()

    # unwrap along cross-track AND along-track (swath can cross dateline diagonally)
    lon_for_contour = np.rad2deg(np.unwrap(np.deg2rad(lon_for_contour), axis=1))
    lon_for_contour = np.rad2deg(np.unwrap(np.deg2rad(lon_for_contour), axis=0))

    # hard-break any remaining discontinuities (both directions)
    jump_thr = 90.0  # use 90° to be aggressive; 180° sometimes still leaves artifacts
    j1 = np.abs(np.diff(lon_for_contour, axis=1)) > jump_thr
    j0 = np.abs(np.diff(lon_for_contour, axis=0)) > jump_thr
    lon_for_contour[:, 1:][j1] = np.nan
    lon_for_contour[1:, :][j0] = np.nan

    # keep it masked where your geo mask is true
    lon_for_contour = np.ma.masked_where(mask, lon_for_contour)
    ...
    def _fmt_lon(v):
        # display in [-180, 180] even though lon_for_contour may be unwrapped
        vv = ((v + 180) % 360) - 180
    ...
    if add_latlon_lines or show_lat_labels or show_lon_labels:
        ...
        # For lon, use unwrapped field for min/max; levels are still in degrees
        lon_min, lon_max = np.nanmin(lon_for_contour), np.nanmax(lon_for_contour)
    ...
    def add_latlon_contours(ax):
        ...
        if lon_levels is not None:
            # IMPORTANT: contour the unwrapped/broken lon field
            ax.contour(lon_for_contour, levels=lon_levels, colors="k", linewidths=0.4, alpha=0.4, origin="lower")
    ...
    def compute_axis_ticks():
        ...
        if lon_levels is not None and show_lon_labels:
            for i in range(ny):
                if np.isfinite(lon_for_contour[i, :]).sum() > 5:
                    i_edge = i
                    break
    ...
    def compute_right_lon_ticks():
        ...
        for j in range(nx - 1, -1, -1):
            if np.isfinite(lon_for_contour[:, j]).sum() > 5:
                j_right = j
                break
        if j_right is None:
            return [], []

        col_lon = lon_for_contour[:, j_right]
    ...
    if RGB_AX0:
        ...
        if right_lon_yticks:
            x_text = rgb.shape[1] - 1 - 5   # put it INSIDE the right edge
            for y, lab in zip(right_lon_yticks, right_lon_ylabels):
                ax0.text(
                    ...
                    fontsize=10,
                    clip_on=True,            # <-- ensure clipping to axes
                )
