	subroutine tran_modisd101(year, jday, temp, wvmr, ozmr, theta,
     +                            kban,jdet,taut)
...
c ... Argument variables
      real temp(*), wvmr(*), ozmr(*), theta, taut(*)
...
c ... Local variables
      ...
      integer iuc(nk), lengcf(nk), pcfval(nk)
...
c ... Data statements
      data pcfval/dry_pcfnum, ozo_pcfnum, wts_pcfnum,
     &            wtl_pcfnum, wco_pcfnum/
...
c ... Statement function definition
      secant(z) = 1.0 / cos(0.01745329 * z)

c-----------------------------------------------------------------------
c     INITIALIZATION (FIRST PASS ONLY)
c-----------------------------------------------------------------------

c ... Check if initialization is required
      if (init .eq. 1) then
...
c ...   Check the platform name
     ...
     &          platform_name(1:5) .eq. 'terra') then
          ksat=1
        elseif (platform_name(1:4) .eq. 'AQUA' .or.
...
c ...     Open the files
          version = 1
          call modis_io_gen_openf(pcfval(i), PGSD_IO_GEN_RDIRUNF,
     &      lengcf(i), iuc(i), version)
...
c ...   Close the coefficient files
        do i = 1, nk 
          call modis_io_gen_closef(pcfval(i), iuc(i))
        end do

c ...   Convert standard water vapor and ozone profiles
        i = 1
        call conpir(pstd, tstd, wstd, ostd, nl, i, pavg,
     &    tref, wref, oref)
...
c     Reference Atmosphere is 1976 U.S. Standard
      ...
      real pstd(nl),tstd(nl),wstd(nl),ostd(nl)
      common /stdatm/ pstd, tstd, wstd, ostd
      save /stdatm/
